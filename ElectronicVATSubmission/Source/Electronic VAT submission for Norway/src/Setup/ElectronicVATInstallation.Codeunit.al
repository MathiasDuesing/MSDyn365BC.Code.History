codeunit 10681 "Electronic VAT Installation"
{
    ...
    begin
        AssistedSetup.Add(BaseAppID.Get(), Page::"Elec. VAT Submission Wizard", AssistedSetupTxt, AssistedSetupGroup::Connect, '', VideoCategory::Connect, AssistedSetupHelpTxt, AssistedSetupDescriptionTxt);
        if ElecVATSetup.Get() and ElecVATSetup.Enabled then
            AssistedSetup.Complete(Page::"Elec. VAT Submission Wizard");
    end;
    ...
    begin
        if ExtensionId <> BaseAppID.Get() then
            exit;
        if PageID <> Page::"Elec. VAT Submission Wizard" then
            exit;
        if ElecVATSetup.Get() and ElecVATSetup.Enabled then
