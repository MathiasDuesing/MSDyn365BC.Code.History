codeunit 10687 "Elec. VAT Connection Mgt."
{
    ...
    var
        DummyDocumentAttachment: Record "Document Attachment";
        ...
        foreach DataJToken in JToken.AsArray() do begin
            ...
            if DataType in ['valideringsresultat', 'betalingsinformasjon', 'kvittering'] then begin
                ...
                if DataType = 'kvittering' then begin
                    FileType := DummyDocumentAttachment."File Type"::PDF;
                    FileExtension := 'pdf';
                    ...
                    FileType := DummyDocumentAttachment."File Type"::XML;
                    FileExtension := 'xml';
                    MimeType := '';
    ...
    local procedure AttachAttachmentToVATRepHeader(var TempBlob: Codeunit "Temp Blob"; VATReportHeader: Record "VAT Report Header"; FileName: Text; FileExtension: Text; FileType: Integer; MimeType: Text)
    var
