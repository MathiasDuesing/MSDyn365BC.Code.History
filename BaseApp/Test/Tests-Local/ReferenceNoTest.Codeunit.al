codeunit 144005 "Reference No Test"
{
    ...
    [Test]
    [HandlerFunctions('PostedInvoiceReportHandler')]
    [Scope('OnPrem')]
    procedure VerifyRefNumOnPostedInvoice()
    var
        SalesHeader: Record "Sales Header";
        SalesInvoiceHeader: Record "Sales Invoice Header";
        PostedDocumentNo: Code[20];
    begin
        // Setup Reference No. generation to generate numbers and set Reference No. to be printed
        // Verify that Reference No. is set on Sales Invoice Header and present on Report
        // Setup
        Initialize;
        SetupRefNumOnSalesAndReceivablesSetup(RefNoNoSeriesCode, true);

        // Exercise
        PostedDocumentNo := CreateAndPostSalesDocument(SalesHeader."Document Type"::Invoice);
        ...
        // Verify
        Assert.AreNotEqual('', SalesInvoiceHeader."Reference No.", 'Reference No. was not set on Invoice');
        VerifyPostedInvoiceReferenceNo(SalesInvoiceHeader."Reference No.");
    end;

    [Test]
    [HandlerFunctions('PostedInvoiceReportHandler')]
    [Scope('OnPrem')]
    procedure VerifyNoRefNumOnPostedInvoice()
    var
        SalesHeader: Record "Sales Header";
        SalesInvoiceHeader: Record "Sales Invoice Header";
        PostedDocumentNo: Code[20];
    begin
        ...
        // Verify that Reference No. is set on Sales Invoice Header, but not present on Report
        // Setup
        Initialize;
        ...
        // Verify
        Assert.AreNotEqual('', SalesInvoiceHeader."Reference No.", 'Reference No. was not set on Invoice');
        VerifyPostedInvoiceReferenceNo('');
    end;

    [Test]
    [HandlerFunctions('PostedInvoiceReportHandler')]
    [Scope('OnPrem')]
    procedure VerifyRefNumOnShippedInvoice()
    var
        SalesHeader: Record "Sales Header";
        SalesInvoiceHeader: Record "Sales Invoice Header";
        PostedDocumentNo: Code[20];
    begin
        ...
        // Exercise
        PostedDocumentNo := CreateAndPostSalesDocument(SalesHeader."Document Type"::Order);
        SalesInvoiceHeader.Get(PostedDocumentNo);
    ...
    [HandlerFunctions('PostedInvoiceReportHandler')]
    [Scope('OnPrem')]
    procedure VerifyNoRefNumOnShippedInvoice()
    var
        SalesHeader: Record "Sales Header";
        SalesInvoiceHeader: Record "Sales Invoice Header";
        PostedDocumentNo: Code[20];
    ...
    [HandlerFunctions('PostedInvoiceReportHandler')]
    [Scope('OnPrem')]
    procedure VerifyRefNumOnPrepaymentInvoice()
    var
        SalesHeader: Record "Sales Header";
    ...
    begin
        ...
        // Verify that Reference No. is set on Sales Invoice Header and present on PrePayment Invoice
        // Setup
        Initialize;
        SetupRefNumOnSalesAndReceivablesSetup(RefNoNoSeriesCode, true);

        CreateSalesHeaderWithPrepaymentPercentage(SalesHeader, LibrarySales.CreateCustomerNo);
        CreateSalesLineWithQtyToShip(SalesLine, SalesHeader);

        // Exercise
        SalesInvoiceHeader.FindLast;
        PreviousPostedDocumentNo := SalesInvoiceHeader."No.";
        LibrarySales.PostSalesPrepaymentInvoice(SalesHeader);
        SalesInvoiceHeader.FindLast;
        PostedDocumentNo := SalesInvoiceHeader."No.";
        Assert.IsTrue(PreviousPostedDocumentNo < PostedDocumentNo, 'Unable to find PrePayment Invoice');
        Commit();
        ...
        // Verify
        Assert.AreNotEqual('', SalesInvoiceHeader."Reference No.", 'Reference No. was not set on PrePayment Invoice');
        VerifyPostedInvoiceReferenceNo(SalesInvoiceHeader."Reference No.");
    end;
    ...
    [HandlerFunctions('PostedInvoiceReportHandler')]
    [Scope('OnPrem')]
    procedure VerifyNoRefNumOnPrepaymentInvoice()
    var
        ...
        PreviousPostedDocumentNo: Code[20];
    begin
        // Setup Reference No. generation to generate numbers and to set Reference No. to NOT be printed.
        // Verify that Reference No. is set on Sales Invoice Header, but not present on PrePayment Invoice
        // Setup
        Initialize;
        SetupRefNumOnSalesAndReceivablesSetup(RefNoNoSeriesCode, false);

        LibrarySales.CreateCustomer(Customer);
        CreateSalesHeaderWithPrepaymentPercentage(SalesHeader, Customer."No.");
        CreateSalesLineWithQtyToShip(SalesLine, SalesHeader);
    ...
    [RequestPageHandler]
    [Scope('OnPrem')]
    procedure PostedInvoiceReportHandler(var SalesInvoiceRequestPage: TestRequestPage "Sales - Invoice")
    begin
        SalesInvoiceRequestPage."Sales Invoice Header".SetFilter("No.", LibraryVariableStorage.DequeueText);
