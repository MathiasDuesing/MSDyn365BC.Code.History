codeunit 144065 "Test CH VATSTAT"
{
    ...
    [Test]
    [HandlerFunctions('AdjustExchageRatesRequestPageHandler,ConfirmHandler,MessageHandler,GLVATReconciliationReportHandler')]
    [Scope('OnPrem')]
    procedure TestAdjustVATBaseAmountsOnVATEntriesAndRunGLVATReconciliation()
    var
        VATPostingSetup: Record "VAT Posting Setup";
    begin
        LibraryERM.FindZeroVATPostingSetup(VATPostingSetup, VATPostingSetup."VAT Calculation Type"::"Normal VAT");
        ExecuteAdjustExchangeRelatedTestWithGLVATReconciliationReport(false, VATPostingSetup."VAT Bus. Posting Group", VATPostingSetup."VAT Prod. Posting Group", true);
    end;
    ...
    begin
        ExecuteAdjustExchangeRelatedTestWithGLVATReconciliationReport(IsLocalWithForeignCurrency, VATBusPostingGroup, VATProdPostingGroup, false);
    end;

    local procedure ExecuteAdjustExchangeRelatedTestWithGLVATReconciliationReport(IsLocalWithForeignCurrency: Boolean; VATBusPostingGroup: Code[20]; VATProdPostingGroup: Code[20]; RunGLVATReconciliationReport: Boolean)
    var
    ...
    begin
        ...
        if RunGLVATReconciliationReport then begin
