codeunit 142092 "ERM Sales/Purchase Tax III"
{
    ...
    [Test]
    [Scope('OnPrem')]
    procedure SalesOrder_TaxArea_Two_TaxGroup_One()
    var
    ...
    begin
        // [FEATURE] [Sales]
        // [SCENARIO 325954] System calculates Tax Amount and Amount Including Tax for the document having lines with different Tax Areas
        Initialize();

        // [GIVEN] 3 Tax Jurisdictions "TJ[1]", "TJ[1]", "TJ[2]"
        // [GIVEN] 1 Tax Group "TG"
        // [GIVEN] 2 Tax Areas "TA[1]", "TA[2]"
        // [GIVEN] Tax Area Line[1] "TA[1]", "TJ[1]" => 2%
        // [GIVEN] Tax Area Line[2] "TA[1]", "TJ[2]" => 3%
        // [GIVEN] Tax Area Line[3] "TA[2]", "TJ[1]" => 2%
        // [GIVEN] Tax Area Line[4] "TA[2]", "TJ[3]" => 5%
        ...
        // [WHEN] Change tax areas in lines 
        // [THEN] Line[1]: Amount Incl. Tax = 1000 * (1 + (2% + 5%)) = 1070
        // [THEN] Line[2]: Amount Incl. Tax = 1000 * (1 + (2% + 3%)) = 1050
        CreateTaxSetup_VSO325954(TaxArea, TaxGroup, TaxDetail);

        LibrarySales.CreateSalesHeader(
          SalesHeader, SalesHeader."Document Type"::Order, LibrarySales.CreateCustomerWithVATBusPostingGroup(''));
        SalesHeader.Validate("Tax Liable", true);
        SalesHeader.Validate("Tax Area Code", TaxArea[1].Code);
        SalesHeader.Modify(true);

        LibrarySales.CreateSalesLine(
          SalesLine[1], SalesHeader, SalesLine[1].Type::"G/L Account", CreateGLAccountNoWithBlankVATSetup, 1);
        LibrarySales.CreateSalesLine(
          SalesLine[2], SalesHeader, SalesLine[2].Type::"G/L Account", CreateGLAccountNoWithBlankVATSetup, 1);

        SalesOrder.OpenEdit();
        SalesOrder.FILTER.SetFilter("No.", SalesHeader."No.");

        SalesOrder.SalesLines."Unit Price".SetValue(1000);
        SalesOrder.SalesLines."Tax Group Code".SetValue(TaxGroup.Code);
        SalesOrder.SalesLines."Tax Area Code".SetValue(TaxArea[1].Code);
        SalesOrder.SalesLines.Next();
        SalesOrder.SalesLines."Unit Price".SetValue(300);
        SalesOrder.SalesLines."Tax Group Code".SetValue(TaxGroup.Code);
        SalesOrder.SalesLines."Tax Area Code".SetValue(TaxArea[1].Code);
        SalesOrder.SalesLines.Prev();
        SalesOrder.SalesLines."Tax Area Code".SetValue(TaxArea[2].Code);
        SalesOrder.SalesLines.Next();
        SalesOrder.Close();
        ...
        SalesLine[1].TestField(
          "Amount Including VAT",
          Round(SalesLine[1]."Unit Price" * (100 + TaxDetail[1]."Tax Below Maximum" + TaxDetail[3]."Tax Below Maximum") / 100));
        SalesLine[2].TestField(
          "Amount Including VAT",
          Round(SalesLine[2]."Unit Price" * (100 + TaxDetail[1]."Tax Below Maximum" + TaxDetail[2]."Tax Below Maximum") / 100));
    end;

    [Test]
    [Scope('OnPrem')]
    procedure PurchaseOrder_TaxArea_Two_TaxGroup_One()
    var
        TaxArea: array[2] of Record "Tax Area";
        TaxGroup: Record "Tax Group";
        TaxDetail: array[3] of Record "Tax Detail";
    ...
    begin
        ...
        LibraryPurchase.CreatePurchHeader(
          PurchaseHeader, PurchaseHeader."Document Type"::Order, LibraryPurchase.CreateVendorWithVATBusPostingGroup(''));
        PurchaseHeader.Validate("Tax Liable", true);
        PurchaseHeader.Validate("Tax Area Code", TaxArea[1].Code);
        PurchaseHeader.Modify(true);

        LibraryPurchase.CreatePurchaseLine(
          PurchaseLine[1], PurchaseHeader, PurchaseLine[1].Type::"G/L Account", CreateGLAccountNoWithBlankVATSetup, 1);
        PurchaseLine[1].Validate("Tax Group Code", TaxGroup.Code);
        PurchaseLine[1].Modify(true);

        LibraryPurchase.CreatePurchaseLine(
          PurchaseLine[2], PurchaseHeader, PurchaseLine[2].Type::"G/L Account", CreateGLAccountNoWithBlankVATSetup, 1);
        PurchaseLine[2].Validate("Tax Group Code", TaxGroup.Code);
        PurchaseLine[2].Modify(true);

        PurchaseOrder.OpenEdit();
        PurchaseOrder.FILTER.SetFilter("No.", PurchaseHeader."No.");

        PurchaseOrder.PurchLines."Direct Unit Cost".SetValue(1000);
        PurchaseOrder.PurchLines."Tax Area Code".SetValue(TaxArea[1].Code);
        PurchaseOrder.PurchLines.Next();
        PurchaseOrder.PurchLines."Direct Unit Cost".SetValue(300);
        PurchaseOrder.PurchLines."Tax Area Code".SetValue(TaxArea[1].Code);
        PurchaseOrder.PurchLines.Prev();
        PurchaseOrder.PurchLines."Tax Area Code".SetValue(TaxArea[2].Code);
        PurchaseOrder.PurchLines.Next();
        PurchaseOrder.Close();
        ...
        PurchaseLine[1].TestField(
          "Amount Including VAT",
          Round(
            PurchaseLine[1]."Direct Unit Cost" * (100 + TaxDetail[1]."Tax Below Maximum" + TaxDetail[3]."Tax Below Maximum") / 100));
        PurchaseLine[2].TestField(
          "Amount Including VAT",
          Round(
            PurchaseLine[2]."Direct Unit Cost" * (100 + TaxDetail[1]."Tax Below Maximum" + TaxDetail[2]."Tax Below Maximum") / 100));
    end;
    ...
    local procedure CreateTaxSetup_VSO325954(var TaxArea: array[2] of Record "Tax Area"; var TaxGroup: Record "Tax Group"; var TaxDetail: array[3] of Record "Tax Detail")
    var
        TaxJurisdiction: array[3] of Record "Tax Jurisdiction";
    begin
        TaxArea[1].Get(LibraryERMTax.CreateTaxArea_CA());
        TaxArea[2].Get(LibraryERMTax.CreateTaxArea_CA());

        TaxGroup.Get(LibraryERMTax.CreateTaxGroupCode());

        TaxJurisdiction[1].Get(LibraryERMTax.CreateTaxJurisdiction_CA());
        TaxJurisdiction[2].Get(LibraryERMTax.CreateTaxJurisdiction_CA());
        TaxJurisdiction[3].Get(LibraryERMTax.CreateTaxJurisdiction_CA());

        LibraryERMTax.CreateTaxAreaLine(TaxArea[1].Code, TaxJurisdiction[1].Code);
        LibraryERMTax.CreateTaxAreaLine(TaxArea[1].Code, TaxJurisdiction[2].Code);
        LibraryERMTax.CreateTaxAreaLine(TaxArea[2].Code, TaxJurisdiction[1].Code);
        LibraryERMTax.CreateTaxAreaLine(TaxArea[2].Code, TaxJurisdiction[3].Code);

        LibraryERMTax.CreateTaxDetailWithTaxType(
          TaxDetail[1], TaxJurisdiction[1].Code, TaxGroup.Code, TaxDetail[1]."Tax Type"::"Sales Tax Only", 2, 0);
        LibraryERMTax.CreateTaxDetailWithTaxType(
          TaxDetail[2], TaxJurisdiction[2].Code, TaxGroup.Code, TaxDetail[2]."Tax Type"::"Sales Tax Only", 3, 0);
        LibraryERMTax.CreateTaxDetailWithTaxType(
          TaxDetail[3], TaxJurisdiction[3].Code, TaxGroup.Code, TaxDetail[3]."Tax Type"::"Sales Tax Only", 5, 0);
    end;
