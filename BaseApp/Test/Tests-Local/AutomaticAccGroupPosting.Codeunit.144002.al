codeunit 144002 "Automatic Acc. Group Posting"
{
    ...
    var
        ...
        WrongDimValueErr: Label 'Wrong value in %1 in G/L Entry.';

    [Test]
    [Scope('OnPrem')]
    procedure CheckDimsInGLEntryAfterCopyingIfEmpty()
    var
    ...
    begin
        ...
        DimSetID := CreateDimSetByDimValue(0, DimensionValue);
        CreateAutomaticAccGroupWithDimensions(AutomaticAccLine, DimSetID);

        // Execute
        GenJnlLine.CopyDimensionFromAutoAccLine(AutomaticAccLine);

        // Verify
        Assert.AreEqual(
          DimSetID, GenJnlLine."Dimension Set ID",
          StrSubstNo(WrongDimValueErr, Format(GenJnlLine."Dimension Set ID")));
        VerifyGlobalShortcutDimValuesInGenJnlLine(GenJnlLine, DimensionValue.Code, '');
    end;

    [Test]
    [Scope('OnPrem')]
    procedure CheckDimsInGLEntryAfterCopyingIfNotEmpty()
    var
        GLSetup: Record "General Ledger Setup";
        DimensionValue: Record "Dimension Value";
        AutomaticAccLine: Record "Automatic Acc. Line";
    ...
    begin
        ...
        LibraryDimension.CreateDimensionValue(DimensionValue, GLSetup."Global Dimension 1 Code");
        GenJnlLineDimSetID := CreateDimSetByDimValue(0, DimensionValue);
        CreateGenJnlLineWithDimSetID(GenJnlLine, GenJnlLineDimSetID);
    ...
    [Test]
    [Scope('OnPrem')]
    procedure CheckMergedDimsInGLEntryAfterCopyingIfNotEmpty()
    var
        GLSetup: Record "General Ledger Setup";
        DimensionValue: Record "Dimension Value";
        AutomaticAccLine: Record "Automatic Acc. Line";
    ...
    begin
        ...
        GenJnlLineDimSetID := CreateDimSetByDimValue(0, DimensionValue);
        CreateGenJnlLineWithDimSetID(GenJnlLine, GenJnlLineDimSetID);
        DimValueCodeInGenJnlLine := DimensionValue.Code;
        ...
        // Verify
        Assert.AreNotEqual(
          GenJnlLineDimSetID, GenJnlLine."Dimension Set ID",
          StrSubstNo(WrongDimValueErr, Format(GenJnlLine."Dimension Set ID")));
        Assert.AreNotEqual(
          DimSetID, GenJnlLine."Dimension Set ID",
          StrSubstNo(WrongDimValueErr, Format(GenJnlLine."Dimension Set ID")));
        VerifyGlobalShortcutDimValuesInGenJnlLine(
          GenJnlLine, DimValueCodeInGenJnlLine, DimensionValue.Code);
    end;
    ...
    local procedure CreateAutomaticAccGroupWithDimensions(var AutomaticAccLine: Record "Automatic Acc. Line"; DimSetID: Integer)
    var
        AutomaticAccHeader: Record "Automatic Acc. Header";
        AutomaticAccNo: Code[10];
    begin
        AutomaticAccNo := CreateAutomaticAccHeader(AutomaticAccHeader);
        CreateBalancedAutoAccLines(AutomaticAccLine, AutomaticAccNo, 100, DimSetID);
    end;
    ...
    begin
        ...
        GenJnlLine.Validate("Auto. Acc. Group", AutomaticAccGroupNo);
    ...
    var
        AutomaticAccLine: Record "Automatic Acc. Line";
    begin
        CreateAutomaticAccGroupWithDimensions(AutomaticAccLine, AutomaticAccDimSetID);
        GLAccountNo := AutomaticAccLine."G/L Account No.";
        exit(CreatePostGenJnlLineWithDimSetIDAndAccGroup(AutomaticAccLine."Automatic Acc. No.", GenJnlLineDimSetID));
    end;
    ...
    local procedure VerifyGlobalShortcutDimValuesInGenJnlLine(GenJnlLine: Record "Gen. Journal Line"; GlobalDimValue1: Code[20]; GlobalDimValue2: Code[20])
    begin
        Assert.AreEqual(
          GlobalDimValue1, GenJnlLine."Shortcut Dimension 1 Code",
          StrSubstNo(WrongDimValueErr, Format(GenJnlLine."Shortcut Dimension 1 Code")));
        Assert.AreEqual(
          GlobalDimValue2, GenJnlLine."Shortcut Dimension 2 Code",
          StrSubstNo(WrongDimValueErr, Format(GenJnlLine."Shortcut Dimension 2 Code")));
    end;
