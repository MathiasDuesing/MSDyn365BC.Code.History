codeunit 134590 "Mandatory Fields Tests"
{
    ...
    [Test]
    [Scope('OnPrem')]
    procedure VerifyMXFieldsAreVisible()
    var
        CustomerCard: TestPage "Customer Card";
    begin
        // [FEATURE] [Customer]
        // [SCENARIO 160798] Stan should be able to see MX fields

        // [GIVEN] Company information "Country/Region Code" is 'MX'
        LibraryApplicationArea.EnableFoundationSetup;

        DeleteAllTemplates;
        // [WHEN] Customer window opens up
        CustomerCard.OpenNew;

        // [THEN] Mexico fields should be visible
        Assert.IsTrue(CustomerCard."RFC No.".Visible, 'RFC No. should be visible.');
        Assert.IsTrue(CustomerCard."CURP No.".Visible, 'CURP No. should be visible.');
        Assert.IsTrue(CustomerCard."State Inscription".Visible, 'State Inscription should be visible.');
        CustomerCard.Close;
    end;
