codeunit 134767 "Test OData Wizard US"
{
    ...
    [Test]
    [Scope('OnPrem')]
    procedure TestExcelWorkBookGeneration()
    var
        TenantWebService: Record "Tenant Web Service";
        TenantWebServiceColumns: Record "Tenant Web Service Columns";
        ODataUtility: Codeunit ODataUtility;
    ...
    begin
        ...
        Assert.IsTrue(DataEntityExportInfo.RefreshOnOpen, 'Unexpected value for DataEntityExportInfo.RefreshOnOpen');

        // [THEN] The correct Office App Id is set.
        OfficeAppInfo := DataEntityExportInfo.AppReference;
        Assert.AreEqual('WA104379629', OfficeAppInfo.Id, 'Unexpected value for OfficeAppInfo.Id');

        // [THEN] The correct Tenant is set.
        AuthenticationOverrides := DataEntityExportInfo.Authentication;
        Assert.AreEqual(AzureADTenant.GetAadTenantId, AuthenticationOverrides.Tenant, 'Unexpected value for Authentication.Tenant');

        // [THEN] The hostname is a substring of the full URL (e.g. it doesn't contain the query strings from GETURL) if OnPrem else point to the service.
        ConnectionInfo := DataEntityExportInfo.Connection;
        ...
        EntityFilterCollectionNode := DataEntityInfo.Filter;
        Assert.AreEqual(ExpectedFilterTok, EntityFilterCollectionNode.ToString, 'Unexpected EntityFilterCollectionNode');
    end;

    [Test]
    [Scope('OnPrem')]
    procedure TestServiceNameTransformation()
    var
        TenantWebService: Record "Tenant Web Service";
        TenantWebServiceColumns: Record "Tenant Web Service Columns";
        ODataUtility: Codeunit ODataUtility;
    ...
    begin
        ...
        Assert.AreEqual(ExpectedServiceName, DataEntityInfo.Name, 'Unexpected value in DataEntityInfo.Name');
        Assert.AreEqual(ExpectedServiceName, DataEntityInfo.PublicName, 'Unexpected value in DataEntityInfo.PublicName');
    end;
