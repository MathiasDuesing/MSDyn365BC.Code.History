codeunit 134130 "ERM Reverse And VAT Settlement"
{
    ...
    begin
        // [FEATURE] [Report] [Sales]
        // [SCENARIO 404686] Run Calc. and Post VAT Settlement report with Post option set on three posted Sales Invoices with different VAT Posting Setup of "Reverse Charge VAT" calculation type.
        Initialize();
    ...
    begin
        // [FEATURE] [Report] [Sales]
        // [SCENARIO 404686] Run Calc. and Post VAT Settlement report with Post option not set on three posted Sales Invoices with different VAT Posting Setup of "Reverse Charge VAT" calculation type.
        Initialize();
    ...
    begin
        ...
        // [THEN] VAT Entries 901, 902, 903 were closed. Closed by Entry No. for each of these VAT Entries was set to 904, 905, 906 respectively.
        // [THEN] "Entry No." for Settlement VAT Entries in report results are 904, 905, 906.
        FindVATEntries(VATEntry, VATPostingSetup, PostedDocNo, VATEntry[1].Type::Purchase);
        GetSettlementVATEntriesNo(SettlementVATEntryNo, VATPostingSetup, VATSettlementDocNo);
        VerifyVATEntryClosedByEntryNo(VATEntry, SettlementVATEntryNo);
        VerifyVATEntryNoInVATSettlementReportResults(VATEntry, SettlementVATEntryNo);
    ...
    begin
        ...
        GenJournalLine.Validate("Use Tax", UseTax);
        GenJournalLine.Modify(true);
    ...
    local procedure RunCalcAndPostVATSettlementReport(var DocumentNo: Code[20]; VATPostingSetup: array[3] of Record "VAT Posting Setup"; PostSettlement: Boolean)
    var
    ...
    local procedure FilterVATEntryByDocumentNoPostingGroups(var VATEntry: Record "VAT Entry"; VATBusPostingGroupCode: Code[20]; VATProdPostingGroupCode: Code[20]; DocumentNo: Code[20]; VATEntryType: Option)
    begin
    ...
    local procedure FindVATEntries(var VATEntry: array[3] of Record "VAT Entry"; VATPostingSetup: array[3] of Record "VAT Posting Setup"; DocumentNo: array[3] of Code[20]; VATEntryType: Option)
    var
        i: Integer;
    begin
        for i := 1 to ArrayLen(VATEntry) do begin
            FilterVATEntryByDocumentNoPostingGroups(VATEntry[i], VATPostingSetup[i]."VAT Bus. Posting Group", VATPostingSetup[i]."VAT Prod. Posting Group", DocumentNo[i], VATEntryType);
    ...
    begin
        for i := 1 to ArrayLen(SettlementVATEntryNo) do begin
            FilterVATEntryByDocumentNoPostingGroups(VATEntry, VATPostingSetup[i]."VAT Bus. Posting Group", VATPostingSetup[i]."VAT Prod. Posting Group", DocumentNo, VATEntry.Type::Settlement);
            VATEntry.FindFirst();
