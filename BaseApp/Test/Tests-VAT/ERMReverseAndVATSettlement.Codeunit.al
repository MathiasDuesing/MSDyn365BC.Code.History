codeunit 134130 "ERM Reverse And VAT Settlement"
{
    ...
    local procedure RunCalcAndPostVATSettlementReportWithoutRequestPage(var DocumentNo: Code[20]; VATPostingSetup: array[3] of Record "VAT Posting Setup"; PostSettlement: Boolean)
    var
        FilterVATPostingSetup: Record "VAT Posting Setup";
        CalcandPostVATSettlement: Report "Calc. and Post VAT Settlement";
        TemplateName: Code[10];
    ...
    begin
        if MakeInconsistent then
            if GenJnlLine."VAT Base Amount" > 0 then
                GenJnlLine.Amount := Round(GenJnlLine."VAT Base Amount" * 1.1);
    end;
}
