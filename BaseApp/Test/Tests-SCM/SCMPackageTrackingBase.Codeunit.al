codeunit 137263 "SCM Tracking Package Base"
{
    ...
    local procedure CreatePostPurchInvoice(VendNo: Code[20]; LocationCode: Code[10]; CurrencyCode: Code[10]; ItemNo: Code[20]; PackageNo: Code[50])
    var
    ...
    begin
        ...
        CreatePurchLine(Purchheader, ItemNo, PackageNo);
        LibraryPurchase.PostPurchaseDocument(Purchheader, true, true);
    end;

    local procedure CreatePurchLine(var PurchHeader: Record "Purchase Header"; ItemNo: Code[20]; PackageNo: Code[50])
    var
    ...
    begin
        ...
        LibraryItemTracking.CreatePurchOrderItemTracking(ReservationEntry, PurchLine, '', '', PackageNo, Qty)
    end;

    local procedure CreateReleaseSalesInvoice(CustNo: Code[20]; LocationCode: Code[10]; CurrencyCode: Code[10]; ItemNo: Code[20]; PackageNo: Code[50]): Code[20]
    var
        SalesHeader: Record "Sales Header";
    ...
    begin
        ...
        CreateSalesLine(SalesHeader, ItemNo, PackageNo);
    ...
    local procedure CreateSalesLine(var SalesHeader: Record "Sales Header"; ItemNo: Code[20]; PackageNo: Code[50])
    var
        SalesLine: Record "Sales Line";
        ReservationEntry: Record "Reservation Entry";
        Qty: Integer;
    begin
        ...
        LibraryItemTracking.CreateSalesOrderItemTracking(ReservationEntry, SalesLine, '', '', PackageNo, Qty);
    end;
    ...
    local procedure CreateItemTrackingCode(var ItemTrackingCode: Record "Item Tracking Code"): Code[10]
    begin
        LibraryItemTracking.CreateItemTrackingCode(ItemTrackingCode, false, false, true);
    ...
    local procedure CreatePackageNo(ItemNo: Code[20]; VendorNo: Code[20]): Code[50]
    var
        PackageNoInfo: Record "Package No. Information";
    begin
