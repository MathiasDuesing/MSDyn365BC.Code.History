codeunit 816 "Purch. Post Invoice" implements "Invoice Posting"
{
    ...
    begin
        ...
        LineDiscountPosting := PurchSetup."Post Line Discount";
        ...
        if PurchSetup."Post Payment Discount" then begin
            CalcPaymentDiscountPosting(PurchHeader, PurchLine, PurchLineACY, InvoicePostingBuffer);
    ...
    local procedure CalcPaymentDiscountPosting(PurchHeader: Record "Purchase Header"; PurchLine: Record "Purchase Line"; PurchLineACY: Record "Purchase Line"; var InvoicePostingBuffer: Record "Invoice Posting Buffer")
    begin
        case PurchLine."VAT Calculation Type" of
            PurchLine."VAT Calculation Type"::"Normal VAT", PurchLine."VAT Calculation Type"::"Full VAT",
          PurchLine."VAT Calculation Type"::"No Taxable VAT":
            ...
            PurchLine."VAT Calculation Type"::"Reverse Charge VAT":
                InvoicePostingBuffer.CalcDiscountNoVAT(-PurchLine."Pmt. Discount Amount", -PurchLineACY."Pmt. Discount Amount");
            PurchLine."VAT Calculation Type"::"Sales Tax":
    ...
    begin
        with GenJnlLine do begin
            ...
            if (PurchHeader."Generate Autoinvoices" or PurchHeader."Generate Autocredit Memo") and
               (InvoicePostingBuffer."VAT Calculation Type" = InvoicePostingBuffer."VAT Calculation Type"::"Reverse Charge VAT")
            then begin
                "Generate AutoInvoices" := true;
                "AutoDoc. No." := InvoicePostingParameters."Auto Document No.";
            end else
                ...
                case InvoicePostingBuffer."VAT Calculation Type" of
                    InvoicePostingBuffer."VAT Calculation Type"::"Reverse Charge VAT":
                        begin
                            ...
                            VATBaseAmount := InvoicePostingBuffer."VAT Base Amount";
    ...
    begin
        ...
        then begin
            if TempDeferralHeader."Amount to Defer" <> 0 then begin
                ...
                OnPrepareDeferralLineOnBeforeDeferralPostingBufferPrepareInitialAmounts(
