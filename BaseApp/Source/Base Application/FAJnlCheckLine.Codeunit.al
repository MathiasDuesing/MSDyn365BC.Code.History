codeunit 5631 "FA Jnl.-Check Line"
{
    ...
    begin
        if GenJnlPosting then
            with GenJnlLine do begin
                ...
                if GLSetup."Enable Russian Accounting" and ("FA Posting Type" = "FA Posting Type"::Disposal) and (not "System-Created Entry") then
                    TestField(Amount, 0);
            end;
    ...
    [Scope('OnPrem')]
    procedure CheckInitialAcquisition()
    begin
        ...
        if GenJnlPosting then
            with GenJnlLine do begin
                if "Prepmt. Diff. Appln. Entry No." <> 0 then
                    exit;
                if "VAT Allocation Type" = "VAT Allocation Type"::Charge then
                    exit;
    ...
    [Scope('OnPrem')]
    procedure CheckPrevDepreciation()
    var
        DepreciationCalc: Codeunit "Depreciation Calculation";
    begin
        FADeprBook.CalcFields("Book Value");
        if (FA.Status = FA.Status::Operation) and (not FA."Undepreciable FA") and (FADeprBook."Book Value" <> 0) and
        ...
        then
            if FADeprBook."Depreciation Starting Date" < PostingDate then
                if DepreciationCalc.ExistingDeprPeriods(FANo, DeprBookCode, PostingDate) = 0 then begin
                    if FAPostingType = FAPostingType::Disposal then begin
                        if (CalcDate('<CM>', FADeprBook."Depreciation Starting Date") < PostingDate) then
                            Error(Text12400)
                    end else
