codeunit 5605 "Calculate Disposal"
{
                ...
                repeat
                    ...
                    if "Disposal Entry No." > MaxDisposalNo then begin
                        ...
                        if "Document Type" in ["Document Type"::Invoice, "Document Type"::"Credit Memo"] then begin
                            DisposalAmount := DisposalAmount + Amount;
                            SalesDisposal := true;
                        end;
                    end;
                until Next() = 0;
    ...
    begin
        ...
        with FADeprBook do begin
            ...
            if DeprBook."G/L Integration - Disposal" and DeprBook."G/L Integration - Acq. Cost" then
                GainLoss := "Book Value";
        end;
        EntryAmounts[2] := GainLoss;
        EntryAmounts[3] := -GainLoss;
    end;
    ...
    begin
        ...
        with FALedgEntry do begin
            ...
            TransactionNo := FALedgEntry."Transaction No.";
            DepreciationCalc.SetFAFilter(FALedgEntry, ErrFALedgEntry."FA No.", ErrFALedgEntry."Depreciation Book Code", true);
            for i := 2 to 14 do begin
                SetRange("FA Posting Category", SetFAPostingCategory(i));
                SetRange("FA Posting Type", SetFAPostingType(i));
                if Find('-') then
                    repeat
                        if "Transaction No." = TransactionNo then begin
                            EntryNumbers[i] := "Entry No.";
                            EntryAmounts[i] := -Amount;
                        end;
                    until Next() = 0;
