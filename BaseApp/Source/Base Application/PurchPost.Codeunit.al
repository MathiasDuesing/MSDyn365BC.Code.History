codeunit 90 "Purch.-Post"
{
    ...
    begin
        ...
        if TempPurchLineGlobal.FindSet() then
            repeat
                ...
                PostPurchLine(
                  PurchHeader, TempPurchLineGlobal, TempInvoicePostBuffer,
                  TempVATAmountLine, TempVATAmountLineRemainder, TempDropShptPostBuffer, EverythingInvoiced, ICGenJnlLineNo);
        ...
        OnAfterPostPurchLines(
          ...
          SuppressCommit, EverythingInvoiced, TempInvoicePostBuffer);
        ...
        OnRunOnBeforeMakeInventoryAdjustment(PurchHeader, GenJnlPostLine);
    ...
    begin
        with PurchHeader do begin
            ...
            CheckCashDeskUserRights(PurchHeader);
            CheckDeferralSchedule(PurchHeader);
            // NAVCZ
            ...
            if Receive then begin
                ...
                CheckItemChargeForReceive(PurchHeader); // NAVCZ
            ...
            if Ship then begin
                ...
                CheckItemChargeForShip(PurchHeader); // NAVCZ
                if not InvtPickPutaway then
            ...
            CheckIntrastatTransactionSetup(PurchHeader); // NAVCZ

            if Invoice and PurchSetup."Ext. Doc. No. Mandatory" then
                CheckExtDocNo(PurchHeader);

            CheckVATDate(PurchHeader); // NAVCZ
            ...
            if "Has Letter Line Relation" then begin
                CheckAllQtyToInvoice(PurchHeader);
            ...
            if "Posting Desc. Code" <> '' then begin
                ...
                if PostingDesc."Validate on Posting" then
                    "Posting Description" := GetPostingDescription(PurchHeader);
            end;
            // NAVCZ

            DropShipOrder := UpdateAssosOrderPostingNos(PurchHeader);

            CheckAcceptabilityDocNo(PurchHeader); // NAVCZ
    ...
    begin
        with PurchLine do begin
            ...
            // NAVCZ
            if ("Qty. to Receive" <> 0) or ("Return Qty. to Ship" <> 0) then
            ...
            case Type of
                ...
                Type::"Charge (Item)":
                    ...
                    OnPostPurchLineOnTypeCaseElse(PurchHeader, PurchLine, PurchInvHeader, PurchCrMemoHeader, SrcCode);
            end;
            ...
            if not IsHandled then
                ...
                then
                    InsertReceiptLine(PurchHeader, PurchRcptHeader, PurchLine, CostBaseAmount);

            IsHandled := false;
            OnPostPurchLineOnBeforeInsertReturnShipmentLine(PurchHeader, PurchLine, IsHandled, ReturnShptHeader, TempPurchLineGlobal, RoundingLineInserted);
            if not IsHandled then
                if (ReturnShptHeader."No." <> '') and ("Return Shipment No." = '') and
                   not RoundingLineInserted
                then
                    InsertReturnShipmentLine(PurchHeader, ReturnShptHeader, PurchLine, CostBaseAmount);
            ...
            if PurchHeader.Invoice then
                if "Document Type" in ["Document Type"::Order, "Document Type"::Invoice] then begin
                    ...
                    OnPostPurchLineOnBeforeInsertCrMemoLine(PurchHeader, PurchLine, IsHandled);
                    if not IsHandled then begin
    ...
    begin
        ...
        with PurchHeader do begin
            ...
            // NAVCZ
            if (PurchSetup."Ext. Doc. No. Mandatory" or (GenJnlLineExtDocNo <> '')) and not ExtDocNoConfirmed then begin
                CheckExternalDocumentNumber(VendLedgEntry, PurchHeader);
                CheckExternalDocumentNumberInPostedDocuments(PurchHeader);
            end;
            // NAVCZ
            ...
            if IsAdvanceRelated then begin
                ...
                PurchPostAdvances.PostInvoiceCorrection(
                  PurchHeader, PurchInvHeader, TempPurchaseLine, GenJnlPostLine, InvoicedAdvAmount);
                TempPurchaseLine.Reset();
            end;
            // NAVCZ

            UpdatePurchaseHeader(VendLedgEntry);
            OnPostGLAndVendorOnAfterUpdatePurchaseHeader(PreviewMode, GenJnlLineDocNo);

            // Balancing account
            if (TotalPurchLine."Amount Including VAT" + VendLedgEntry."Remaining Pmt. Disc. Possible") <> 0 then // NAVCZ
    ...
    begin
        OnBeforePostItemLine(PurchHeader, PurchLine, PurchRcptHeader, RemQtyToBeInvoiced);
    ...
    begin
        IsHandled := false;
        OnBeforeProcessAssocItemJnlLine(PurchLine, IsHandled);
        if IsHandled then
            exit;
            ...
            repeat
                ...
                case TempItemChargeAssgntPurch."Applies-to Doc. Type" of
                  ...
                  TempItemChargeAssgntPurch."Applies-to Doc. Type"::"Credit Memo":
                    ...
                    15:
                        begin
                            PostItemChargePerPosAdjItem(PurchHeader, PurchLine);
                            TempItemChargeAssgntPurch.Mark(true);
                        end;
                // NAVCZ
                end;
    ...
    begin
        ...
        OnBeforePostItemJnlLine(
          ...
          ItemLedgShptEntryNo, ItemChargeNo, TrackingSpecification, SuppressCommit, IsHandled);
        if IsHandled then
            exit;
        ...
        with ItemJnlLine do begin
            ...
            // NAVCZ
            "G/L Correction" := PurchHeader.Correction xor PurchLine.Negative;
            if PurchHeader."Perform. Country/Region Code" <> '' then begin
                if "Country/Region Code" = '' then begin
                    CompanyInfo.Get();
                    "Country/Region Code" := CompanyInfo."Country/Region Code";
                end;
                "Perform. Country/Region Code" := PurchHeader."Perform. Country/Region Code";
            end;
            // NAVCZ
            ...
            if ItemChargeNo <> '' then begin
                ...
                // NAVCZ
                "Incl. in Intrastat Amount" := TempItemChargeAssgntPurch."Incl. in Intrastat Amount";
                "Incl. in Intrastat Stat. Value" := TempItemChargeAssgntPurch."Incl. in Intrastat Stat. Value";
                // NAVCZ
            ...
            if QtyToBeInvoiced <> 0 then begin
                ...
                OnPostItemJnlLineOnAfterSetFactor(PurchLine, Factor, GenJnlLineExtDocNo);
                // NAVCZ
                if PurchLine."VAT Calculation Type" = PurchLine."VAT Calculation Type"::"Reverse Charge VAT" then
                    PurchLine."VAT Amount (Non Deductible)" :=
                      CalcVATAmount(
                        PurchLine."VAT Bus. Posting Group", PurchLine."VAT Prod. Posting Group", PurchLine."VAT Base (Non Deductible)");
                Amount := (PurchLine.Amount + PurchLine."VAT Amount (Non Deductible)") * Factor + RemAmt;
                // NAVCZ
            ...
            "Invoice-to Source No." := PurchHeader."Pay-to Vendor No.";
            // NAVCZ
            "Source No. 2" := PurchHeader."Pay-to Vendor No.";
            "Source No. 3" := PurchHeader."Order Address Code";
            // NAVCZ
            "Source Code" := SrcCode;
            "Purchasing Code" := PurchLine."Purchasing Code";

            OnPostItemJnlLineOnAfterPrepareItemJnlLine(ItemJnlLine, PurchLine, PurchHeader, PreviewMode, GenJnlLineDocNo, TrackingSpecification);
            ...
            OnPostItemJnlLineOnBeforeItemJnlPostLineRunWithCheck(ItemJnlLine, PurchLine, DropShipOrder);
    ...
    begin
        IsHandled := false;
        OnBeforeClearRemAmt(PurchHeader, IsHandled);
        if IsHandled then
            exit;
    ...
    begin
        IsHandled := false;
        OnBeforeShouldPostWhseJnlLine(PurchLine, Result, IsHandled, ItemJnlLine, TempWhseJnlLine);
        if IsHandled then
            exit(Result);
    ...
    begin
        ...
        with TempItemChargeAssgntPurch do begin
            ...
            // NAVCZ
            if ItemChargePurchLine."VAT Calculation Type" = ItemChargePurchLine."VAT Calculation Type"::"Reverse Charge VAT" then
                ItemChargePurchLine."VAT Amount (Non Deductible)" :=
                  CalcVATAmount(
                    ItemChargePurchLine."VAT Bus. Posting Group",
                    ItemChargePurchLine."VAT Prod. Posting Group",
                    ItemChargePurchLine."VAT Base (Non Deductible)");
            ItemJnlLine2.Amount :=
              ("Amount to Assign" + ItemChargePurchLine."VAT Amount (Non Deductible)" /
               ItemChargePurchLine.Quantity * "Qty. to Assign") *
    ...
    begin
        IsHandled := false;
        OnBeforePostItemTrackingItemChargePerOrder(TempTrackingSpecificationInv, IsHandled);
        if IsHandled then
            exit;
    ...
    begin
        ...
        OnPostItemChargePerRcptOnAfterCalcDistributeCharge(PurchHeader, PurchLine, PurchRcptLine);
    ...
    begin
        ...
        with ItemJnlLine do begin
            ...
            // NAVCZ
            "Intrastat Transaction" := SalesOrderHeader.IsIntrastatTransaction();
            // recalc to base UOM
            if "Net Weight" <> 0 then
                if SalesOrderLine."Qty. per Unit of Measure" <> 0 then
                    "Net Weight" := Round("Net Weight" / SalesOrderLine."Qty. per Unit of Measure", 0.00001);
            if SalesOrderHeader."Perform. Country/Region Code" <> '' then begin
                if "Country/Region Code" = '' then begin
                    CompanyInformation.Get();
                    "Country/Region Code" := CompanyInformation."Country/Region Code";
                end;
                "Perform. Country/Region Code" := SalesOrderHeader."Perform. Country/Region Code";
            end;
            // NAVCZ
        end;
    ...
    begin
        with PurchHeader do begin
            ...
            if LinesWereModified then
                RefreshTempLines(PurchHeader, TempPurchLineGlobal);
            // NAVCZ TestStatusRelease(PurchHeader);
            Status := PrevStatus;
            RestorePurchaseHeader(PurchHeader, PurchaseHeaderCopy);
    ...
    begin
        ...
        with PurchLine do begin
            case Type of
                ...
                Type::"Charge (Item)":
                    TestPurchLineItemCharge(PurchLine, PurchHeader);
                Type::"Fixed Asset":
    ...
    local procedure TestPurchLineItemCharge(PurchaseLine: Record "Purchase Line"; PurchaseHeader: Record "Purchase Header")
    var
        IsHandled: Boolean;
    begin
        IsHandled := false;
    ...
    begin
        IsHandled := false;
        OnBeforeUpdateAssocOrder(TempDropShptPostBuffer, IsHandled);
        if IsHandled then
            exit;
                    ...
                    repeat
                        if SalesOrderHeader."No." <> TempPurchLine."Sales Order No." then begin
                            ...
                            if SalesOrderHeader."Shipping No." <> '' then
                                NoSeriesMgt.CheckAcceptabilityDocNo(SalesOrderHeader."Shipping No.",
                                  SalesOrderHeader."Shipping No. Series", "Posting Date");
                            // NAVCZ
                        end;
    ...
    begin
        ...
        with PurchHeader do begin
            ...
            then begin
                ...
                OnFinalizePostingOnAfterUpdateItemChargeAssgnt(PurchHeader);
            end else begin
                OnFinalizePostingOnBeforeInsertTrackingSpecification(TempDropShptPostBuffer, PurchHeader, TempTrackingSpecification, EverythingInvoiced, TempPurchLineGlobal);
                case "Document Type" of
                    "Document Type"::Invoice:
                        begin
            ...
            OnFinalizePostingOnBeforeInsertValueEntryRelation(PurchHeader);
    ...
    local procedure DeleteApprovalEntries(var PurchHeader: Record "Purchase Header")
    var
        IsHandled: Boolean;
    begin
        IsHandled := false;
        OnBeforeDeleteApprovalEntries(PurchHeader, IsHandled);
        if IsHandled then
            exit;

        ApprovalsMgmt.DeleteApprovalEntries(PurchHeader.RecordId());
    end;
    ...
    var
        ...
        TotalNonDeductBase: Decimal;
        TotalNonDeductVAT: Decimal;
        AmtToDefer: Decimal;
        AmtToDeferACY: Decimal;
        TotalVATBase: Decimal;
        TotalVATBaseACY: Decimal;
        AmtToDeferWithoutNonDeductVAT: Decimal;
        AmtToDeferACYWithoutNonDeductVAT: Decimal;
    ...
    begin
        ...
        OnFillInvoicePostBufferOnBeforePreparePurchase(PurchHeader, PurchLine);
        ...
        // NAVCZ
        TotalNonDeductBase := PurchLine."VAT Base (Non Deductible)";
        TotalNonDeductVAT := PurchLine."VAT Amount (Non Deductible)";
        // NAVCZ
                ...
                if (InvoicePostBuffer.Amount <> 0) or (InvoicePostBuffer."Amount (ACY)" <> 0) then begin
                    ...
                    if InvoicePostBuffer.Type = InvoicePostBuffer.Type::"Fixed Asset" then begin
                        FillInvoicePostBufferFADiscount(
                          ...
                          TotalNonDeductBase, TotalNonDeductVAT, PurchLine);
                        ...
                        InvoicePostBuffer.UpdateNonDeductable(TotalNonDeductBase, TotalNonDeductVAT); // NAVCZ
                        UpdateInvoicePostBuffer(TempInvoicePostBuffer, InvoicePostBuffer, PurchLine);
            ...
            if (InvoicePostBuffer.Amount <> 0) or (InvoicePostBuffer."Amount (ACY)" <> 0) then begin
                ...
                if InvoicePostBuffer.Type = InvoicePostBuffer.Type::"Fixed Asset" then begin
                    ...
                    InvoicePostBuffer.SetAccount(
                      GenPostingSetup.GetPurchLineDiscAccount, TotalVAT, TotalVATACY, TotalAmount, TotalAmountACY);
                    InvoicePostBuffer.UpdateVATBase(TotalVATBase, TotalVATBaseACY);
                    TempInvoicePostBuffer.UpdateNonDeductable(TotalNonDeductBase, TotalNonDeductVAT); // NAVCZ
                    UpdateInvoicePostBuffer(TempInvoicePostBuffer, InvoicePostBuffer, PurchLine);
                end;
        ...
        // NAVCZ
        AmtToDeferWithoutNonDeductVAT := AmtToDefer - TotalNonDeductVAT;
        AmtToDeferACYWithoutNonDeductVAT := AmtToDeferACY - TotalNonDeductVAT;

        DeferralUtilities.AdjustTotalAmountForDeferralsNoBase(
          PurchLine."Deferral Code", AmtToDeferWithoutNonDeductVAT, AmtToDeferACYWithoutNonDeductVAT, TotalAmount, TotalAmountACY);

        if (AmtToDeferWithoutNonDeductVAT = 0) or (AmtToDeferACYWithoutNonDeductVAT = 0) then begin
            AmtToDefer := AmtToDeferWithoutNonDeductVAT;
            AmtToDeferACY := AmtToDeferACYWithoutNonDeductVAT;
        end;
        // NAVCZ
        ...
        InvoicePostBuffer.UpdateVATBase(TotalVATBase, TotalVATBaseACY);
        // NAVCZ
        InvoicePostBuffer.SetNonDeductable(TotalNonDeductBase, TotalNonDeductVAT);
        InvoicePostBuffer.SetExtAmounts(PurchLine."Ext. Amount (LCY)",
          PurchLine."Ext.Amount Including VAT (LCY)", PurchLine."Ext. VAT Difference (LCY)");
        InvoicePostBuffer.SetVATDifferenceLCY(PurchLine."VAT Difference (LCY)");
        // NAVCZ
    ...
    local procedure FillInvoicePostBufferFADiscount(var TempInvoicePostBuffer: Record "Invoice Post. Buffer" temporary; var InvoicePostBuffer: Record "Invoice Post. Buffer"; GenPostingSetup: Record "General Posting Setup"; AccountNo: Code[20]; TotalVAT: Decimal; TotalVATACY: Decimal; TotalAmount: Decimal; TotalAmountACY: Decimal; TotalVATBase: Decimal; TotalVATBaseACY: Decimal; var TotalNonDeductBase: Decimal; var TotalNonDeductVAT: Decimal; PurchLine: Record "Purchase Line")
    var
        DeprBook: Record "Depreciation Book";
        IsHandled: Boolean;
    begin
    ...
    local procedure UpdateInvoicePostBuffer(var TempInvoicePostBuffer: Record "Invoice Post. Buffer" temporary; InvoicePostBuffer: Record "Invoice Post. Buffer"; PurchLine: Record "Purchase Line")
    begin
        if InvoicePostBuffer.Type = InvoicePostBuffer.Type::"Fixed Asset" then begin
            FALineNo := FALineNo + 1;
            InvoicePostBuffer."Fixed Asset Line No." := FALineNo;
            // NAVCZ
            if PurchSetup."G/L Entry as Doc. Lines (FA)" then
        ...
        // NAVCZ
        case true of
            (PurchLine.Type = PurchLine.Type::"G/L Account") and PurchSetup."G/L Entry as Doc. Lines (Acc.)":
                begin
                    InvoicePostBuffer."Fixed Asset Line No." := PurchLine."Line No.";
                    InvoicePostBuffer."Entry Description" := PurchLine.Description;
                end;
            (PurchLine.Type = PurchLine.Type::Item) and PurchSetup."G/L Entry as Doc. Lines (Item)":
                begin
                    InvoicePostBuffer."Fixed Asset Line No." := PurchLine."Line No.";
                    InvoicePostBuffer."Entry Description" := PurchLine.Description;
                end;
            (PurchLine.Type = PurchLine.Type::"Charge (Item)") and PurchSetup."G/L Entry as Doc. Lines (Char)":
                begin
                    InvoicePostBuffer."Fixed Asset Line No." := PurchLine."Line No.";
                    InvoicePostBuffer."Entry Description" := PurchLine.Description;
                end;
        ...
        with PurchLine do
            if (PurchLineQty = 0) or ("Direct Unit Cost" = 0) then begin
                ...
                "VAT Amount (Non Deductible)" :=
                  Round(("Amount Including VAT" - "VAT Base Amount") * "VAT % (Non Deductible)" / 100, Currency."Amount Rounding Precision");
                // NAVCZ
    ...
    begin
        IsHandled := false;
        OnBeforeCalcLineAmountAndLineDiscountAmount(PurchHeader, PurchLine, PurchLineQty, IsHandled);
        if IsHandled then
            exit;
    ...
    begin
        ...
        with PurchLine do begin
            ...
            if PurchHeader."Currency Code" <> '' then begin
                ...
                // NAVCZ
                if PurchHeader."VAT Date" = 0D then
                    UseVATdate := Usedate
                else
                    UseVATdate := PurchHeader."VAT Date";
                // NAVCZ
                ...
                "Ext.Amount Including VAT (LCY)" :=
                  Round(
                    CurrExchRate.ExchangeAmtFCYToLCY(
                      UseVATdate, PurchHeader."Currency Code",
                      TotalPurchLine."Amount Including VAT", PurchHeader."VAT Currency Factor")) -
                  TotalPurchLineLCY."Ext.Amount Including VAT (LCY)";
                if NoVAT then begin
                    ...
                    "Ext. Amount (LCY)" :=
                      Round(
                        CurrExchRate.ExchangeAmtFCYToLCY(
                          UseVATdate, PurchHeader."Currency Code",
                          TotalPurchLine.Amount, PurchHeader."VAT Currency Factor")) -
                      TotalPurchLineLCY."Ext. Amount (LCY)";
                end;
                // NAVCZ
                "Line Amount" :=
                  Round(
                ...
                "VAT Base (Non Deductible)" :=
                  Round(
                    CurrExchRate.ExchangeAmtFCYToLCY(
                      Usedate, PurchHeader."Currency Code",
                      TotalPurchLine."VAT Base (Non Deductible)", PurchHeader."Currency Factor")) -
                  TotalPurchLineLCY."VAT Base (Non Deductible)";
                "VAT Amount (Non Deductible)" :=
                  Round(
                    CurrExchRate.ExchangeAmtFCYToLCY(
                      Usedate, PurchHeader."Currency Code",
                      TotalPurchLine."VAT Amount (Non Deductible)", PurchHeader."Currency Factor")) -
                  TotalPurchLineLCY."VAT Amount (Non Deductible)";
            end;
    ...
    begin
        with PurchLine do begin
            ...
            // NAVCZ
            "Ext. Amount (LCY)" := -"Ext. Amount (LCY)";
            "Ext.Amount Including VAT (LCY)" := -"Ext.Amount Including VAT (LCY)";
            "Ext. VAT Difference (LCY)" := -"Ext. VAT Difference (LCY)";
            // NAVCZ
    ...
    begin
        with PurchLine do begin
            ...
            if Type in [Type::"G/L Account", Type::Item, Type::"Fixed Asset"] then begin
                Increment(TotalPurchLine."VAT Base (Non Deductible)", "VAT Base (Non Deductible)");
                Increment(TotalPurchLine."VAT Amount (Non Deductible)", "VAT Amount (Non Deductible)");
            end;
            Increment(TotalPurchLine."VAT Difference (LCY)", "VAT Difference (LCY)");
            // NAVCZ
            ...
            // NAVCZ
            Increment(TotalPurchLine."Prepmt. VAT Amount Inv.", "Prepmt. VAT Amount Inv.");
            Increment(TotalPurchLine."Ext. VAT Difference (LCY)", "Ext. VAT Difference (LCY)");
            Increment(TotalPurchLine."Ext. Amount (LCY)", "Ext. Amount (LCY)");
            Increment(TotalPurchLine."Ext.Amount Including VAT (LCY)", "Ext.Amount Including VAT (LCY)");
            // NAVCZ
    ...
    var
        ...
        BiggestLineNo: Integer;
    begin
        OnBeforeSumPurchLines2(QtyType, PurchHeader, OldPurchLine, TempVATAmountLine);
        TempVATAmountLineRemainder.DeleteAll();
    ...
    begin
        IsHandled := false;
        OnBeforeUpdatePurchaseHeader(VendorLedgerEntry, PurchInvHeader, PurchCrMemoHeader, GenJnlLineDocType.AsInteger(), IsHandled);
        if IsHandled then
            exit;
        ...
        OnAfterUpdatePurchaseHeader(VendorLedgerEntry, PurchInvHeader, PurchCrMemoHeader, GenJnlLineDocType.AsInteger());
    end;
    ...
    begin
        ...
        with GenJnlLine do begin
            // NAVCZ
            "Allow Application" := not PurchHeader.IsAdvanceRelated;
            "Bank Account Code" := PurchHeader."Bank Account Code";
            "Bank Account No." := PurchHeader."Bank Account No.";
            "Specific Symbol" := PurchHeader."Specific Symbol";
            if PurchHeader."Variable Symbol" <> '' then
                "Variable Symbol" := PurchHeader."Variable Symbol"
            else
                "Variable Symbol" := VariableSymbol;
            "Constant Symbol" := PurchHeader."Constant Symbol";
            "Transit No." := PurchHeader."Transit No.";
            IBAN := PurchHeader.IBAN;
            "SWIFT Code" := PurchHeader."SWIFT Code";
            // NAVCZ
            ...
            // NAVCZ
            if PurchHeader."Currency Code" <> '' then
                "Amount (LCY)" := "Amount (LCY)" - TotalPurchLineLCY2."VAT Difference (LCY)";
            // NAVCZ
    ...
    local procedure PostBalancingEntry(PurchHeader: Record "Purchase Header"; TotalPurchLine2: Record "Purchase Line"; TotalPurchLineLCY2: Record "Purchase Line"; DocType: Enum "Gen. Journal Document Type"; DocNo: Code[20]; ExtDocNo: Code[35]; SourceCode: Code[10]; InvoicedAdvAmount: Decimal)
    var
        GenJnlLine: Record "Gen. Journal Line";
        VendLedgEntry: Record "Vendor Ledger Entry";
    begin
        ...
        with GenJnlLine do begin
            ...
            if PurchHeader."Currency Code" <> '' then
                "Amount (LCY)" := "Amount (LCY)" + TotalPurchLineLCY2."VAT Difference (LCY)";
            "Amount (LCY)" := "Amount (LCY)" - Round(InvoicedAdvAmount / "Currency Factor");
            // NAVCZ
    ...
    local procedure UpdatePurchChargeAssgntLines(PurchOrderLine: Record "Purchase Line"; ApplToDocType: Enum "Purchase Applies-to Document Type"; ApplToDocNo: Code[20]; ApplToDocLineNo: Integer; QtytoAssign: Decimal)
    var
        ItemChargeAssgntPurch: Record "Item Charge Assignment (Purch)";
        ...
        if TempReservEntryPurchase.FindSet() then
            repeat
                ...
                with TempReservEntryJobCons do begin
                    ...
                    UpdateJobConsumptionReservationApplToItemEntry(TempReservEntryJobCons, ItemJournalLine, TempTrackingSpecification, IsNonInventoriableItem);
    ...
    local procedure UpdateJobConsumptionReservationApplToItemEntry(var TempReservEntryJobCons: Record "Reservation Entry" temporary; var ItemJournalLine: Record "Item Journal Line"; var TempTrackingSpecification: Record "Tracking Specification" temporary; IsNonInventoriableItem: Boolean)
    var
        IsHandled: Boolean;
    begin
        IsHandled := false;
        OnBeforeUpdateJobConsumptionReservationApplToItemEntry(TempReservEntryJobCons, ItemJournalLine, IsNonInventoriableItem, IsHandled);
        if IsHandled then
            exit;
    ...
    begin
        // NAVCZ
        if InvPostingBuffer."VAT % (Non Deductible)" = 0 then
            exit;
        GenJnlLine."VAT % (Non Deductible)" := InvPostingBuffer."VAT % (Non Deductible)";
        GenJnlLine."VAT Base (Non Deductible)" := InvPostingBuffer."VAT Base (Non Deductible)";
        GenJnlLine."VAT Amount (Non Deductible)" := InvPostingBuffer."VAT Amount (Non Deductible)";
        GenJnlLine."VAT Base LCY (Non Deduct.)" := InvPostingBuffer."VAT Base (Non Deductible)";
        GenJnlLine."VAT Amount LCY (Non Deduct.)" := InvPostingBuffer."VAT Amount (Non Deductible)";
        if (GenJnlLine."VAT Base (Non Deductible)" = 0) or (GenJnlLine."VAT Amount (Non Deductible)" = 0) then
            GenJnlLine.Validate("VAT % (Non Deductible)");
    end;

    [Scope('OnPrem')]
    procedure DeleteAdvLetterRelation(PurchLine: Record "Purchase Line")
    var
        LineRelation: Record "Advance Letter Line Relation";
    begin
        // NAVCZ
        if not (PurchLine."Document Type" in [PurchLine."Document Type"::Order, PurchLine."Document Type"::Invoice]) then
            exit;

        LineRelation.Reset();
        LineRelation.SetRange(Type, LineRelation.Type::Purchase);
        LineRelation.SetRange("Document Type", PurchLine."Document Type");
        LineRelation.SetRange("Document No.", PurchLine."Document No.");
        LineRelation.DeleteAll();
    end;

    local procedure PostVATDelay(PurchHeader: Record "Purchase Header"; InvoicePostBuffer: Record "Invoice Post. Buffer"; Post: Boolean; PostingDate: Date; IsCorrection: Boolean; VATPostingSetup: Record "VAT Posting Setup"; VATDate: Date)
    var
        GenJnlLine: Record "Gen. Journal Line";
    begin
        // NAVCZ
        with GenJnlLine do begin
            ...
            "Account No." := VATPostingSetup."Purchase VAT Delay Account";
            ...
            "Source Code" := SourceCodeSetup."Purchase VAT Delay";
            ...
            if InvoicePostBuffer."VAT Calculation Type" = InvoicePostBuffer."VAT Calculation Type"::"Reverse Charge VAT" then
                "Bal. Account No." := VATPostingSetup."Purchase VAT Delay Account";

            InitVATDelayAmounts(PurchHeader, InvoicePostBuffer, Post, GenJnlLine);

            if PurchHeader."Perform. Country/Region Code" <> '' then begin
                "Perform. Country/Region Code" := PurchHeader."Perform. Country/Region Code";
                "Currency Code VAT" := PurchHeader."Currency Code";
                if Post then
                    "Currency Factor VAT" := PurchHeader."VAT Currency Factor"
                else
                    "Currency Factor VAT" := PurchHeader."Currency Factor";
            end;
    ...
    local procedure InitVATDelayAmounts(PurchHeader: Record "Purchase Header"; InvoicePostBuffer: Record "Invoice Post. Buffer"; Post: Boolean; var GenJnlLine: Record "Gen. Journal Line")
    var
        CurrFactor: Decimal;
    begin
        // NAVCZ
        with GenJnlLine do
            if Post then begin
                if InvoicePostBuffer."VAT Calculation Type" = InvoicePostBuffer."VAT Calculation Type"::"Normal VAT" then begin
                    ...
                    if PurchHeader."Prices Including VAT" then begin
                        Amount := Round(InvoicePostBuffer."Ext. Amount" - InvoicePostBuffer."Ext. VAT Difference (LCY)",
                            Currency."Amount Rounding Precision");
                        "VAT Amount" := Round(InvoicePostBuffer."Ext. Amount Including VAT" - InvoicePostBuffer."Ext. Amount" +
                            InvoicePostBuffer."Ext. VAT Difference (LCY)", Currency."Amount Rounding Precision");
                        "VAT Base Amount" := Amount;
                        "Source Currency Amount" := Round(InvoicePostBuffer."Amount (ACY)", Currency."Amount Rounding Precision");
                        ...
                        Amount := Round(InvoicePostBuffer."Ext. Amount", Currency."Amount Rounding Precision");
                        "VAT Amount" := Round(InvoicePostBuffer."Ext. Amount Including VAT" - InvoicePostBuffer."Ext. Amount" +
                            InvoicePostBuffer."Ext. VAT Difference (LCY)", Currency."Amount Rounding Precision");
                        "VAT Base Amount" := Amount;
                        "Source Currency Amount" := Round(InvoicePostBuffer."Amount (ACY)", Currency."Amount Rounding Precision");
                    ...
                    // Reverse Charge VAT
                    if PurchHeader."VAT Currency Factor" = 0 then
                        CurrFactor := 1
                    else
                        CurrFactor := PurchHeader."Currency Factor" / PurchHeader."VAT Currency Factor";
                    if CurrFactor = 0 then
                        CurrFactor := 1;

                    Amount := Round(InvoicePostBuffer.Amount * CurrFactor, Currency."Amount Rounding Precision");
                    "VAT Amount" := Round(InvoicePostBuffer."VAT Amount" *
                        CurrFactor, Currency."Amount Rounding Precision");
                    "VAT Base Amount" := Amount;
                    "Source Currency Amount" := Round(InvoicePostBuffer."Amount (ACY)" * CurrFactor,
                        Currency."Amount Rounding Precision");
                    "Source Curr. VAT Amount" := Round(InvoicePostBuffer."VAT Amount (ACY)" * CurrFactor,
                        Currency."Amount Rounding Precision");
                    "Source Curr. VAT Base Amount" := "Source Currency Amount" - "Source Curr. VAT Amount";
                    "VAT Difference" := Round(InvoicePostBuffer."VAT Difference" * CurrFactor,
                        Currency."Amount Rounding Precision");
                end;

                // Post VAT Non Deductible
                if InvoicePostBuffer."VAT % (Non Deductible)" <> 0 then begin
                    "VAT % (Non Deductible)" := InvoicePostBuffer."VAT % (Non Deductible)";
                    "VAT Base (Non Deductible)" :=
                      Round("VAT Base Amount" * InvoicePostBuffer."VAT % (Non Deductible)" / 100,
                        Currency."Amount Rounding Precision");
                    "VAT Amount (Non Deductible)" :=
                      Round("VAT Amount" * InvoicePostBuffer."VAT % (Non Deductible)" / 100,
                        Currency."Amount Rounding Precision");
                    "VAT Base LCY (Non Deduct.)" := "VAT Base (Non Deductible)";
                    "VAT Amount LCY (Non Deduct.)" := "VAT Amount (Non Deductible)";
                    if ("VAT Base (Non Deductible)" = 0) or ("VAT Amount (Non Deductible)" = 0) then
                        Validate("VAT % (Non Deductible)");
                end;
            end else begin
                Quantity := -InvoicePostBuffer.Quantity;
                Amount := -Round(InvoicePostBuffer.Amount, Currency."Amount Rounding Precision");
                "VAT Amount" := -Round(InvoicePostBuffer."VAT Amount", Currency."Amount Rounding Precision");
                "VAT Base Amount" := Amount;
                "Source Currency Amount" := -Round(InvoicePostBuffer."Amount (ACY)", Currency."Amount Rounding Precision");
                "Source Curr. VAT Amount" := -Round(InvoicePostBuffer."VAT Amount (ACY)", Currency."Amount Rounding Precision");
                "Source Curr. VAT Base Amount" := "Source Currency Amount";
                "VAT Difference" := -Round(InvoicePostBuffer."VAT Difference", Currency."Amount Rounding Precision");

                // Correct Non VAT Deductible
                if InvoicePostBuffer."VAT % (Non Deductible)" <> 0 then begin
                    "VAT % (Non Deductible)" := InvoicePostBuffer."VAT % (Non Deductible)";
                    ...
                    "VAT Base LCY (Non Deduct.)" := -InvoicePostBuffer."VAT Base (Non Deductible)";
                    "VAT Amount LCY (Non Deduct.)" := -InvoicePostBuffer."VAT Amount (Non Deductible)";
                    if ("VAT Base (Non Deductible)" = 0) or ("VAT Amount (Non Deductible)" = 0) then
                        Validate("VAT % (Non Deductible)");
                end;
            end;
    ...
    local procedure PostVATDelayDifference(PurchHeader: Record "Purchase Header"; InvoicePostBuffer: Record "Invoice Post. Buffer"; PostingDate: Date; VATPostingSetup: Record "VAT Posting Setup"; VATDate: Date; AmtType: Option Base,VAT)
    var
        CalcAmt: Decimal;
        AccountNo: Code[20];
        AmtTmp: Decimal;
    begin
        ...
        case AmtType of
            AmtType::Base:
                begin
                    AccountNo := VATPostingSetup."Purchase VAT Delay Account";
                    if PurchHeader."Prices Including VAT" then
                        ...
                        // Amount - Ext.Amount
                        AmtTmp := Round(InvoicePostBuffer."Ext. Amount Including VAT" - InvoicePostBuffer."Ext. Amount" +
                            InvoicePostBuffer."Ext. VAT Difference (LCY)", Currency."Amount Rounding Precision");
                        AmtTmp := Round(AmtTmp * InvoicePostBuffer."VAT % (Non Deductible)" / 100, Currency."Amount Rounding Precision");
                        CalcAmt := (InvoicePostBuffer.Amount + InvoicePostBuffer."VAT Amount (Non Deductible)") -
                          (Round(InvoicePostBuffer."Ext. Amount", Currency."Amount Rounding Precision") + AmtTmp);
                    end;
                end;
            AmtType::VAT:
                begin
                    if InvoicePostBuffer."VAT % (Non Deductible)" <> 0 then begin
                        AmtTmp := Round(InvoicePostBuffer."Ext. Amount Including VAT" - InvoicePostBuffer."Ext. Amount" +
                            InvoicePostBuffer."Ext. VAT Difference (LCY)", Currency."Amount Rounding Precision");
                        AmtTmp := Round(AmtTmp * InvoicePostBuffer."VAT % (Non Deductible)" / 100, Currency."Amount Rounding Precision");
                        AmtTmp := (InvoicePostBuffer.Amount + InvoicePostBuffer."VAT Amount (Non Deductible)") -
                          (Round(InvoicePostBuffer."Ext. Amount", Currency."Amount Rounding Precision") + AmtTmp);
                        CalcAmt := ((InvoicePostBuffer.Amount + InvoicePostBuffer."VAT Amount") -
                                    InvoicePostBuffer."Ext. Amount Including VAT") - AmtTmp;
                    end else
        ...
        PostVATDelayDiffJournal(PurchHeader, InvoicePostBuffer, PostingDate, VATDate, CalcAmt, AccountNo);
    end;

    local procedure PostVATDelayDiffReverseVAT(PurchHeader: Record "Purchase Header"; InvoicePostBuffer: Record "Invoice Post. Buffer"; PostingDate: Date; VATPostingSetup: Record "VAT Posting Setup"; VATDate: Date)
    var
        AccountNo: Code[20];
        AmtTmp: Decimal;
        AmtTmp2: Decimal;
    ...
    begin
        with PurchHeader do begin
            if "VAT Currency Factor" = 0 then
                CurrFactor := 1
            else
                CurrFactor := "Currency Factor" / "VAT Currency Factor";
            if CurrFactor = 0 then
                CurrFactor := 1;

            // VAT Amt. Non Deduct
            AmtTmp := InvoicePostBuffer."VAT Amount";
            AmtTmp := Round(AmtTmp * InvoicePostBuffer."VAT % (Non Deductible)" / 100, Currency."Amount Rounding Precision");

            // VAT Amt. Non Deduct new Factor
            AmtTmp2 := Round(InvoicePostBuffer."VAT Amount" * CurrFactor, Currency."Amount Rounding Precision");
            AmtTmp2 := Round(AmtTmp2 * InvoicePostBuffer."VAT % (Non Deductible)" / 100, Currency."Amount Rounding Precision");

            CalcAmt := AmtTmp - AmtTmp2;
        end;
    ...
    local procedure PostVATDelayDiffJournal(PurchHeader: Record "Purchase Header"; InvoicePostBuffer: Record "Invoice Post. Buffer"; PostingDate: Date; VATDate: Date; CalcAmt: Decimal; AccountNo: Code[20])
    var
        GenJnlLine: Record "Gen. Journal Line";
    begin
        // post GenJnlLine
        with GenJnlLine do begin
            ...
            "VAT Delay" := true; // NAVCZ
            OnPostVATDelayDiffJournalOnBeforeRunGenJnlPostLine(GenJnlLine);
            RunGenJnlPostLine(GenJnlLine);
        end;
    end;

    local procedure PostItemChargePerPosAdjItem(PurchHeader: Record "Purchase Header"; PurchLine: Record "Purchase Line")
    var
        ItemLedgerEntry: Record "Item Ledger Entry";
        DummyTrackingSpecification: Record "Tracking Specification";
        CurrExchRate: Record "Currency Exchange Rate";
    begin
        // NAVCZ
        with TempItemChargeAssgntPurch do begin
            if not ItemLedgerEntry.Get("Applies-to Doc. Line No.") then
                Error(TextDeletedErr);
            PurchLine."No." := "Item No.";
            PurchLine."Appl.-to Item Entry" := ItemLedgerEntry."Entry No.";
            PurchLine.Amount := "Amount to Assign";
            PurchLine."Unit Cost" :=
              Round(
                PurchLine.Amount / ItemLedgerEntry.Quantity, GLSetup."Unit-Amount Rounding Precision");
            if "Document Type" in ["Document Type"::"Return Order", "Document Type"::"Credit Memo"] then
            ...
            if PurchHeader."Currency Code" <> '' then
                PurchLine.Amount :=
                  CurrExchRate.ExchangeAmtFCYToLCY(
                    Usedate, PurchHeader."Currency Code", PurchLine.Amount, PurchHeader."Currency Factor");
            PurchLine."Inv. Discount Amount" := Round(
                PurchLine."Inv. Discount Amount" / PurchLine.Quantity * "Qty. to Assign",
                GLSetup."Amount Rounding Precision");

            PurchLine.Amount := Round(PurchLine.Amount, GLSetup."Amount Rounding Precision");
        ...
        if PurchLine.FindSet(false, false) then
            repeat
                if VATPostingSetup.Get(PurchLine."VAT Bus. Posting Group", PurchLine."VAT Prod. Posting Group") then
                    if VATPostingSetup."Reverse Charge Check" > 0 then
                        if not ((PurchLine.Type = PurchLine.Type::"G/L Account") and
                                (VATPostingSetup."Purch. Ded. VAT Base Adj. Acc." = PurchLine."No."))
                        then begin
                            PurchLine.TestField("Tariff No.");
                            if TariffNo.Get(PurchLine."Tariff No.") then
                                if TariffNo."VAT Stat. Unit of Measure Code" <> '' then
                                    PurchLine.TestField("Unit of Measure Code", TariffNo."VAT Stat. Unit of Measure Code");
                        end;
            until PurchLine.Next() = 0;
    end;

    [Scope('OnPrem')]
    procedure AdjustAdvLetterRelation(PurchHeader: Record "Purchase Header"; LineNo: Integer; UpdAPInvAmount: Boolean)
    var
        AdvanceLetterLineRelation: Record "Advance Letter Line Relation";
        LetterLine: Record "Purch. Advance Letter Line";
        TLetterLine: Record "Purch. Advance Letter Line" temporary;
    begin
        // NAVCZ
        ...
        if UpdAPInvAmount then
            if TLetterLine.FindSet() then
                repeat
                    ...
                    PurchPostAdvances.UpdInvAmountToLineRelations(LetterLine);
    ...
    begin
        ...
        OnAfterInsertPostedHeaders(PurchHeader, PurchRcptHeader, PurchInvHeader, PurchCrMemoHeader, ReturnShptHeader, PurchSetup);
    end;
    ...
    local procedure InsertReceiptLine(PurchHeader: Record "Purchase Header"; PurchRcptHeader: Record "Purch. Rcpt. Header"; PurchLine: Record "Purchase Line"; CostBaseAmount: Decimal)
    var
        PurchRcptLine: Record "Purch. Rcpt. Line";
    ...
    begin
        ...
        OnInsertReceiptLineOnAfterInitPurchRcptLine(PurchRcptLine, PurchLine, ItemLedgShptEntryNo, xPurchLine);

        IsHandled := false;
        OnInsertReceiptLineOnBeforeProcessWhseShptRcpt(PurchLine, IsHandled);
        if not IsHandled then
        ...
        PurchRcptLine."Posting Date" := PurchHeader."Posting Date"; // NAVCZ
    ...
    local procedure InsertReturnShipmentLine(PurchHeader: Record "Purchase Header"; ReturnShptHeader: Record "Return Shipment Header"; PurchLine: Record "Purchase Line"; CostBaseAmount: Decimal)
    var
        ReturnShptLine: Record "Return Shipment Line";
        WhseRcptLine: Record "Warehouse Receipt Line";
        WhseShptLine: Record "Warehouse Shipment Line";
    ...
    begin
        ...
        ReturnShptLine."Posting Date" := PurchHeader."Posting Date"; // NAVCZ
    ...
    begin
        with PurchHeader do begin
            ...
            if ("Variable Symbol" = '') and (not BankAccount.IsEmpty) then begin
                VariableSymbol := BankOperationsFunctions.CreateVariableSymbol("Vendor Invoice No.");
                PurchInvHeader."Variable Symbol" := VariableSymbol;
            end;
            // NAVCZ
    ...
    begin
        with PurchHeader do begin
            ...
            // NAVCZ
            if ("Variable Symbol" = '') and (not BankAccount.IsEmpty) then begin
                VariableSymbol := BankOperationsFunctions.CreateVariableSymbol("Vendor Cr. Memo No.");
                PurchCrMemoHdr."Variable Symbol" := VariableSymbol;
            end;
            // NAVCZ
    ...
    begin
        ...
        with GenJnlLine do begin
            ...
            TransferNonDedVAT(GenJnlLine, InvoicePostBuffer);
    ...
    begin
        ...
        OnValidatePostingAndDocumentDateOnAfterCalcPostingDateExists(PurchaseHeader, PostingDateExists, ReplacePostingDate, PostingDate);
    ...
    local procedure ValidateVATDate(var PurchaseHeader: Record "Purchase Header")
    var
        BatchProcessingMgt: Codeunit "Batch Processing Mgt.";
        VATDate: Date;
    ...
    begin
        ...
        GetGLSetup;
        if GLSetup."Use VAT Date" and VATDateExists and (ReplaceVATDate or (PurchaseHeader."VAT Date" = 0D)) then begin
            PurchaseHeader.Validate("VAT Date", VATDate);
            PurchaseHeader.Modify();
        end;

        if BatchProcessingMgt.GetParameterDate(PurchaseHeader.RecordId, "Batch Posting Parameter Type"::"Posting Date", PostingDate) and
           not GLSetup."Use VAT Date" and (PurchaseHeader."Posting Date" <> PurchaseHeader."VAT Date")
        then begin
            PurchaseHeader.Validate("VAT Date", PurchaseHeader."Posting Date");
            PurchaseHeader.Modify();
        end;
    end;
    ...
    var
        ...
        Handled: Boolean;
    begin
        OnBeforeCheckExternalDocumentNumber(VendLedgEntry, PurchaseHeader, Handled, GenJnlLineDocType.AsInteger(), GenJnlLineExtDocNo);
        if Handled then
            exit;
    ...
    local procedure CheckExternalDocumentNumberInPostedDocuments(var PurchaseHeader: Record "Purchase Header")
    var
        PurchInvHeader: Record "Purch. Inv. Header";
            ...
            PurchaseHeader."Document Type"::Order:
                begin
                    ...
                    if GenJnlLineDocNo <> '' then
                        PurchInvHeader.SetFilter("No.", '<>%1', GenJnlLineDocNo);
                    PurchInvHeader.SetRange("Vendor Invoice No.", GenJnlLineExtDocNo);
                    PurchInvHeader.SetRange("Pay-to Vendor No.", PurchaseHeader."Pay-to Vendor No.");
                    if PurchInvHeader.FindFirst then
                        Error(
                          PurchaseAlreadyExistsErr, PurchaseHeader."Document Type"::Invoice, GenJnlLineExtDocNo);
                end;
            else begin
                    PurchCrMemoHdr.Reset();
                    PurchCrMemoHdr.SetCurrentKey("Vendor Cr. Memo No.");
                    if GenJnlLineDocNo <> '' then
                        PurchCrMemoHdr.SetFilter("No.", '<>%1', GenJnlLineDocNo);
                    PurchCrMemoHdr.SetRange("Vendor Cr. Memo No.", GenJnlLineExtDocNo);
                    PurchCrMemoHdr.SetRange("Pay-to Vendor No.", PurchaseHeader."Pay-to Vendor No.");
                    if PurchCrMemoHdr.FindFirst then
                        Error(
                          PurchaseAlreadyExistsErr, PurchaseHeader."Document Type"::"Credit Memo", GenJnlLineExtDocNo);
                end;
        end;
    end;

    [TryFunction]
    local procedure TryCheckExternalDocumentNumber(var PurchaseHeader: Record "Purchase Header")
    begin
        // NAVCZ
        CheckExternalDocumentNumber(VendLedgEntry, PurchaseHeader);
        CheckExternalDocumentNumberInPostedDocuments(PurchaseHeader);
    end;

    local procedure CalcVATAmount(VATBusPstGroup: Code[20]; VATProdPstGroup: Code[20]; VATBase: Decimal): Decimal
    var
        VATPostingSetup: Record "VAT Posting Setup";
    begin
        // NAVCZ
        VATPostingSetup.Get(VATBusPstGroup, VATProdPstGroup);
        exit(VATBase * VATPostingSetup."VAT %" / 100);
    end;
    ...
    begin
        ...
        if TempInvoicePostBuffer.Find('+') then
            repeat
                ...
                // NAVCZ
                if (PurchHeader."Currency Code" <> '') and (PurchHeader."Currency Factor" <> PurchHeader."VAT Currency Factor") and
                   ((TempInvoicePostBuffer."VAT Calculation Type" = TempInvoicePostBuffer."VAT Calculation Type"::"Reverse Charge VAT") or
                    (TempInvoicePostBuffer."VAT Calculation Type" = TempInvoicePostBuffer."VAT Calculation Type"::"Normal VAT")) and
                ...
                then begin
                    ...
                    VATPostingSetup.TestField("Purchase VAT Delay Account");
                    SourceCodeSetup.TestField("Purchase VAT Delay");
                    PostVATDelay(
                      PurchHeader, TempInvoicePostBuffer, false, PurchHeader."Posting Date", true, VATPostingSetup, PurchHeader."VAT Date");
                    PostVATDelay(
                      PurchHeader, TempInvoicePostBuffer, true, PurchHeader."Posting Date", false, VATPostingSetup, PurchHeader."VAT Date");
                    ...
                    case TempInvoicePostBuffer."VAT Calculation Type" of
                        TempInvoicePostBuffer."VAT Calculation Type"::"Normal VAT":
                            begin
                                PostVATDelayDifference(
                                  PurchHeader, TempInvoicePostBuffer, PurchHeader."Posting Date", VATPostingSetup, PurchHeader."VAT Date", 0);
                                PostVATDelayDifference(
                                  PurchHeader, TempInvoicePostBuffer, PurchHeader."Posting Date", VATPostingSetup, PurchHeader."VAT Date", 1);
                            end;
                        TempInvoicePostBuffer."VAT Calculation Type"::"Reverse Charge VAT":
                            PostVATDelayDiffReverseVAT(
                              PurchHeader, TempInvoicePostBuffer, PurchHeader."Posting Date", VATPostingSetup, PurchHeader."VAT Date");
                    end;
                end;
            // NAVCZ
                    ...
                    TempInvoicePostBuffer."VAT Calculation Type"::"Reverse Charge VAT":
                        begin
                            ...
                            if PurchSetup."Allow VAT Difference" then
                                VATBaseAmount := TempInvoicePostBuffer.Amount
                            else
                                VATBaseAmount := TempInvoicePostBuffer."VAT Base Amount";
                            VATBaseAmount := VATBaseAmount * (1 - PurchHeader."VAT Base Discount %" / 100);
                            ...
                            if TempInvoicePostBufferReverseCharge.Find then begin
                                ...
                                if PurchSetup."Allow VAT Difference" then
                                    TempInvoicePostBuffer."VAT Amount" := VATAmountLine.RoundVAT(VATAmountRemainder)
                                else
                                ...
                                if PurchSetup."Allow VAT Difference" then
                                    ...
                                    VATBaseAmount := TempInvoicePostBuffer."VAT Base Amount";
                                TempInvoicePostBuffer."VAT Base Amount" := Round(VATBaseAmount * (1 - PurchHeader."VAT Base Discount %" / 100));
    ...
    begin
        with PurchHeader do begin
            ...
            OnPostItemTrackingForShipmentOnAfterReturnShptLineReset(ReturnShptLine, PurchHeader);
            case "Document Type" of
                "Document Type"::"Return Order":
                    begin
                ...
                repeat
                    ...
                    if PurchHeader.Invoice then begin
                        ...
                        if "Quantity Invoiced" = Quantity then begin
                            AdjustAdvLetterRelation(PurchHeader, "Line No.", true);
                            PurchPostAdvances.UpdateOrderLine(TempPurchLine, PurchHeader."Prices Including VAT", true);
                        end;
                        // NAVCZ
                    end;
    ...
    begin
        ...
        IsHandled := false;
        OnBeforePostUpdateInvoiceLine(TempPurchLine, IsHandled);
        if IsHandled then
            exit;
        ...
        with TempPurchLine do begin
            ...
            if FindSet() then
                repeat
                    ...
                    PurchOrderLine.InitOutstanding();
                    PurchOrderLine.Modify();
                    OnPostUpdateCreditMemoLineOnAfterPurchOrderLineModify(PurchOrderLine, TempPurchLine);
                until Next() = 0;
        end;
    ...
    local procedure CheckCashDeskUserRights(var PurchHeader: Record "Purchase Header")
    begin
        // NAVCZ
        with PurchHeader do begin
            if "Cash Document Status" > "Cash Document Status"::" " then
                CashDeskMgt.CheckUserRights("Cash Desk Code", "Cash Document Status"::Create, false);

            if ("Cash Document Status" = "Cash Document Status"::Release) or
               ("Cash Document Status" = "Cash Document Status"::"Release and Print")
            then
                CashDeskMgt.CheckUserRights("Cash Desk Code", "Cash Document Status", false);

            if ("Cash Document Status" = "Cash Document Status"::Post) or
               ("Cash Document Status" = "Cash Document Status"::"Post and Print")
            then begin
                CashDeskMgt.CheckUserRights("Cash Desk Code", "Cash Document Status"::Release, false);
                CashDeskMgt.CheckUserRights("Cash Desk Code", "Cash Document Status", false);
            end;
        end;
    end;

    local procedure CheckDeferralSchedule(var PurchHeader: Record "Purchase Header")
    var
        PurchLine: Record "Purchase Line";
    begin
        // NAVCZ
        with PurchHeader do begin
            ...
            PurchLine.SetFilter("VAT % (Non Deductible)", '<>0');
            if PurchLine.IsEmpty then
                exit;

            Error(DeferralScheduleErr);
        end;
    end;
            ...
            if PurchLine.FindSet() then
                repeat
                    ...
                    if ItemCharge."Assigment on Receive/Shipment" then begin
                        ...
                        PurchLine.TestField("Qty. to Assign",
                          PurchLine."Qty. to Receive" + PurchLine."Quantity Received" - PurchLine."Quantity Invoiced");
                    end;
                until PurchLine.Next() = 0;
        end;
    end;
            ...
            if PurchLine.FindSet() then
                repeat
                    ...
                    if ItemCharge."Assigment on Receive/Shipment" then begin
                        PurchLine.CalcFields("Qty. to Assign");
                        PurchLine.TestField("Qty. to Assign",
                          PurchLine."Return Qty. to Ship" + PurchLine."Return Qty. Shipped" - PurchLine."Quantity Invoiced");
                    end;
                until PurchLine.Next() = 0;
        end;
    end;

    local procedure CheckIntrastatTransactionSetup(var PurchHeader: Record "Purchase Header")
    begin
        // NAVCZ
        with PurchHeader do begin
    ...
    [Obsolete('Moved to Core Localization Pack for Czech.', '17.0')]
    local procedure CheckVATDate(var PurchHeader: Record "Purchase Header")
    var
        GenJnlCheckLine: Codeunit "Gen. Jnl.-Check Line";
        ForwardLinkMgt: Codeunit "Forward Link Mgt.";
        SetupRecID: RecordID;
    ...
    local procedure CheckAcceptabilityDocNo(var PurchHeader: Record "Purchase Header")
    var
        NoSeriesMgt: Codeunit NoSeriesManagement;
    begin
        // NAVCZ
        with PurchHeader do begin
            if Receive and ("Receiving No." <> '') and not PreviewMode then
                NoSeriesMgt.CheckAcceptabilityDocNo("Receiving No.", "Receiving No. Series", "Posting Date");
            ...
            if Ship and ("Return Shipment No." <> '') and not PreviewMode then
                NoSeriesMgt.CheckAcceptabilityDocNo("Return Shipment No.", "Return Shipment No. Series", "Posting Date");
        end;
    end;

    local procedure CheckAndConfirmExternalDocumentNumber(var PurchHeader: Record "Purchase Header")
    var
        GenJnlLine: Record "Gen. Journal Line";
    begin
        // NAVCZ
        with PurchHeader do begin
            ...
            if (PurchSetup."Ext. Doc. No. Mandatory" or (GenJnlLineExtDocNo <> '')) and GuiAllowed then
                if not TryCheckExternalDocumentNumber(PurchHeader) then begin
                    GenJnlLine."Document Type" := GenJnlLineDocType;
                    ExtDocNoConfirmed :=
                      Confirm(
                        PurchaseAlreadyExistsQst, false,
                        GenJnlLine."Document Type", GenJnlLineExtDocNo);
                    if not ExtDocNoConfirmed then
                        Error('');
                end;
        end;
    end;
    ...
    local procedure CheckAllQtyToInvoice(PurchHeader: Record "Purchase Header")
    var
        PurchLine: Record "Purchase Line";
    begin
        // NAVCZ
        if not (PurchHeader."Document Type" in [PurchHeader."Document Type"::Order,
        ...
        if PurchLine.FindSet() then
            repeat
                ...
                CheckQtyToInvoice(PurchLine);
            until PurchLine.Next() = 0;
    end;

    local procedure CheckQtyToInvoice(PurchLine: Record "Purchase Line")
    var
        MaxQtyToInvoice: Decimal;
    begin
        // NAVCZ
        MaxQtyToInvoice := PurchLine.MaxQtyToInvoice;

        if PurchLine."Qty. to Invoice" > MaxQtyToInvoice then
            PurchLine.TestField("Qty. to Invoice", MaxQtyToInvoice);
    end;
    ...
    local procedure CheckAssociatedSalesOrderLine(PurchaseLine: Record "Purchase Line")
    var
        SalesLine: Record "Sales Line";
        IsHandled: Boolean;
    begin
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterPostPurchLines(var PurchHeader: Record "Purchase Header"; var PurchRcptHeader: Record "Purch. Rcpt. Header"; var PurchInvHeader: Record "Purch. Inv. Header"; var PurchCrMemoHdr: Record "Purch. Cr. Memo Hdr."; var ReturnShipmentHeader: Record "Return Shipment Header"; WhseShip: Boolean; WhseReceive: Boolean; var PurchLinesProcessed: Boolean; CommitIsSuppressed: Boolean; EverythingInvoiced: Boolean; var TempInvoicePostBuffer: Record "Invoice Post. Buffer" temporary)
    begin
    end;
    ...
    [IntegrationEvent(true, false)]
    local procedure OnBeforeCalcLineAmountAndLineDiscountAmount(PurchHeader: Record "Purchase Header"; var PurchLine: Record "Purchase Line"; PurchLineQty: Decimal; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeClearRemAmt(PurchaseHeader: Record "Purchase Header"; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeDeleteApprovalEntries(var PurchaseHeader: Record "Purchase Header"; var IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeDivideAmount(PurchHeader: Record "Purchase Header"; var PurchLine: Record "Purchase Line"; QtyType: Option General,Invoicing,Shipping; var PurchLineQty: Decimal; var TempVATAmountLine: Record "VAT Amount Line" temporary; var TempVATAmountLineRemainder: Record "VAT Amount Line" temporary)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeProcessAssocItemJnlLine(var PurchaseLine: Record "Purchase Line"; IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforePostItemJnlLine(PurchHeader: Record "Purchase Header"; var PurchLine: Record "Purchase Line"; var QtyToBeReceived: Decimal; var QtyToBeReceivedBase: Decimal; var QtyToBeInvoiced: Decimal; var QtyToBeInvoicedBase: Decimal; var ItemLedgShptEntryNo: Integer; var ItemChargeNo: Code[20]; var TrackingSpecification: Record "Tracking Specification"; CommitIsSupressed: Boolean; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforePostItemLine(PurchHeader: Record "Purchase Header"; PurchLine: Record "Purchase Line"; PurchRcptHeader: Record "Purch. Rcpt. Header"; RemQtyToBeInvoiced: Decimal)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforePostItemTrackingItemChargePerOrder(var TempTrackingSpecificationChargeAssmt: Record "Tracking Specification" temporary; var IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforePostItemTrackingLineOnPostPurchLine(PurchaseHeader: Record "Purchase Header"; PurchaseLine: Record "Purchase Line"; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeSumPurchLines2(QtyType: Option; var PurchHeader: Record "Purchase Header"; var PurchLine: Record "Purchase Line"; var VATAmountLine: Record "VAT Amount Line")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeUpdateAssocOrder(var TempDropShptPostBuffer: Record "Drop Shpt. Post. Buffer" temporary; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeShouldPostWhseJnlLine(PurchLine: Record "Purchase Line"; var Result: Boolean; var IsHandled: Boolean; var ItemJnlLine: Record "Item Journal Line"; var TempWhseJnlLine: Record "Warehouse Journal Line" temporary)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnCheckAssocOrderLinesOnBeforeCheckOrderLine(PurchaseHeader: Record "Purchase Header"; var PurchaseLine: Record "Purchase Line"; var IsHandled: Boolean; SalesOrderLine: Record "Sales Line")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnInsertReceiptLineOnAfterInitPurchRcptLine(var PurchRcptLine: Record "Purch. Rcpt. Line"; PurchLine: Record "Purchase Line"; ItemLedgShptEntryNo: Integer; xPurchLine: Record "Purchase Line")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnPostItemChargePerRcptOnAfterCalcDistributeCharge(PurchHeader: Record "Purchase Header"; PurchLine: Record "Purchase Line"; var PurchRcptLine: record "Purch. Rcpt. Line")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnPostItemJnlLineOnAfterSetFactor(var PurchaseLine: Record "Purchase Line"; var Factor: Decimal; var GenJnlLineExtDocNo: Code[35])
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnPostItemJnlLineOnAfterPrepareItemJnlLine(var ItemJournalLine: Record "Item Journal Line"; PurchaseLine: Record "Purchase Line"; PurchaseHeader: Record "Purchase Header"; PreviewMode: Boolean; var GenJnlLineDocNo: code[20]; TrackingSpecification: Record "Tracking Specification")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnPostItemJnlLineOnBeforeItemJnlPostLineRunWithCheck(var ItemJnlLine: Record "Item Journal Line"; var PurchaseLine: Record "Purchase Line"; DropShipOrder: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnPostPurchLineOnBeforeInsertCrMemoLine(PurchaseHeader: Record "Purchase Header"; PurchaseLine: Record "Purchase Line"; var IsHandled: Boolean);
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnPostPurchLineOnBeforeInsertReturnShipmentLine(var PurchaseHeader: Record "Purchase Header"; PurchaseLine: Record "Purchase Line"; var IsHandled: Boolean; ReturnShptHeader: Record "Return Shipment Header"; TempPurchLineGlobal: Record "Purchase Line"; RoundingLineInserted: Boolean);
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnPostPurchLineOnTypeCaseElse(var PurchaseHeader: Record "Purchase Header"; var PurchaseLine: Record "Purchase Line"; PurchInvHeader: Record "Purch. Inv. Header"; PurchCrMemoHeader: Record "Purch. Cr. Memo Hdr."; SourceCode: Code[10]);
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnPostUpdateCreditMemoLineOnAfterPurchOrderLineModify(var PurchaseLine: Record "Purchase Line"; var TempPurchaseLine: Record "Purchase Line" temporary)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnPostVATDelayDiffJournalOnBeforeRunGenJnlPostLine(var GenJnlLine: Record "Gen. Journal Line");
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnRunOnBeforeMakeInventoryAdjustment(var PurchaseHeader: Record "Purchase Header"; var GenJnlPostLine: Codeunit "Gen. Jnl.-Post Line")
    begin
    end;
    ...
    [IntegrationEvent(true, false)]
    local procedure OnCheckAndUpdateOnAfterCopyAndCheckItemCharge(PurchHeader: Record "Purchase Header")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforePostUpdateInvoiceLine(var TempPurchLine: Record "Purchase Line" temporary; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnPostItemTrackingForShipmentOnAfterReturnShptLineReset(var ReturnShptLine: Record "Return Shipment Line"; PurchHeader: Record "Purchase Header")
    begin
    end;
    ...
    [IntegrationEvent(true, false)]
    local procedure OnPostGLAndVendorOnAfterUpdatePurchaseHeader(PreviewMode: Boolean; var GenJnlLineDocNo: Code[20])
    begin
    end;
    ...
    [IntegrationEvent(true, false)]
    local procedure OnFinalizePostingOnAfterUpdateItemChargeAssgnt(var PurchHeader: Record "Purchase Header")
    begin
    end;

    [IntegrationEvent(true, false)]
    local procedure OnFinalizePostingOnBeforeInsertValueEntryRelation(var PurchHeader: Record "Purchase Header")
    begin
    end;

    [IntegrationEvent(true, false)]
    local procedure OnFinalizePostingOnBeforeInsertTrackingSpecification(var TempDropShptPostBuffer: Record "Drop Shpt. Post. Buffer" temporary; PurchHeader: Record "Purchase Header"; var TempTrackingSpecification: Record "Tracking Specification" temporary; EverythingInvoiced: Boolean; var TempPurchLineGlobal: Record "Purchase Line")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnFillInvoicePostBufferOnBeforePreparePurchase(var PurchHeader: Record "Purchase Header"; var PurchLine: Record "Purchase Line")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnInsertReceiptLineOnBeforeProcessWhseShptRcpt(var PurchLine: Record "Purchase Line"; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnCheckAndUpdateOnBeforeArchiveUnpostedOrder(PurchHeader: Record "Purchase Header"; PreviewMode: Boolean; var IsHandled: Boolean)
    begin
    end;
