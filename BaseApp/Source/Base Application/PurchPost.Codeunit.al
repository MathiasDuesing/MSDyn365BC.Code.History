codeunit 90 "Purch.-Post"
{
...
#endif
        OnRunOnAfterPostInvoice(PurchHeader, PurchRcptHeader, ReturnShptHeader, PurchInvHeader, PurchCrMemoHeader, PreviewMode, Window);

        if ICGenJnlLineNo > 0 then
            PostICGenJnl();

        OnRunOnBeforeMakeInventoryAdjustment(PurchHeader, GenJnlPostLine, ItemJnlPostLine);
        ...
        with PurchLine do begin
            ...
            if PurchHeader.Invoice then
                if "Document Type" in ["Document Type"::Order, "Document Type"::Invoice] then begin
                    ...
                    OnPostPurchLineOnBeforeInsertCrMemoLine(PurchHeader, PurchLine, IsHandled, PurchCrMemoLine);
                    if not IsHandled then begin
...
#endif
        OnBeforePostInvoice(PurchHeader, PreviewMode, SuppressCommit, GenJnlPostLine, IsHandled);
        if IsHandled then
            exit;

        with PurchHeader do begin
            // Post purchase and VAT to G/L entries from buffer
            PostInvoicePostingBuffer(PurchHeader);
...
#if not CLEAN19
        OnAfterPostGLAndVendor(PurchHeader, GenJnlPostLine, TotalPurchLine, TotalPurchLineLCY, SuppressCommit, VendLedgEntry);
#endif
    ...
    begin
        OnBeforePostItemLine(PurchHeader, PurchLine, PurchRcptHeader, RemQtyToBeInvoiced, TempPurchLineGlobal, TempDropShptPostBuffer, RemQtyToBeInvoicedBase);
    ...
    begin
        ...
        OnBeforePostItemJnlLine(
          ...
          ItemLedgShptEntryNo, ItemChargeNo, TrackingSpecification, SuppressCommit, IsHandled, ItemJnlPostLine);
        if IsHandled then
            exit;
        ...
        with ItemJnlLine do begin
            ...
            if QtyToBeInvoiced <> 0 then begin
                ...
                OnPostItemJnlLineOnAfterSetFactor(PurchLine, Factor, GenJnlLineExtDocNo, ItemJnlLine);
            ...
            OnPostItemJnlLineOnBeforeItemJnlPostLineRunWithCheck(ItemJnlLine, PurchLine, DropShipOrder, PurchHeader, WhseReceive, QtyToBeReceived, QtyToBeReceivedBase, QtyToBeInvoiced, QtyToBeInvoicedBase);
            ...
            if TransRcptLine."Item Rcpt. Entry No." = 0 then
                ...
                if ItemApplnEntry.FindSet() then
                    repeat
                        ...
                        OnPostItemChargePerTransferOnBeforePostItemJnlLine(PurchHeader, PurchLine2, ItemApplnEntry);
                        PostItemJnlLine(
    ...
    begin
        with PurchHeader do begin
            ...
            OnBeforeUpdateAssosOrderPostingNos(TempPurchLine, PurchHeader);
    ...
    var
        ...
        SkipDelete: Boolean;
    begin
        OnBeforeDeleteAfterPosting(PurchHeader, PurchInvHeader, PurchCrMemoHeader, SkipDelete, SuppressCommit, TempPurchLine);
        if SkipDelete then
            exit;
    ...
    begin
        ...
        with PurchHeader do begin
            ...
            then begin
                ...
                OnFinalizePostingOnAfterUpdateItemChargeAssgnt(PurchHeader, TempDropShptPostBuffer, EverythingInvoiced, TempPurchLineGlobal);
            end else begin
                ...
                IsHandled := false;
                OnFinalizePostingOnBeforeUpdateAfterPosting(PurchHeader, TempDropShptPostBuffer, EverythingInvoiced, IsHandled, TempPurchLineGlobal);
                if not IsHandled then begin
    ...
    begin
        ...
        OnAfterDeleteApprovalEntries(PurchHeader, PurchInvHeader);
    end;
    ...
    begin
        IsHandled := false;
        OnBeforeFillInvoicePostBuffer(PurchHeader, PurchLine, PurchLineACY, InvoicePostBuffer, IsHandled);
        if IsHandled then
            exit;

        GenPostingSetup.Get(PurchLine."Gen. Bus. Posting Group", PurchLine."Gen. Prod. Posting Group");
        OnFillInvoicePostBufferOnBeforePreparePurchase(PurchHeader, PurchLine, InvoicePostBuffer, PurchLineACY);
        ...
        OnFillInvoicePostingBufferOnBeforeSetAccount(PurchHeader, PurchLine, PurchAccount);
    ...
    begin
        ...
        with PurchLine do begin
            ...
            OnRoundAmountOnBeforeIncrAmount(PurchHeader, PurchLine, PurchLineQty, TotalPurchLine, TotalPurchLineLCY, xPurchLine, CurrExchRate, NoVAT);
    ...
    local procedure UpdatePurchaseHeader(var VendorLedgerEntry: Record "Vendor Ledger Entry")
    var
    ...
    begin
        OnBeforeUpdateQtyToBeInvoicedForReceipt(QtyToBeInvoiced, QtyToBeInvoicedBase, TrackingSpecificationExists, PurchLine, PurchRcptLine, InvoicingTrackingSpecification);
    ...
    begin
        OnBeforeUpdateQtyToBeInvoicedForReturnShipment(QtyToBeInvoiced, QtyToBeInvoicedBase, TrackingSpecificationExists, PurchLine, ReturnShipmentLine, InvoicingTrackingSpecification);
    ...
    begin
        ...
        with TempItemChargeAssgntPurch do begin
            ...
            if PurchHeader."Currency Code" <> '' then
                PurchLine.Amount :=
                  CurrExchRate.ExchangeAmtFCYToLCY(
                    PurchHeader.GetUseDate(), PurchHeader."Currency Code", PurchLine.Amount, PurchHeader."Currency Factor");
    ...
    begin
        // NAVCZ
        OnBeforeAdjustAdvLetterRelation(PurchHeader, LineNo, UpdAPInvAmount, IsHandled);
        If IsHandled then
            Exit;
    ...
    begin
        ...
        OnBeforeInsertPostedHeaders(PurchHeader, TempWhseRcptHeader, TempWhseShptHeader, GenJnlPostLine);
    ...
    begin
        ...
        OnInsertReceiptLineOnAfterInitPurchRcptLine(PurchRcptLine, PurchLine, ItemLedgShptEntryNo, xPurchLine, PurchRcptHeader, CostBaseAmount, PostedWhseRcptHeader, WhseRcptHeader);
        ...
        if not IsHandled then
            if (PurchLine.Type = PurchLine.Type::Item) and (PurchLine."Qty. to Receive" <> 0) then begin
                if WhseReceive and PurchLine.IsInventoriableItem() then
                ...
                if WhseShip and PurchLine.IsInventoriableItem() then
    ...
    begin
        ...
        OnInsertReturnShipmentLineOnAfterReturnShptLineInit(ReturnShptHeader, ReturnShptLine, PurchLine, xPurchLine, CostBaseAmount);
    ...
    local procedure PostInvoicePostingBuffer(PurchHeader: Record "Purchase Header")
    var
        VATPostingSetup: Record "VAT Posting Setup";
        LineCount: Integer;
        GLEntryNo: Integer;
    ...
    local procedure CalculateVATAmountInBuffer(PurchHeader: Record "Purchase Header"; var InvoicePostBuffer: Record "Invoice Post. Buffer"; var VATAmountRemainder: Decimal; var VATAmountACYRemainder: Decimal)
    var
    ...
    begin
        IsHandled := false;
        OnBeforeCalculateVATAmountInBuffer(PurchHeader, InvoicePostBuffer, IsHandled);
        if IsHandled then
            exit;

        case InvoicePostBuffer."VAT Calculation Type" of
            InvoicePostBuffer."VAT Calculation Type"::"Reverse Charge VAT":
                begin
                    VATPostingSetup.Get(
                        InvoicePostBuffer."VAT Bus. Posting Group", InvoicePostBuffer."VAT Prod. Posting Group");
                    OnPostInvoicePostingBufferOnAfterVATPostingSetupGet(VATPostingSetup);

                    if PurchSetup."Allow VAT Difference" then
                        VATAmount :=
                            InvoicePostBuffer.Amount * (1 - PurchHeader."VAT Base Discount %" / 100) *
                            VATPostingSetup."VAT %" / 100
                    else
                        VATAmount :=
                            InvoicePostBuffer."VAT Base Amount" * (1 - PurchHeader."VAT Base Discount %" / 100) *
                            VATPostingSetup."VAT %" / 100;

                    VATAmountACY :=
                        InvoicePostBuffer."VAT Base Amount (ACY)" * (1 - PurchHeader."VAT Base Discount %" / 100) *
                        VATPostingSetup."VAT %" / 100;

                    TempInvoicePostBufferReverseCharge := InvoicePostBuffer;
                    if TempInvoicePostBufferReverseCharge.Find() then begin
                        VATAmountRemainder += VATAmount;
                        if PurchSetup."Allow VAT Difference" then
                            InvoicePostBuffer."VAT Amount" := VATAmountLine.RoundVAT(VATAmountRemainder)
                        else
                            InvoicePostBuffer."VAT Amount" := Round(VATAmountRemainder);
                        VATAmountRemainder -= TempInvoicePostBuffer."VAT Amount";

                        VATAmountACYRemainder += VATAmountACY;
                        InvoicePostBuffer."VAT Amount (ACY)" := Round(VATAmountACYRemainder, Currency."Amount Rounding Precision");
                        VATAmountACYRemainder -= InvoicePostBuffer."VAT Amount (ACY)"
                    end else begin
                        if PurchSetup."Allow VAT Difference" then
                            InvoicePostBuffer."VAT Amount" := VATAmountLine.RoundVAT(VATAmount)
                        else
                            InvoicePostBuffer."VAT Amount" := Round(VATAmount);
                        InvoicePostBuffer."VAT Amount (ACY)" := Round(VATAmountACY, Currency."Amount Rounding Precision");
                    end;
                end;
            InvoicePostBuffer."VAT Calculation Type"::"Sales Tax":
                if InvoicePostBuffer."Use Tax" then begin
                    InvoicePostBuffer."VAT Amount" :=
                        Round(
                            SalesTaxCalculate.CalculateTax(
                                InvoicePostBuffer."Tax Area Code", InvoicePostBuffer."Tax Group Code",
                                InvoicePostBuffer."Tax Liable", PurchHeader."Posting Date",
                                InvoicePostBuffer.Amount, InvoicePostBuffer.Quantity, 0));
    ...
    local procedure SetCheckApplToItemEntry(PurchLine: Record "Purchase Line") Result: Boolean
    var
        IsHandled: Boolean;
    begin
        IsHandled := false;
        OnBeforeSetCheckApplToItemEntry(PurchLine, Result, IsHandled);
        if IsHandled then
            exit(Result);
    ...
    begin
        ...
        OnAfterPurchRcptLineInsert(PurchLine, PurchRcptLine, ItemLedgShptEntryNo, WhseShip, WhseReceive, SuppressCommit, PurchInvHeader, TempTrackingSpecification, PurchRcptHeader, TempWhseRcptHeader, xPurchLine);
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterDeleteApprovalEntries(var PurchaseHeader: Record "Purchase Header"; var PurchInvHeader: Record "Purch. Inv. Header")
    begin
    end;
...
#if not CLEAN19
    [Obsolete('Replaced by OnAfterPostInvoice()', '19.0')]
    [IntegrationEvent(false, false)]
    local procedure OnAfterPostGLAndVendor(var PurchHeader: Record "Purchase Header"; var GenJnlPostLine: Codeunit "Gen. Jnl.-Post Line"; TotalPurchLine: Record "Purchase Line"; TotalPurchLineLCY: Record "Purchase Line"; CommitIsSupressed: Boolean; var VendorLedgerEntry: Record "Vendor Ledger Entry")
    begin
    end;
#endif
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeDeleteAfterPosting(var PurchaseHeader: Record "Purchase Header"; var PurchInvHeader: Record "Purch. Inv. Header"; var PurchCrMemoHdr: Record "Purch. Cr. Memo Hdr."; var SkipDelete: Boolean; CommitIsSupressed: Boolean; var TempPurchLine: Record "Purchase Line" temporary)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeFillInvoicePostBuffer(PurchHeader: Record "Purchase Header"; PurchLine: Record "Purchase Line"; PurchLineACY: Record "Purchase Line"; InvoicePostBuffer: Record "Invoice Post. Buffer"; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeInsertPostedHeaders(var PurchaseHeader: Record "Purchase Header"; var WarehouseReceiptHeader: Record "Warehouse Receipt Header"; var WarehouseShipmentHeader: Record "Warehouse Shipment Header"; var GenJnlPostLine: Codeunit "Gen. Jnl.-Post Line")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforePostInvoice(var PurchHeader: Record "Purchase Header"; PreviewMode: Boolean; CommitIsSupressed: Boolean; var GenJnlPostLine: Codeunit "Gen. Jnl.-Post Line"; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeProcessAssocItemJnlLine(var PurchaseLine: Record "Purchase Line"; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeSetCheckApplToItemEntry(var PurchaseLine: Record "Purchase Line"; var Result: Boolean; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforePostItemJnlLine(PurchHeader: Record "Purchase Header"; var PurchLine: Record "Purchase Line"; var QtyToBeReceived: Decimal; var QtyToBeReceivedBase: Decimal; var QtyToBeInvoiced: Decimal; var QtyToBeInvoicedBase: Decimal; var ItemLedgShptEntryNo: Integer; var ItemChargeNo: Code[20]; var TrackingSpecification: Record "Tracking Specification"; CommitIsSupressed: Boolean; var IsHandled: Boolean; var ItemJnlPostLine: Codeunit "Item Jnl.-Post Line")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeUpdateQtyToBeInvoicedForReceipt(var QtyToBeInvoiced: Decimal; var QtyToBeInvoicedBase: Decimal; TrackingSpecificationExists: Boolean; PurchLine: Record "Purchase Line"; PurchRcptLine: Record "Purch. Rcpt. Line"; InvoicingTrackingSpecification: Record "Tracking Specification")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeUpdateQtyToBeInvoicedForReturnShipment(var QtyToBeInvoiced: Decimal; var QtyToBeInvoicedBase: Decimal; TrackingSpecificationExists: Boolean; PurchLine: Record "Purchase Line"; ReturnShipmentLine: Record "Return Shipment Line"; InvoicingTrackingSpecification: Record "Tracking Specification")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnFillInvoicePostingBufferOnBeforeSetAccount(PurchaseHeader: Record "Purchase Header"; PurchaseLine: Record "Purchase Line"; var PurchAccount: Code[20])
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnInsertReturnShipmentLineOnAfterReturnShptLineInit(var ReturnShptHeader: Record "Return Shipment Header"; var ReturnShptLine: Record "Return Shipment Line"; var PurchLine: Record "Purchase Line"; var xPurchLine: Record "Purchase Line"; var CostBaseAmount: Decimal);
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnPostItemChargePerTransferOnBeforePostItemJnlLine(PurchHeader: Record "Purchase Header"; var PurchLine: Record "Purchase Line"; ItemApplnEntry: Record "Item Application Entry")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnPostPurchLineOnBeforeInsertCrMemoLine(PurchaseHeader: Record "Purchase Header"; PurchaseLine: Record "Purchase Line"; var IsHandled: Boolean; var PurchCrMemoLine: Record "Purch. Cr. Memo Line");
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnRunOnBeforeMakeInventoryAdjustment(var PurchaseHeader: Record "Purchase Header"; var GenJnlPostLine: Codeunit "Gen. Jnl.-Post Line"; var ItemJnlPostLine: Codeunit "Item Jnl.-Post Line")
    begin
    end;
...
#if not CLEAN19
    [Obsolete('Replaced by event OnAfterUpdatePurchaseHeader()', '19.0')]
    [IntegrationEvent(true, false)]
    ...
    [IntegrationEvent(true, false)]
    local procedure OnFinalizePostingOnAfterUpdateItemChargeAssgnt(var PurchHeader: Record "Purchase Header"; var TempDropShptPostBuffer: Record "Drop Shpt. Post. Buffer" temporary; var EverythingInvoiced: Boolean; var TempPurchLine: Record "Purchase Line" temporary)
    begin
    end;
    ...
    local procedure OnFinalizePostingOnBeforeUpdateWhseDocuments(var PurchaseHeader: Record "Purchase Header"; WarehouseReceiptHeader: Record "Warehouse Receipt Header"; TempWarehouseReceiptHeader: Record "Warehouse Receipt Header" temporary;
        WarehouseShipmentHeader: Record "Warehouse Shipment Header"; TempWarehouseShipmentHeader: Record "Warehouse Shipment Header" temporary; WarehouseReceive: Boolean; WarehouseShip: Boolean; IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnFillInvoicePostBufferOnBeforePreparePurchase(var PurchHeader: Record "Purchase Header"; var PurchLine: Record "Purchase Line"; var InvoicePostBuffer: Record "Invoice Post. Buffer"; PurchLineACY: Record "Purchase Line")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeUpdateAssosOrderPostingNos(var TempPurchLine: Record "Purchase Line" temporary; var PurchHeader: Record "Purchase Header")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnRunOnAfterPostInvoice(var PurchaseHeader: Record "Purchase Header"; var PurchRcptHeader: Record "Purch. Rcpt. Header"; var ReturnShipmentHeader: Record "Return Shipment Header"; var PurchInvHeader: Record "Purch. Inv. Header"; var PurchCrMemoHdr: Record "Purch. Cr. Memo Hdr."; var PreviewMode: Boolean; var Window: Dialog)
    begin
    end;
