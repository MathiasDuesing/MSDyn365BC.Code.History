codeunit 90 "Purch.-Post"
{
...
#endif
        OnBeforePostInvoice(PurchHeader, PreviewMode, SuppressCommit, GenJnlPostLine, IsHandled, Window, HideProgressWindow, TotalPurchLine, TotalPurchLineLCY, InvoicePostingInterface, InvoicePostingParameters);
        if IsHandled then
            exit;
        ...
        with PurchHeader do begin
            ...
            if (TotalPurchLine."Amount Including VAT" + VendLedgEntry."Remaining Pmt. Disc. Possible") <> 0 then // NAVCZ
                if "Bal. Account No." <> '' then begin
                    ...
                    if UseLegacyInvoicePosting() then
                        PostBalancingEntry(
                            PurchHeader, TotalPurchLine, TotalPurchLineLCY, GenJnlLineDocType, GenJnlLineDocNo, GenJnlLineExtDocNo, SrcCode, InvoicedAdvAmount)
                    else
#endif
...
#if not CLEAN18
            Increment(TotalPurchLine."VAT Difference (LCY)", "VAT Difference (LCY)"); // NAVCZ
#endif
    ...
    begin
        ...
        if not (PurchHeader.Receive or PurchHeader.Invoice or PurchHeader.Ship) then
            Error(ErrorInfo.Create(NothingToPostErr, true, PurchHeader));
    end;
    ...
    internal procedure OnPostVATDelayOnBeforeRunGenJnlPostLine(var GenJnlLine: Record "Gen. Journal Line");
    begin
    end;

    [IntegrationEvent(false, false)]
    [Obsolete('The function from which this event was triggered has been deleted.', '20.0')]
    internal procedure OnPostVATDelayDiffJournalOnBeforeRunGenJnlPostLine(var GenJnlLine: Record "Gen. Journal Line");
    begin
    end;
#endif
