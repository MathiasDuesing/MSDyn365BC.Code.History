codeunit 90 "Purch.-Post"
{
    ...
    local procedure CalcInvoice(var PurchHeader: Record "Purchase Header"; var ReverseChargeVAT: Boolean) NewInvoice: Boolean
    var
    ...
    begin
        with PurchHeader do begin
            ...
            if Receive then begin
                ...
                if (not InvtPickPutaway) and CheckIfInvPutawayExists(PurchHeader) then
                    Error(InvPutAwayExistsErr);
            end;

            if Ship then begin
                Ship := CheckTrackingAndWarehouseForShip(PurchHeader);
                if (not InvtPickPutaway) and CheckIfInvPickExists then
    ...
    begin
        ...
        with GenJnlLine do begin
            ...
            if not PreviewMode then
                if PurchHeader."Currency Code" = '' then begin
                    if Abs("Amount (LCY)") <> PurchHeader."Check Total" then
                        Error(Text1130004, Abs("Amount (LCY)"), PurchHeader."Check Total");
                end else
                    if Abs(TotalPurchLine2."Amount Including VAT") <> PurchHeader."Check Total" then
                        Error(Text1130005, Abs(TotalPurchLine2."Amount Including VAT"), PurchHeader."Check Total", PurchHeader."Currency Code");
    ...
    begin
        ...
        VendLedgEntry.SetRange("Document Type", PurchHeader."Document Type".AsInteger());
    ...
    local procedure PostInvoicePostBufferLine(var PurchHeader: Record "Purchase Header"; InvoicePostBuffer: Record "Invoice Post. Buffer"; var GenJnlLine: Record "Gen. Journal Line"): Integer
    begin
