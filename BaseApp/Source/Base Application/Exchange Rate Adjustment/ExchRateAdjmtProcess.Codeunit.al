codeunit 699 "Exch. Rate Adjmt. Process"
{
    ...
    begin
        ...
        CheckPostingDate();
        if ExchRateAdjmtParameters."End Date" = 0D then
            Error(Text12401Err);
    ...
    var
        ...
        TempSelectedDimGains: Record "Selected Dimension" temporary;
        TempSelectedDimLosses: Record "Selected Dimension" temporary;
        ...
        DimensionsTxt: Label 'Dimensions';
    ...
    begin
        ...
        if CustNo <> 0 then
            if GLSetup."Enable Russian Accounting" then
                HandlePostAdjmtRU("Exch. Rate Adjmt. Account Type"::Customer)
            else
    ...
    begin
        ShouldAdjust := not (GLSetup."Cancel Prepmt. Adjmt. in TA" and CustLedgEntry.Prepayment);
    ...
    begin
        ShouldAdjust := not (GLSetup."Cancel Prepmt. Adjmt. in TA" and VendLedgEntry.Prepayment);
    ...
    local procedure PostAdjmt(ExchRateAdjmtBuffer: Record "Exch. Rate Adjmt. Buffer"; TempDimSetEntry: Record "Dimension Set Entry" temporary; CVLedgEntryBuf: Record "CV Ledger Entry Buffer"; Gains: Boolean): Integer
    begin
        exit(
            PostAdjmt(
    ...
    local procedure PostCustAdjmt(ExchRateAdjmtBuffer: Record "Exch. Rate Adjmt. Buffer"; var TempDtldCVLedgEntryBuf: Record "Detailed CV Ledg. Entry Buffer" temporary; var TempDimSetEntry: Record "Dimension Set Entry" temporary; CVLedgEntryBuf: Record "CV Ledger Entry Buffer")
    begin
        TempDtldCVLedgEntryBuf."Transaction No." := PostAdjmt(ExchRateAdjmtBuffer, TempDimSetEntry, CVLedgEntryBuf, true);
    ...
    local procedure PostVendAdjmt(ExchRateAdjmtBuffer: Record "Exch. Rate Adjmt. Buffer"; var TempDtldCVLedgEntryBuf: Record "Detailed CV Ledg. Entry Buffer" temporary; var TempDimSetEntry: Record "Dimension Set Entry" temporary; CVLedgEntryBuf: Record "CV Ledger Entry Buffer")
    begin
        TempDtldCVLedgEntryBuf."Transaction No." := PostAdjmt(ExchRateAdjmtBuffer, TempDimSetEntry, CVLedgEntryBuf, true);
        if TempDtldCVLedgEntryBuf.Insert() then;
        InsertExchRateAdjmtReg(
                    ...
                    if TempExchRateAdjmtBuffer."Adjmt. Amount" <> 0 then
                        case AdjustAccType of
                            "Exch. Rate Adjmt. Account Type"::Customer:
                                PostCustAdjmt(TempExchRateAdjmtBuffer, TempDtldCVLedgEntryBuf, TempDimSetEntry, CVLedgEntryBuf);
                            "Exch. Rate Adjmt. Account Type"::Vendor:
                                PostVendAdjmt(TempExchRateAdjmtBuffer, TempDtldCVLedgEntryBuf, TempDimSetEntry, CVLedgEntryBuf);
                        end;
    ...
    begin
        ShouldAdjust := not Currency.Conventional;
    ...
    local procedure HandlePostAdjmtRU(AdjustAccType: Enum "Exch. Rate Adjmt. Account Type")
    var
        CustPostingGr: Record "Customer Posting Group";
        VendPostingGr: Record "Vendor Posting Group";
    ...
    begin
        ...
        if TempExchRateAdjmtBuffer2.Find('-') then
            repeat
                ...
                TempDimSetEntry.DeleteAll();
                TempExchRateAdjmtBuffer.Find('-');
                DimBufMgt.GetDimensions(TempExchRateAdjmtBuffer."Dimension Entry No.", TempDimBuf);
                DimMgt.CopyDimBufToDimSetEntry(TempDimBuf, TempDimSetEntry);
                repeat
                    if TempExchRateAdjmtBuffer."Adjmt. Amount" <> 0 then begin
                        case AdjustAccType of
                            "Exch. Rate Adjmt. Account Type"::Customer:
                                begin
                                    CustPostingGr.Get(TempExchRateAdjmtBuffer."Posting Group");
                                    CustLedgEntryToCVLedgEntry(TempExchRateAdjmtBuffer."Entry No.", TempCVLedgEntryBuf);
                                    if TempCVLedgEntryBuf.Prepayment then begin
                                        if GLSetup."Cancel Curr. Prepmt. Adjmt." then begin
                                            Currency.Get(TempExchRateAdjmtBuffer."Currency Code");
                                    ...
                                    InsertExchRateAdjmtReg(
                                        "Exch. Rate Adjmt. Account Type"::Customer, TempExchRateAdjmtBuffer."Posting Group", TempExchRateAdjmtBuffer."Currency Code");
                                end;
                            "Exch. Rate Adjmt. Account Type"::Vendor:
                                begin
                                    VendPostingGr.Get(TempExchRateAdjmtBuffer."Posting Group");
                                    VendLedgEntryToCVLedgEntry(TempExchRateAdjmtBuffer."Entry No.", TempCVLedgEntryBuf);
                                    if TempCVLedgEntryBuf.Prepayment then begin
                                        if GLSetup."Cancel Curr. Prepmt. Adjmt." then begin
                                            Currency.Get(TempExchRateAdjmtBuffer."Currency Code");
                                            PostAccount := Currency.GetPDBalGainLossAccTA();
                                    ...
                                    InsertExchRateAdjmtReg(
                                        "Exch. Rate Adjmt. Account Type"::Vendor, TempExchRateAdjmtBuffer."Posting Group", TempExchRateAdjmtBuffer."Currency Code");
                                end;
                        end;
                        if GLSetup."Summarize Gains/Losses" then
                            if (TempExchRateAdjmtBuffer."Gains Amount" <> 0) and (TempExchRateAdjmtBuffer."Losses Amount" <> 0) then
                                case true of
                                    TempExchRateAdjmtBuffer."Gains Amount" < 0:
                                        begin
                                            TempExchRateAdjmtBuffer."Losses Amount" += TempExchRateAdjmtBuffer."Gains Amount";
                                            TempExchRateAdjmtBuffer."Gains Amount" := 0;
                                        end;
                                    TempExchRateAdjmtBuffer."Losses Amount" > 0:
                                        begin
                                            TempExchRateAdjmtBuffer."Gains Amount" += TempExchRateAdjmtBuffer."Losses Amount";
                                            TempExchRateAdjmtBuffer."Losses Amount" := 0;
                                        end;
                                end;
                        if TempExchRateAdjmtBuffer."Gains Amount" <> 0 then begin
                            if GLSetup."Currency Adjmt with Correction" then begin
                                TempCVLedgEntryBuf.Positive := TempExchRateAdjmtBuffer."Gains Amount" < 0;
                                Gains := true;
                            end else
                                Gains := TempExchRateAdjmtBuffer."Gains Amount" > 0;
                            PostAdjmt(
                                PostAccount, TempExchRateAdjmtBuffer."Gains Amount", TempExchRateAdjmtBuffer."Adjmt. Base",
                                TempExchRateAdjmtBuffer."Currency Code", TempDimSetEntry,
                                TempExchRateAdjmtBuffer."Posting Date", TempExchRateAdjmtBuffer."IC Partner Code", TempCVLedgEntryBuf, Gains);
                        end;
                        if TempExchRateAdjmtBuffer."Losses Amount" <> 0 then begin
                            if GLSetup."Currency Adjmt with Correction" then begin
                                TempCVLedgEntryBuf.Positive := TempExchRateAdjmtBuffer."Losses Amount" > 0;
                                Gains := false;
                            end else
                                Gains := TempExchRateAdjmtBuffer."Losses Amount" > 0;
                            PostAdjmt(
                                PostAccount, TempExchRateAdjmtBuffer."Losses Amount", TempExchRateAdjmtBuffer."Adjmt. Base",
                                TempExchRateAdjmtBuffer."Currency Code", TempDimSetEntry,
                                TempExchRateAdjmtBuffer."Posting Date", TempExchRateAdjmtBuffer."IC Partner Code", TempCVLedgEntryBuf, Gains);
                        end;
                    end;
    ...
    local procedure GetTempJnlLineDim(var TempDimSetEntry: Record "Dimension Set Entry"; var GenJnlLine: Record "Gen. Journal Line"; BalAccType: Option "None",Real,Unreal; CurrencyCode: Code[10]; Gains: Boolean)
    var
    ...
    begin
        ...
        if Gains then
            CopySelectedDimToJnlLineDim(TempSelectedDimGains, TempDimSetEntry)
        else
            CopySelectedDimToJnlLineDim(TempSelectedDimLosses, TempDimSetEntry);
    end;

    local procedure CopySelectedDimToJnlLineDim(var SelectedDim: Record "Selected Dimension"; var DimSetEntry: Record "Dimension Set Entry")
    var
    ...
    begin
        if SelectedDim.FindSet() then
            repeat
                ...
                DimSetEntry."Dimension Code" := CopyStr(SelectedDim."Dimension Code", 1, MaxStrLen(DimSetEntry."Dimension Code"));
    ...
    local procedure GetSelectedDims()
    begin
        if ExchRateAdjmtParameters."Gains Dimensions" <> '' then begin
            DimSelectionBuf.CompareDimText(3, REPORT::"Exch. Rate Adjustment", '', ExchRateAdjmtParameters."Gains Dimensions", DimensionsTxt);
            SelectedDim.GetSelectedDim(
                CopyStr(UserId(), 1, MaxStrLen(SelectedDim."User ID")), 3, REPORT::"Exch. Rate Adjustment", '', TempSelectedDimGains);
            TempSelectedDimGains.SetFilter("New Dimension Value Code", '<>%1', '');
        end;
        if ExchRateAdjmtParameters."Losses Dimensions" <> '' then begin
            DimSelectionBuf.CompareDimText(3, -REPORT::"Exch. Rate Adjustment", '', ExchRateAdjmtParameters."Losses Dimensions", DimensionsTxt);
            SelectedDim.GetSelectedDim(
                CopyStr(UserId(), 1, MaxStrLen(SelectedDim."User ID")), 3, -REPORT::"Exch. Rate Adjustment", '', TempSelectedDimLosses);
            TempSelectedDimLosses.SetFilter("New Dimension Value Code", '<>%1', '');
        end;
    end;

    local procedure CustLedgEntryToCVLedgEntry(EntryNo: Integer; var TempCVLedgEntryBuf: Record "CV Ledger Entry Buffer" temporary)
    var
        CustLedgEntry: Record "Cust. Ledger Entry";
    begin
        CustLedgEntry.Get(EntryNo);
        TempCVLedgEntryBuf."Document No." := CustLedgEntry."Document No.";
        TempCVLedgEntryBuf."Document Type" := CustLedgEntry."Document Type";
        TempCVLedgEntryBuf."Bal. Account Type" := "Gen. Journal Account Type"::Customer;
        TempCVLedgEntryBuf."Bal. Account No." := CustLedgEntry."Customer No.";
        TempCVLedgEntryBuf.Prepayment := CustLedgEntry.Prepayment;
        TempCVLedgEntryBuf."Agreement No." := CustLedgEntry."Agreement No.";
    end;

    local procedure VendLedgEntryToCVLedgEntry(EntryNo: Integer; var TempCVLedgEntryBuf: Record "CV Ledger Entry Buffer" temporary)
    var
        VendLedgEntry: Record "Vendor Ledger Entry";
    begin
        VendLedgEntry.Get(EntryNo);
        TempCVLedgEntryBuf."Document No." := VendLedgEntry."Document No.";
        TempCVLedgEntryBuf."Document Type" := VendLedgEntry."Document Type";
        TempCVLedgEntryBuf."Bal. Account Type" := "Gen. Journal Account Type"::Vendor;
        TempCVLedgEntryBuf."Bal. Account No." := VendLedgEntry."Vendor No.";
        TempCVLedgEntryBuf.Prepayment := VendLedgEntry.Prepayment;
        TempCVLedgEntryBuf."Agreement No." := VendLedgEntry."Agreement No.";
    end;

    local procedure UpdateTransactionNo(AdjustAccType: Enum "Exch. Rate Adjmt. Account Type")
    var
        GLEntry: Record "G/L Entry";
    begin
        case AdjustAccType of
    ...
    local procedure GetTaxAccRemainingAmt(AdjustAccType: Option Customer,Vendor; CVLedgerEntryNo: Integer; PostingDate3: Date; RemainingAmount: Decimal): Decimal
    var
        DtldCustLedgEntry2: Record "Detailed Cust. Ledg. Entry";
        DtldVendLedgEntry2: Record "Detailed Vendor Ledg. Entry";
