page 5802 "Value Entries"
{
    ...
    layout
    {
        area(content)
        {
            repeater(Control1)
            {
                ...
                field("Red Storno"; Rec."Red Storno")
                {
                    ApplicationArea = Basic, Suite;
                    ToolTip = 'Specifies if the value entry uses red storno.';
                }
                field(DebitField; DebitText)
                {
                    ApplicationArea = Basic, Suite;
    ...
    [Scope('OnPrem')]
    procedure CalcRedStornoAmount(Amount: Decimal; RedStorno: Boolean) ResultAmount: Text[30]
    begin
    ...
    local procedure CreditTextOnFormat(var Text: Text[1024])
    begin
        Text := CalcRedStornoAmount(-"Cost Amount (Actual)", "Red Storno");
