table 39 "Purchase Line"
{
    ...
    fields
    {
        ...
        field(15; Quantity; Decimal)
        {
            ...
            var
                ...
                TextCZ000: Label 'must be integral number';
                IsHandled: Boolean;
            ...
            begin
                ...
                if Type = Type::Item then begin
                    ...
                    if ItemUoM."Indivisible Unit" then begin
                        if "Quantity (Base)" <> Round("Quantity (Base)", 1) then
                            FieldError("Quantity (Base)", TextCZ000);
                    end;
                end;
                // NAVCZ
                ...
                then begin
                    ...
                    "VAT Base Amount" := 0;
                    "VAT Base (Non Deductible)" := 0; // NAVCZ
                    "VAT Amount (Non Deductible)" := 0; // NAVCZ
                end;
        ...
        field(90; "VAT Prod. Posting Group"; Code[20])
        {
            ...
            begin
                ...
                "VAT % (Non Deductible)" := GetVATDeduction; // NAVCZ
                UpdateAmounts;
        ...
        field(5401; "Prod. Order No."; Code[20])
        {
            ...
            begin
                ...
                // NAVCZ
                if (CurrFieldNo <> 0) and ("Prod. Order No." <> xRec."Prod. Order No.") then begin
                    "Routing No." := '';
                    "Operation No." := '';
                    "Work Center No." := '';
        ...
        {
            CalcFormula = Sum ("Advance Letter Line Relation"."Requested Amount" WHERE(Type = CONST(Purchase),
                                                                                       "Document No." = FIELD("Document No."),
                                                                                       "Document Line No." = FIELD("Line No.")));
        ...
        field(99000751; "Operation No."; Code[10])
        {
            ...
            begin
                ...
                Description := ProdOrderRtngLine.Description;  // NAVCZ
            end;
        }
        field(99000752; "Work Center No."; Code[20])
        {
            ...
            begin
                ...
                TestField("Buy-from Vendor No."); // NAVCZ
                WorkCenter.TestField("Subcontractor No.", "Buy-from Vendor No."); // NAVCZ
        ...
        field(99000754; "Prod. Order Line No."; Integer)
        {
            ...
            begin
                // NAVCZ
                if POLine.Get(POLine.Status::Released, "Prod. Order No.", "Prod. Order Line No.") then begin
                    TestField(Type, Type::Item);
                    TestField("No.");
                    POLine.TestField("Item No.", "No.");
    ...
    begin
        ...
        if not "System-Created Entry" and ("Document Type" in ["Document Type"::Order, "Document Type"::Invoice]) and
    ...
    begin
        ...
        if OldInvDiscAmtToInv <> "Inv. Disc. Amount to Invoice" then begin
            ...
            "Ext. VAT Difference (LCY)" := 0; // NAVCZ
        end;
    ...
    begin
        ItemCharge.Get("No.");
        ItemCharge.TestField("Sales Only", false); // NAVCZ
    ...
    var
        ...
        VATAmountNonDeductible: Decimal;
    ...
    begin
        ...
        VATAmountNonDeductible := "VAT Amount (Non Deductible)"; // NAVCZ
        "Recalculate Invoice Disc." := true;
        ...
        // NAVCZ
        if VATAmountNonDeductible <> "VAT Amount (Non Deductible)" then
            LineAmountChanged := true;
        // NAVCZ
                ...
                case "VAT Calculation Type" of
                    ...
                    "VAT Calculation Type"::"Reverse Charge VAT":
                        begin
                            ...
                            "VAT Amount (Non Deductible)" :=
                              Round(
                                ("Amount Including VAT" - "VAT Base Amount") * "VAT % (Non Deductible)" / 100,
                                Currency."Amount Rounding Precision");
                            // NAVCZ
                        end;
                    "VAT Calculation Type"::"Full VAT":
    ...
    begin
        ...
        case "FA Posting Type" of
            ...
            "FA Posting Type"::Maintenance:
                begin
                    // NAVCZ
                    if FASetup."FA Maintenance By Maint. Code" and ("Maintenance Code" <> '') then begin
                ...
                repeat
                    ...
                    then begin
                        ...
                        if VATAmountLine.Modified or VATAmountLine."Modified (LCY)" then begin // NAVCZ
                            ...
                            if QtyType <> QtyType::General then begin
                                if (VATAmountLine."Line Amount" - VATAmountLine."Invoice Discount Amount") = 0 then begin
                                    ...
                                    VATDifferenceLCY :=
                                      ...
                                      (VATAmountLine."Line Amount" - VATAmountLine."Invoice Discount Amount");
                                    ExtVATDifferenceLCY :=
                                      TempVATAmountLineRemainder."Ext. VAT Difference (LCY)" +
                                      VATAmountLine."Ext. VAT Difference (LCY)" * (LineAmountToInvoice - InvDiscAmount) /
                                      (VATAmountLine."Line Amount" - VATAmountLine."Invoice Discount Amount");
                                end;
                                if LineAmountToInvoice = 0 then begin
                                    ...
                                    "Ext. VAT Difference (LCY)" := Round(ExtVATDifferenceLCY, Currency."Amount Rounding Precision"); // NAVCZ
                                end;
                            end;
                            // NAVCZ
                            ...
                            // NAVCZ
                            TempVATAmountLineRemainder."VAT Difference (LCY)" := VATDifferenceLCY - "VAT Difference (LCY)";
                            TempVATAmountLineRemainder."Ext. VAT Difference (LCY)" := ExtVATDifferenceLCY - "Ext. VAT Difference (LCY)";
                            // NAVCZ
    ...
    begin
                ...
                repeat
                    if not ZeroAmountLine(QtyType) then begin
                        ...
                        then begin
                            ...
                            VATAmountLine."VAT % (Non Deductible)" := "VAT % (Non Deductible)";
                        ...
                        case QtyType of
                            QtyType::General:
                                begin
                                    ...
                                    VATAmountLine."Ext. VAT Difference (LCY)" += "Ext. VAT Difference (LCY)";
                                ...
                                begin
                                    ...
                                    if Type in [Type::"G/L Account", Type::Item, Type::"Fixed Asset"] then begin
                                        ...
                                        VATAmountLine."VAT Amount (Non Deductible)" +=
                                          Round("VAT Amount (Non Deductible)", Currency."Amount Rounding Precision");
                                    end;
                                    // NAVCZ
                                    OnCalcVATAmountLinesOnBeforeVATAmountLineSumLine(Rec, VATAmountLine, QtyType);
        ...
        VATAmountLine.UpdateLinesWithVATCorrection(
          Currency, PurchHeader."Prices Including VAT", PurchSetup."Allow VAT Difference", VATCorrection);

        if PurchSetup."Allow VAT Difference" and (PurchHeader."Currency Code" <> '') and not VATAmountLine.IsEmpty then
            UpdateLinesLCY(PurchHeader, TempPurchLine, QtyType, VATCorrection, VATAmountLine);
        // NAVCZ
    ...
    begin
        ...
        if ((Quantity - "Quantity Invoiced") <> 0) and (("Prepmt. Amt. Inv." - "Prepmt Amt Deducted") > 0) then begin
            ...
            if PurchHeader."Prepayment Type" = PurchHeader."Prepayment Type"::Advance then begin
                GrossAmount := Round("Direct Unit Cost" * Quantity, Currency."Amount Rounding Precision");
                "Prepmt Amt to Deduct" :=
                  Round(
                    Round(
                      Round(
    ...
    begin
        ...
        then begin
            ...
            PurchHeader.Get("Document Type", "Document No.");
            NonDeductVATSetup.SetRange("From Date", 0D, PurchHeader."VAT Date");
    ...
    begin
        ...
        with PurchLine do
            if (PurchLineQty = 0) or ("Direct Unit Cost" = 0) then begin
                ...
                "Line Amount" := GetLineAmountToHandle(PurchLineQty);
    ...
    local procedure RoundAmount(PurchLineQty: Decimal; PurchHeader: Record "Purchase Header"; var PurchLine: Record "Purchase Line")
    var
    ...
    begin
        // NAVCZ
        with PurchLine do begin
            IncrAmount(TotalPurchLine, PurchLine);

            if PurchHeader."Currency Code" <> '' then begin
                if PurchHeader."Posting Date" = 0D then
                    PurchHeader."Posting Date" := WorkDate;
                if PurchHeader."VAT Date" = 0D then
                    PurchHeader."VAT Date" := PurchHeader."Posting Date";
                ...
                "Ext.Amount Including VAT (LCY)" :=
                  Round(
                    CurrExchRate.ExchangeAmtFCYToLCY(
                      PurchHeader."VAT Date", PurchHeader."Currency Code",
                ...
                VATCoefficientRounded.RoundPurchaseLine(PurchLine, PurchHeader);
            end;

            IncrAmount(TotalPurchLineLCY, PurchLine);
            Increment(TotalPurchLineLCY."Unit Cost (LCY)", Round(PurchLineQty * "Unit Cost (LCY)"));
        end;
    end;

    local procedure IncrAmount(var TotalPurchLine: Record "Purchase Line"; PurchLine: Record "Purchase Line")
    begin
        // NAVCZ
        with PurchLine do begin
    ...
    local procedure UpdateLinesLCY(PurchHeader: Record "Purchase Header"; var TempPurchLine: Record "Purchase Line" temporary; QtyType: Option General,Invoicing,Shipping; VATCorrection: Boolean; var VATAmountLine: Record "VAT Amount Line")
    var
    ...
    begin
        ...
        TempPurchLine2.Copy(TempPurchLine, true);
        ...
        if TempPurchLine.FindSet then begin
            repeat
                case QtyType of
                    QtyType::General:
                        QtyFactor := TempPurchLine.Quantity;
                    QtyType::Invoicing:
                        QtyFactor := TempPurchLine."Qty. to Invoice";
                    QtyType::Shipping:
                        QtyFactor := TempPurchLine."Qty. to Receive";
                end;
                DivideAmount(PurchHeader, TempPurchLine, QtyType, QtyFactor, VATAmountLine, TempVATAmountLineRemainder);
                RoundAmount(QtyFactor, PurchHeader, TempPurchLine);

                VATAmountLine."Amount Including VAT (LCY)" += TempPurchLine."Amount Including VAT";
                VATAmountLine."VAT Base (LCY)" += TempPurchLine.Amount;
                ...
                VATAmountLine."Ext.Amount Including VAT (LCY)" += TempPurchLine."Ext.Amount Including VAT (LCY)";
                VATAmountLine."Ext. VAT Base (LCY)" += TempPurchLine."Ext. Amount (LCY)";
                VATAmountLine."Ext. VAT Amount (LCY)" :=
                  VATAmountLine."Ext.Amount Including VAT (LCY)" - VATAmountLine."Ext. VAT Base (LCY)";
                VATAmountLine."Ext. Calc. VAT Amount (LCY)" := VATAmountLine."Ext. VAT Amount (LCY)";
                VATAmountLine."Ext. VAT Amount (LCY)" += VATAmountLine."Ext. VAT Difference (LCY)";
                ...
                if not VATCorrection then begin
                    ...
                    TempPurchLine2.SetRange("Tax Group Code", TempPurchLine."Tax Group Code");
                    if QtyType = QtyType::Invoicing then
                        TempPurchLine2.SetFilter("Qty. to Invoice", '<>0');
                    TempPurchLine2.FindLast;
                    if TempPurchLine2."Line No." = TempPurchLine."Line No." then begin
                        ...
                        VATAmountLine."Ext. VAT Amount (LCY)" := VATAmountLine.RoundVAT(VATAmountLine."Ext. VAT Amount (LCY)");
                        VATAmountLine."Ext. VAT Difference (LCY)" :=
                          VATAmountLine."Ext. VAT Amount (LCY)" - VATAmountLine."Ext. Calc. VAT Amount (LCY)";
                        if PurchHeader."Currency Factor" <> PurchHeader."VAT Currency Factor" then
                            VATAmountLine."Letter VAT Difference (LCY)" :=
                              VATAmountLine."Ext. VAT Amount (LCY)" - VATAmountLine."Letter VAT Amount (LCY)"
                        else
                            VATAmountLine."Letter VAT Difference (LCY)" :=
                              VATAmountLine."VAT Amount (LCY)" - VATAmountLine."Letter VAT Amount (LCY)";
            ...
            VATAmountLine."Ext.Amount Including VAT (LCY)" := Round(
                VATAmountLine."Ext.Amount Including VAT (LCY)", Currency."Amount Rounding Precision");
            VATAmountLine."Ext. VAT Base (LCY)" := Round(
                VATAmountLine."Ext. VAT Base (LCY)", Currency."Amount Rounding Precision");
            VATAmountLine."Ext. Calc. VAT Amount (LCY)" := Round(
                VATAmountLine."Ext. Calc. VAT Amount (LCY)", Currency."Amount Rounding Precision");
            VATAmountLine."Ext. VAT Amount (LCY)" := VATAmountLine.RoundVAT(
                Round(VATAmountLine."Ext. VAT Amount (LCY)", Currency."Amount Rounding Precision"));
            if PurchHeader."Currency Factor" <> PurchHeader."VAT Currency Factor" then
                VATAmountLine."Letter VAT Difference (LCY)" :=
                  VATAmountLine."Ext. VAT Amount (LCY)" - VATAmountLine."Letter VAT Amount (LCY)"
            else
    ...
    var
        ...
        Text055: Label 'The quantity must be a whole number, because item %1 is set up for SN Specific Tracking.';
    begin
        // NAVCZ
    ...
    [Scope('OnPrem')]
    procedure xTestLeterLineExist() lboLinkExist: Boolean
    var
    ...
    [Scope('OnPrem')]
    procedure CalcLetterVATAmount(var PurchHeader: Record "Purchase Header"; var PurchLine: Record "Purchase Line"): Decimal
    var
        LetterRelation: Record "Advance Letter Line Relation";
        VATAmtLCY: Decimal;
    begin
        // NAVCZ
        LetterRelation.Reset();
        LetterRelation.SetRange(Type, LetterRelation.Type::Purchase);
        LetterRelation.SetRange("Document No.", PurchLine."Document No.");
        LetterRelation.SetRange("Document Line No.", PurchLine."Line No.");
        if LetterRelation.FindSet(false, false) then
            repeat
                if PurchHeader."Currency Code" <> '' then begin
                    if (PurchHeader."Currency Factor" <> PurchHeader."VAT Currency Factor") and (PurchHeader."VAT Currency Factor" <> 0) then
                        VATAmtLCY += Round(LetterRelation."VAT Doc. VAT Amount" /
                            PurchHeader."VAT Currency Factor", Currency."Amount Rounding Precision")
                    else
                        if PurchHeader."Currency Factor" <> 0 then
                            VATAmtLCY += Round(LetterRelation."VAT Doc. VAT Amount" /
                                PurchHeader."Currency Factor", Currency."Amount Rounding Precision");
                end else
                    VATAmtLCY += LetterRelation."VAT Doc. VAT Amount";
            until LetterRelation.Next = 0;
    ...
    begin
        if "Prepayment %" <> 0 then begin
            if not VendPostGr.Get(PurchHeader."Vendor Posting Group") then
                VendPostGr.Init();
            if not ((Type = Type::"G/L Account") and ("No." = VendPostGr."Invoice Rounding Account")) then begin
                if Quantity < 0 then
