            ...
            begin
                ...
                case Type of
                    ...
                    Type::"Fixed Asset":
                        CopyFromFixedAsset();
                    Type::"Charge (Item)":
                        CopyFromItemCharge(TempPurchLine);
                end;
                Validate("FA Charge No.");
...
#pragma warning restore
        ...
        field(12400; "FA Location Code"; Code[10])
        {
            ...
            begin
                if FALocation.ReadPermission then
                    if FALocation.Get("FA Location Code") then begin
                        "Location Code" := FALocation."Location Code";
                        "Employee No." := FALocation."Employee No.";
                    end;
            end;
        }
        field(12401; "Empl. Purchase Entry No."; Integer)
        {
            ...
            TableRelation = "Vendor Ledger Entry" WHERE(Open = CONST(true),
                                                         "Vendor No." = FIELD("Empl. Purchase Vendor No."));

            trigger OnValidate()
            var
                VendLedgEntry: Record "Vendor Ledger Entry";
            begin
                TestField(Type, Type::"Empl. Purchase");
                GetPurchHeader;
                ...
                if VendLedgEntry.Get("Empl. Purchase Entry No.") then begin
                    "Empl. Purchase Vendor No." := VendLedgEntry."Vendor No.";
                    Description := CopyStr(VendLedgEntry.Description + Text12400 + Format(VendLedgEntry."Document No."),
                                           1, MaxStrLen(Description));
                    "Description 2" := CopyStr(VendLedgEntry.Description + Text12400 + Format(VendLedgEntry."Document No."),
                                               MaxStrLen(Description) + 1, MaxStrLen("Description 2"));
                    "Empl. Purchase Document Date" := VendLedgEntry."Document Date";
                    "Empl. Purchase Document No." := VendLedgEntry."External Document No.";
                    "Agreement No." := VendLedgEntry."Agreement No.";
                    Validate(Quantity, 1);
                    VendLedgEntry.CalcFields("Remaining Amt. (LCY)", VendLedgEntry."Remaining Amount");
                    if PurchHeader."Currency Code" = '' then
                        Validate("Direct Unit Cost", -VendLedgEntry."Remaining Amt. (LCY)")
                    else
                        Validate("Direct Unit Cost", -VendLedgEntry."Remaining Amount");
                    "Dimension Set ID" := VendLedgEntry."Dimension Set ID";
                end else begin
                    Description := '';
                    "Description 2" := '';
                    "Empl. Purchase Document Date" := 0D;
                    "Empl. Purchase Document No." := '';
                end;
            end;
        }
        ...
        field(12403; "Empl. Purchase Vendor No."; Code[20])
        {
            ...
            TableRelation = IF (Type = FILTER("Empl. Purchase")) Vendor WHERE("Vendor Type" = CONST(Vendor));
            ...
            begin
                ...
                DimMgt.AddDimSource(DefaultDimSource, Database::Vendor, Rec."Empl. Purchase Vendor No.");
                DimMgt.AddDimSource(DefaultDimSource, Database::Job, Rec."Job No.");
                ...
                if "Empl. Purchase Vendor No." <> xRec."Empl. Purchase Vendor No." then
                    Validate("Empl. Purchase Entry No.", 0);
            end;
        }
        ...
        field(12486; "FA Charge No."; Code[20])
        {
            ...
            TableRelation = IF (Type = CONST("Fixed Asset")) "FA Charge";

            trigger OnValidate()
            var
                DefaultDimSource: List of [Dictionary of [Integer, Code[20]]];
            begin
                if "FA Charge No." <> '' then begin
                    FACharge.Get("FA Charge No.");
                    Validate("Gen. Prod. Posting Group", FACharge."Gen. Prod. Posting Group");
                    Validate("VAT Prod. Posting Group", FACharge."VAT Prod. Posting Group");
                    Validate("Tax Difference Code", FACharge."Tax Difference Code");

                    DimMgt.AddDimSource(DefaultDimSource, Database::"FA Charge", Rec."FA Charge No.");
                    DimMgt.AddDimSource(DefaultDimSource, DimMgt.PurchLineTypeToTableID(Type), Rec."No.");
                    DimMgt.AddDimSource(DefaultDimSource, Database::"Responsibility Center", Rec."Responsibility Center");
                    DimMgt.AddDimSource(DefaultDimSource, Database::"Work Center", Rec."Work Center No.");
                    CreateDim(DefaultDimSource);
                end;
            ...
            begin
                if "Tax Difference Code" <> xRec."Tax Difference Code" then
                    if "Tax Difference Code" <> '' then begin
                        ...
                        if "FA Charge No." <> '' then begin
                            FACharge.Get("FA Charge No.");
                            TestField("Tax Difference Code", FACharge."Tax Difference Code");
                        end;
                    end;
            end;
        }
    ...
    keys
    {
        ...
        key(Key6; "Document Type", "Pay-to Vendor No.", "Shortcut Dimension 1 Code", "Shortcut Dimension 2 Code", "Currency Code", "Agreement No.")
        {
            Enabled = false;
            MaintainSIFTIndex = false;
            MaintainSQLIndex = false;
    ...
    var
        ...
        Text12400: Label ' on Document No. ';
        Text12401: Label 'You must specify the field  Deprecation Book Code';
        Text12402: Label 'You cannot select type Empl. Purch. for currency advance report';
        Text12403: Label 'Custom Declaration %1 does not contain Item No. %2.';
    ...
    local procedure CopyFromGLAccount(var TempPurchLine: Record "Purchase Line" temporary)
    begin
        GLAcc.Get("No.");
        GLAcc.CheckGLAcc();
        ...
        if PurchHeader."Prepmt. Diff. Appln. Entry No." <> 0 then begin
            "Gen. Prod. Posting Group" := TempPurchLine."Gen. Prod. Posting Group";
            "VAT Prod. Posting Group" := TempPurchLine."VAT Prod. Posting Group";
        end;
    ...
    begin
        ...
        if GLSetup."Enable Russian Accounting" then
            if TaxRegisterSetup.Get() then
                Validate("Tax Difference Code", TaxRegisterSetup."Default FA TD Code");
        OnAfterAssignFixedAssetValues(Rec, FixedAsset, PurchHeader);
    end;

    local procedure CopyFromItemCharge(var TempPurchLine: Record "Purchase Line" temporary)
    begin
        ItemCharge.Get("No.");
    ...
    begin
        FixedAsset.Get("No.");
        "Depreciation Book Code" := FixedAsset.GetDefDeprBook;
    ...
    begin
        ...
        if GLSetup."Enable Russian Accounting" then begin
            if (Type <> Type::"Fixed Asset") and (Type <> Type::"Empl. Purchase") then
                Validate("VAT Prod. Posting Group")
            else
