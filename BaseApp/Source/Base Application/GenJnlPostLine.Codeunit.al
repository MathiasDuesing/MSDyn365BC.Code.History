codeunit 12 "Gen. Jnl.-Post Line"
{
    ...
    var
        ...
        TempSalesAdvLetterHeader: Record "Sales Advance Letter Header" temporary;
        TempPurchAdvLetterHeader: Record "Purch. Advance Letter Header" temporary;
        ...
        GLSetupRead: Boolean;
        PrepmtReverseApplication: Boolean;
        BalanceCheckAmount3: array[20] of Decimal;
        PostFromCashReg: Boolean;
        PostAdvInvAfterBatch: Boolean;
        FromAdjustment: Boolean;
        IsCustPosting: Boolean;
        AppType: Option Sale,Purchase;
        AdvLetterReverseErr: Label 'You cannot reverse the transaction, because it has a link between Prepayment and Advance Letter.';
        ...
        PostFromPurchDoc: Boolean;
        PostFromPurchAdvLetter: Boolean;
    ...
    begin
        ...
        with GenJnlLine do begin
            ...
            if CheckLine then begin
                ...
                GenJnlCheckLine.xSetCallFromAdjust(FromAdjustment); // NAVCZ
    ...
    begin
        IsHandled := false;
        OnBeforeInitVAT(GenJnlLine, GLEntry, VATPostingSetup, IsHandled);
        if IsHandled then
            exit;
        ...
        with GenJnlLine do
            if "Gen. Posting Type" <> "Gen. Posting Type"::" " then begin // None
                ...
                if not IsHandled then
                    if "Prepayment Type" <> "Prepayment Type"::Advance then // NAVCZ
                                ...
                                "VAT Calculation Type"::"Normal VAT":
                                    begin // NAVCZ
                                        if "VAT Difference" <> 0 then begin
                                            ...
                                            GLEntry."VAT Amount" := CalculatedVATAmountLCY(GenJnlLine); // NAVCZ
                                            ...
                                            // NAVCZ
                                            if CalculatedVATAmountLCY(GenJnlLine) <> "VAT Amount (LCY)" then begin
                                                GLEntry.Amount := "VAT Base Amount (LCY)";
                                                GLEntry."VAT Amount" := "VAT Amount (LCY)";
                                            end;
                                        end;
                                        AdjustNonDedVAT(VATPostingSetup, GenJnlLine, GLEntry, AddCurrGLEntryVATAmt);
                                        // NAVCZ
                                    end;
                                    ...
                                    case "Gen. Posting Type" of
                                        "Gen. Posting Type"::Purchase:
                                            begin // NAVCZ
                                                ...
                                                AdjustNonDedVAT(VATPostingSetup, GenJnlLine, GLEntry, AddCurrGLEntryVATAmt); // NAVCZ
                                            end;
                                    ...
                                    begin
                                        ...
                                        if not IsHandled then
                                            case "Gen. Posting Type" of
                                                "Gen. Posting Type"::Sale:
                                                    // NAVCZ
                                                    if ("Prepayment Type" = "Prepayment Type"::Advance) and Prepayment then begin
                                                        if "Amount (LCY)" <> 0 then
                                                            TestField("Advance VAT Base Amount");
                                                        VATPostingSetup.TestField("Sales Advance Offset VAT Acc.");
                                                        if "Bal. Account No." <> '' then
                                                            TestField("Bal. Account No.", VATPostingSetup."Sales Advance Offset VAT Acc.");
                                                    end else
                                                        // NAVCZ
                                                ...
                                                "Gen. Posting Type"::Purchase:
                                                    ...
                                                    if ("Prepayment Type" = "Prepayment Type"::Advance) and Prepayment then begin
                                                        ...
                                                        if "Bal. Account No." <> '' then
                                                            TestField("Bal. Account No.", VATPostingSetup."Purch. Advance Offset VAT Acc.");
                                                    end else
                                                        // NAVCZ
    ...
    begin
        ...
        with GenJnlLine do begin
            ...
            VATEntry."VAT Identifier" := VATPostingSetup."VAT Identifier"; // NAVCZ
            ...
            // NAVCZ
            if ("VAT Amount (LCY)" <> CalculatedVATAmountLCY(GenJnlLine)) and
               ("Currency Code" <> '')
            then
                VATDifferenceLCY := "VAT Amount (LCY)" - CalculatedVATAmountLCY(GenJnlLine);

            CompanyInfo.Get();
            if VATEntry."Perform. Country/Region Code" = CompanyInfo."Country/Region Code" then
                VATEntry."Perform. Country/Region Code" := '';
            // NAVCZ
            ...
            // NAVCZ
            if ("Gen. Posting Type" = "Gen. Posting Type"::Sale) and ("Document Type" = "Document Type"::"Credit Memo") then
                UnrealizedVAT := UnrealizedVAT or "Postponed VAT";
            // NAVCZ
            ...
            if "Gen. Posting Type" <> "Gen. Posting Type"::" " then begin
                ...
                // NAVCZ
                VATEntry."Advance Letter No." := "Advance Letter No.";
                VATEntry."Advance Letter Line No." := "Advance Letter Line No.";
                VATEntry."Advance Base" := "Advance VAT Base Amount";
                VATEntry."Advance Exch. Rate Difference" := "Advance Exch. Rate Difference";
                VATEntry."Prepayment Type" := "Prepayment Type";
                // NAVCZ
                ...
                // NAVCZ
                if ("VAT Difference (LCY)" <> 0) and ("Advance Letter No." <> '') then
                    VATEntry."VAT Difference" := "VAT Difference (LCY)";
                // NAVCZ
                ...
                // NAVCZ
                if VATEntry."VAT Amount (Non Deductible)" <> 0 then
                    if "Primary VAT Entry No." <> 0 then
                        VATEntry."Primary Entry No." := "Primary VAT Entry No."
                    else
                        VATEntry."Primary Entry No." := VATEntry."Entry No.";
                // NAVCZ
            ...
            if GLEntryVATAmountNotEmpty or
               ((SrcCurrGLEntryVATAmt <> 0) and (SrcCurrCode = AddCurrencyCode)) or
               (VATEntry."VAT Amount (Non Deductible)" <> 0) // NAVCZ
            then
                case "Gen. Posting Type" of
                    "Gen. Posting Type"::Purchase:
                        case VATPostingSetup."VAT Calculation Type" of
                          ...
                          VATPostingSetup."VAT Calculation Type"::"Full VAT":
                                // NAVCZ
                                if ("Prepayment Type" = "Prepayment Type"::Advance) and Prepayment and "System-Created Entry" then
                                    CreateGLEntry(GenJnlLine, "Account No.",
                        ...
                        GLEntryVATAmount, SrcCurrGLEntryVATAmt, true);
                            VATPostingSetup."VAT Calculation Type"::"Reverse Charge VAT":
                                // NAVCZ
                                if ("Prepayment Type" = "Prepayment Type"::Advance) and Prepayment and "System-Created Entry" then begin
                                    CreateGLEntry(GenJnlLine, "Account No.",
                                      GLEntryVATAmount, SrcCurrGLEntryVATAmt, true);
                                    CreateGLEntry(GenJnlLine, VATPostingSetup.GetRevChargeAccount(UnrealizedVAT),
                                      -GLEntryVATAmount, -SrcCurrGLEntryVATAmt, true);
                                end else begin
                                    if GLEntryVATAmount <> 0 then begin
                                        // NAVCZ
                                    ...
                                    // NAVCZ
                                    if "VAT Amount (Non Deductible)" <> 0 then begin
                                        if "Source Currency Code" = GLSetup."Additional Reporting Currency" then
                                            AddNonDedVATAmtACY := "VAT Amount (Non Deductible)"
                                        else
                                            AddNonDedVATAmtACY := CalcLCYToAddCurr("VAT Amount LCY (Non Deduct.)");
                                        // Non Deduct VAT
                                        if VATEntry.Base <> -VATEntry.Amount then
                                            CreateGLEntry(GenJnlLine, VATPostingSetup.GetRevChargeAccount(UnrealizedVAT),
                                              -GLEntryVATAmount - VATEntry."VAT Amount (Non Deductible)", -SrcCurrGLEntryVATAmt - AddNonDedVATAmtACY, true);
                                    end else begin
                                        // NAVCZ
                        ...
                        case VATPostingSetup."VAT Calculation Type" of
                          ...
                          VATPostingSetup."VAT Calculation Type"::"Full VAT":
                                ...
                                if ("Prepayment Type" = "Prepayment Type"::Advance) and Prepayment and "System-Created Entry" then
                                    ...
                                    if "Postponed VAT" then begin
                                        // GetSalesVATUnrealAccount("Postponed VAT")
                                        VATPostingSetup.TestField("Sales VAT Postponed Account");
                                        CreateGLEntry(GenJnlLine, VATPostingSetup."Sales VAT Postponed Account",
                                          GLEntryVATAmount, SrcCurrGLEntryVATAmt, true);
                                    end else
                                        // NAVCZ
    ...
    begin
        ...
        with GenJnlLine do begin
            ...
            if "Applies-to Doc. No." <> '' then begin
                ...
                GLEntry2.SetRange(Closed, false);
                GLEntry2.SetRange("Document No.", "Applies-to Doc. No.");
                GLEntry2.SetRange("Document Type", "Applies-to Doc. Type");
                if GLEntry.Amount < 0 then
                    GLEntry2.SetFilter(Amount, '>0')
                else
                    GLEntry2.SetFilter(Amount, '<0');
                if GLEntry2.FindSet(true) then
                    repeat
                        GLEntry2.CalcFields("Applied Amount");
                        GLEntry2."Amount to Apply" := GLEntry2.Amount - GLEntry2."Applied Amount";
                        if GLEntry2."Amount to Apply" <> 0 then
                            GLEntry2."Applies-to ID" := "Document No.";
                        GLEntry2.Modify();
                    until GLEntry2.Next = 0;

                GLEntry."Applies-to ID" := "Document No.";
                GLEntry."Amount to Apply" := GLEntry.Amount;
    ...
    begin
        ...
        with GenJnlLine do begin
            ...
            // NAVCZ
            ReceivablesAccount :=
              CustPostingGr.GetReceivablesAccNo("Posting Group", Prepayment and ("Prepayment Type" = "Prepayment Type"::Advance));
            ...
            then begin
                CustLedgEntry."Open For Advance Letter" := true;
                SalesPostAdvances.InsertInitialAdvanceLink(CustLedgEntry, GenJnlLine);
            end;
            // NAVCZ
            ...
            // NAVCZ
            TempDtldCVLedgEntryBuf.Advance :=
            ...
            // NAVCZ
            if not CVLedgEntryBuf.Open then
                CVLedgEntryBuf."Open For Advance Letter" := false;
            // NAVCZ
            ...
            SalesPostAdvances.AutoLinkPayment(CustLedgEntry, GenJnlLine); // NAVCZ
    ...
    begin
        ...
        with GenJnlLine do begin
            ...
            // NAVCZ
            PayablesAccount :=
                VendPostingGr.GetPayablesAccNo("Posting Group", Prepayment and ("Prepayment Type" = "Prepayment Type"::Advance));
            ...
            then begin
                VendLedgEntry."Open For Advance Letter" := true;
                PurchPostAdvances.InsertInitialAdvanceLink(VendLedgEntry, GenJnlLine);
            end;
            // NAVCZ
            ...
            PurchPostAdvances.AutoLinkPayment(VendLedgEntry, GenJnlLine); // NAVCZ
    ...
    begin
        ...
        IsBalCheckAmt3 := true;
        for Loop := 1 to 10 do
            IsBalCheckAmt3 := IsBalCheckAmt3 and (BalanceCheckAmount3[Loop] = 0);
        // NAVCZ
        IsTransactionConsistent :=
          IsBalCheckAmt3 and // NAVCZ
        ...
        if TempGLEntryBuf.FindSet() then begin
            repeat
                ...
                if GLSetup."User Checks Allowed" then
                    UserSetupAdvMgt.CheckFiscalYear(GlobalGLEntry);
                // NAVCZ
        ...
        if IsTransactionConsistent then begin
            ...
            if SalesSetup."Automatic Adv. Invoice Posting" then begin
                if not PostAdvInvAfterBatch then
                    SalesPostAdvances.AutoPostAdvanceInvoices
                else
                    SalesPostAdvances.GetLetterHeader(TempSalesAdvLetterHeader);
            end;
            if not PurchLetterTemplate.IsEmpty then begin
                PurchPostAdvances.GetLetterHeader(TempPurchAdvLetterHeader);

                if TempPurchAdvLetterHeader.Find('-') then begin
                    repeat
                        PostAutVAT := PurchSetup."Automatic Adv. Invoice Posting";
                        if TempPurchAdvLetterHeader."Template Code" <> '' then begin
                            if TempPurchAdvLetterHeader."Template Code" <> PurchLetterTemplate.Code then
                                PurchLetterTemplate.Get(TempPurchAdvLetterHeader."Template Code");
                            PostAutVAT := PurchLetterTemplate."Automatic Adv. Invoice Posting";
                        end;
                        if TempPurchAdvLetterHeader."Post Advance VAT Option" = TempPurchAdvLetterHeader."Post Advance VAT Option"::Never then
                            PostAutVAT := true;
                        if not PostAutVAT then
                            TempPurchAdvLetterHeader.Delete();
                    until TempPurchAdvLetterHeader.Next = 0;
                end;
                if not PostAdvInvAfterBatch then
                    if not TempPurchAdvLetterHeader.IsEmpty then begin
                        PurchPostAdvances.SetLetterHeader(TempPurchAdvLetterHeader);
                        PurchPostAdvances.AutoPostAdvanceInvoices;
                    end;
            end else begin
                PurchSetup.Get();
                if PurchSetup."Automatic Adv. Invoice Posting" then begin
                    if not PostAdvInvAfterBatch then
                        PurchPostAdvances.AutoPostAdvanceInvoices
                    else
                        PurchPostAdvances.GetLetterHeader(TempPurchAdvLetterHeader);
    ...
    begin
        with GLEntry do begin
            ...
            GLAcc.Get("G/L Account No."); // NAVCZ
            UpdateCheckAmounts(
              "Posting Date", Amount, "Additional-Currency Amount",
              BalanceCheckAmount, BalanceCheckAmount2, BalanceCheckAddCurrAmount, BalanceCheckAddCurrAmount2, GLAcc."G/L Account Group"); // NAVCZ

            UpdateDebitCredit(GenJnlLine.Correction);
            // NAVCZ
            GetGLSetup;
            if GLSetup."Check Posting Debit/Credit" then
                GLAcc.CheckDebitCredit(GLEntry);
            // NAVCZ
        end;
    ...
    local procedure UpdateCheckAmounts(PostingDate: Date; Amount: Decimal; AddCurrAmount: Decimal; var BalanceCheckAmount: Decimal; var BalanceCheckAmount2: Decimal; var BalanceCheckAddCurrAmount: Decimal; var BalanceCheckAddCurrAmount2: Decimal; GLAccountGroup: Integer)
    begin
        ...
        if GLAccountGroup <> 0 then
            if PostingDate = NormalDate(PostingDate) then begin
                BalanceCheckAmount3[GLAccountGroup] :=
                  BalanceCheckAmount3[GLAccountGroup] + Amount * ((PostingDate - 00000101D) mod 99 + 1);
                BalanceCheckAmount3[GLAccountGroup + 10] := BalanceCheckAmount3[GLAccountGroup + 10] +
                  Amount * ((PostingDate - 00000101D) mod 98 + 1);
            end else begin
                BalanceCheckAmount3[GLAccountGroup] :=
                  BalanceCheckAmount3[GLAccountGroup] +
                  Amount * ((NormalDate(PostingDate) - 00000101D + 50) mod 99 + 1);
                BalanceCheckAmount3[GLAccountGroup + 10] :=
                  BalanceCheckAmount3[GLAccountGroup + 10] +
    ...
    begin
        ...
        VATEntry2.SetRange("VAT Delay", false); // NAVCZ
        ...
        if VATEntry2.FindSet() then begin
            ...
            repeat
                ...
                if VATPostingSetup."Adjust for Payment Discount" then begin
                    ...
                    if AllowTaxCorrDoc then // NAVCZ
                        TaxCorrDocMgt.CreateDocLine(TempVATEntry, OldCVLedgEntryBuf."Document No.");
                end;
            ...
            if LastConnectionNo <> 0 then begin
                ...
                InsertSummarizedVAT(GenJnlLine);
                // NAVCZ
                if AllowTaxCorrDoc then begin
                    TaxCorrDocMgt.CreateDocHeader(DtldCVLedgEntryBuf, OldCVLedgEntryBuf, NewCVLedgEntryBuf."Document No.");
                    TaxCorrDocMgt.UpdateVATEntry(TempVATEntry);
                end;
                // NAVCZ
            end;
        end;
    ...
    begin
        ...
        TempVATEntry."Original Document VAT Date" := GenJnlLine."Original Document VAT Date";
        // NAVCZ
    ...
    var
        OldDtldCVLedgEntryBuf: Record "Detailed CV Ledg. Entry Buffer";
        IsHandled: Boolean;
    begin
        IsHandled := false;
        ...
        if not OldCVLedgEntryBuf.Open then begin
            ...
            OldCVLedgEntryBuf."Open For Advance Letter" := false; // NAVCZ
        end else
            AllApplied := false;

        OldDtldCVLedgEntryBuf := DtldCVLedgEntryBuf; // NAVCZ
        ...
        then
            ...
            then
                if DtldCVLedgEntryBuf."Document Type" = DtldCVLedgEntryBuf."Document Type"::Payment then begin
                    if AppType = AppType::Purchase then
                    ...
                    if AppType = AppType::Purchase then
                        PurchPostAdvances.InsertTempApplnAdvanceLink(OldDtldCVLedgEntryBuf."Entry No.")
                    else
        ...
        if not NewCVLedgEntryBuf.Open and not AllApplied then begin
            ...
            NewCVLedgEntryBuf."Open For Advance Letter" := false; // NAVCZ
        end;
    end;
    ...
    begin
        with DtldCustLedgEntry do begin
            ...
            CustLedgEntry2.Get(DtldCVLedgEntryBuf."CV Ledger Entry No."); // NAVCZ
            "Customer Posting Group" := CustLedgEntry2."Customer Posting Group"; // NAVCZ
            UpdateDebitCredit(GenJnlLine.Correction);
    ...
    begin
        with DtldVendLedgEntry do begin
            ...
            "Source Code" := GenJnlLine."Source Code";
            "Transaction No." := NextTransactionNo;
            VendorLedgEntry2.Get(DtldCVLedgEntryBuf."CV Ledger Entry No."); // NAVCZ
            "Vendor Posting Group" := VendorLedgEntry2."Vendor Posting Group"; // NAVCZ
    ...
    var
        ...
        CreditSetup: Record "Credits Setup";
        Completed: Boolean;
    ...
    begin
        ...
        IsCustPosting := true; // NAVCZ
        Clear(TaxCorrDocMgt); // NAVCZ
        ...
        repeat
            ...
            // NAVCZ
            if CreditSetup.Get then begin
                TempOldCustLedgEntry.CalcFields("Amount on Credit (LCY)");
                TempOldCustLedgEntry.TestField("Amount on Credit (LCY)", 0);
            end;
            // NAVCZ
            ...
            // NAVCZ
            if GenJnlLine.Compensation then
        ...
        // NAVCZ
        if TaxCorrDocMgt.GetLastInsertedCrMemoNo <> '' then begin
            DtldCVLedgEntryBuf.SetCurrentKey("CV Ledger Entry No.", "Entry Type");
            DtldCVLedgEntryBuf.SetFilter("Entry Type", '%1|%2',
              DtldCVLedgEntryBuf."Entry Type"::"Payment Discount",
              DtldCVLedgEntryBuf."Entry Type"::"Payment Discount Tolerance");
            if DtldCVLedgEntryBuf."Currency Code" = '' then begin
                DtldCVLedgEntryBuf.CalcSums(Amount);
                RoundingAmt := -DtldCVLedgEntryBuf.Amount - TaxCorrDocMgt.GetInsertedAmountTotal;
            end else begin
                DtldCVLedgEntryBuf.FindFirst;
                DtldCVLedgEntryBuf."Amount (LCY)" :=
                  Round(DtldCVLedgEntryBuf.Amount / NewCVLedgEntryBuf."Original Currency Factor");
                RoundingAmt := -DtldCVLedgEntryBuf."Amount (LCY)" - TaxCorrDocMgt.GetInsertedAmountTotal;
            end;

            if RoundingAmt <> 0 then
                TaxCorrDocMgt.CreateDocRounding(TaxCorrDocMgt.GetLastInsertedCrMemoNo, RoundingAmt);
        end;
        // NAVCZ
    ...
    var
        ...
        SaveEntryNo: Integer;
        MultiplePostingGrApplied: Boolean;
    begin
        if GenJnlLine."Account Type" <> GenJnlLine."Account Type"::Customer then
            exit;
        ...
        MultiplePostingGrApplied := CheckMultiplePostingGr(DtldCVLedgEntryBuf, true); // NAVCZ
        ...
        if DtldCVLedgEntryBuf.FindSet() then begin
            ...
            repeat
                ...
                then
                    PostDtldCustLedgEntry(GenJnlLine, DtldCVLedgEntryBuf, CustPostingGr, AdjAmount, MultiplePostingGrApplied); // NAVCZ
        ...
        CreateGLEntriesForTotalAmounts(
          GenJnlLine, TempInvPostBuf, AdjAmount, SaveEntryNo,
          CustPostingGr.GetReceivablesAccNo(
            GenJnlLine."Posting Group",
            GenJnlLine.Prepayment and (GenJnlLine."Prepayment Type" = GenJnlLine."Prepayment Type"::Advance)),
          LedgEntryInserted);
        ...
        SalesPostAdvances.DeleteTempApplnAdvanceLink; // NAVCZ
    end;

    local procedure PostDtldCustLedgEntry(GenJnlLine: Record "Gen. Journal Line"; DtldCVLedgEntryBuf: Record "Detailed CV Ledg. Entry Buffer"; CustPostingGr: Record "Customer Posting Group"; var AdjAmount: array[4] of Decimal; MultiplePostingGrApplied: Boolean)
    var
        AccNo: Code[20];
    begin
        // NAVCZ
        if MultiplePostingGrApplied and
           (DtldCVLedgEntryBuf."Entry Type" = DtldCVLedgEntryBuf."Entry Type"::Application)
        then
            AccNo := GetReceivablesAccNo(DtldCVLedgEntryBuf."CV Ledger Entry No.")
        else
            // NAVCZ
            AccNo := GetDtldCustLedgEntryAccNo(GenJnlLine, DtldCVLedgEntryBuf, CustPostingGr, 0, false);
        // NAVCZ
        PostDtldCVLedgEntry(GenJnlLine, DtldCVLedgEntryBuf, AccNo, AdjAmount, false, MultiplePostingGrApplied);
        SalesPostAdvances.InsertApplnAdvanceLink(DtldCVLedgEntryBuf);
        // NAVCZ
    end;

    local procedure PostDtldCustLedgEntryUnapply(GenJnlLine: Record "Gen. Journal Line"; DtldCVLedgEntryBuf: Record "Detailed CV Ledg. Entry Buffer"; CustPostingGr: Record "Customer Posting Group"; OriginalTransactionNo: Integer; MultiplePostingGrApplied: Boolean)
    var
        AdjAmount: array[4] of Decimal;
        AccNo: Code[20];
    begin
        ...
        PostDtldCVLedgEntry(GenJnlLine, DtldCVLedgEntryBuf, AccNo, AdjAmount, true, MultiplePostingGrApplied); // NAVCZ
    end;
    ...
    begin
        ...
        if VATEntry2.FindSet() then begin
            ...
            repeat
                ...
                GenJnlCheckLine.CheckVATDate(GenJnlLine); // NAVCZ
                ...
                VATPart :=
                  VATEntry2.GetUnrealizedVATPart(
                    ...
                    TotalUnrealVATAmountLast, GenJnlLine."Postponed VAT"); // NAVCZ
                        ...
                        VATEntry2."VAT Calculation Type"::"Full VAT":
                            begin
                                ...
                                if GenJnlLine."Postponed VAT" then begin
                                    VATPostingSetup.TestField("Sales VAT Postponed Account");
                                    SalesVATUnrealAccount := VATPostingSetup."Sales VAT Postponed Account";
                                end else
                                    // NAVCZ
    ...
    begin
        ...
        repeat
            ...
            if CreditsSetup.Get then begin
                TempOldVendLedgEntry.CalcFields("Amount on Credit (LCY)");
                TempOldVendLedgEntry.TestField("Amount on Credit (LCY)", 0);
            end;
            // NAVCZ
            ...
            AppType := AppType::Purchase; // NAVCZ
    ...
    begin
        ...
        MultiplePostingGrApplied := CheckMultiplePostingGr(DtldCVLedgEntryBuf, false); // NAVCZ
        ...
        if DtldCVLedgEntryBuf.FindSet() then begin
            ...
            repeat
                ...
                if ((DtldCVLedgEntryBuf."Amount (LCY)" <> 0) or
                   ...
                   ((AddCurrencyCode <> '') and (DtldCVLedgEntryBuf."Additional-Currency Amount" <> 0))
                then
                    PostDtldVendLedgEntry(GenJnlLine, DtldCVLedgEntryBuf, VendPostingGr, AdjAmount, MultiplePostingGrApplied); // NAVCZ
            until DtldCVLedgEntryBuf.Next() = 0;
        end;
        ...
        CreateGLEntriesForTotalAmounts(
            GenJnlLine, TempInvPostBuf, AdjAmount, SaveEntryNo,
            VendPostingGr.GetPayablesAccNo(
                GenJnlLine."Posting Group",
                GenJnlLine.Prepayment and (GenJnlLine."Prepayment Type" = GenJnlLine."Prepayment Type"::Advance)),
            LedgEntryInserted);
        ...
        DtldCVLedgEntryBuf.DeleteAll();
        // NAVCZ
        PurchPostAdvances.DeleteTempApplnAdvanceLink;
        // NAVCZ
    end;

    local procedure PostDtldVendLedgEntry(GenJnlLine: Record "Gen. Journal Line"; DtldCVLedgEntryBuf: Record "Detailed CV Ledg. Entry Buffer"; VendPostingGr: Record "Vendor Posting Group"; var AdjAmount: array[4] of Decimal; MultiplePostingGrApplied: Boolean)
    var
        AccNo: Code[20];
    begin
        // NAVCZ
        if MultiplePostingGrApplied and
           (DtldCVLedgEntryBuf."Entry Type" = DtldCVLedgEntryBuf."Entry Type"::Application)
        then
            AccNo := GetPayablesAccNo(DtldCVLedgEntryBuf."CV Ledger Entry No.")
        else
            // NAVCZ
    ...
    local procedure PostDtldVendLedgEntryUnapply(GenJnlLine: Record "Gen. Journal Line"; DtldCVLedgEntryBuf: Record "Detailed CV Ledg. Entry Buffer"; VendPostingGr: Record "Vendor Posting Group"; OriginalTransactionNo: Integer; MultiplePostingGrApplied: Boolean)
    var
        AccNo: Code[20];
        AdjAmount: array[4] of Decimal;
    begin
    ...
    local procedure PostDtldCVLedgEntry(GenJnlLine: Record "Gen. Journal Line"; DtldCVLedgEntryBuf: Record "Detailed CV Ledg. Entry Buffer"; AccNo: Code[20]; var AdjAmount: array[4] of Decimal; Unapply: Boolean; MultiplePostingGrApplied: Boolean)
    var
        IsHandled: Boolean;
    begin
        ...
        with DtldCVLedgEntryBuf do
            case "Entry Type" of
                ...
                "Entry Type"::Application:
                    // NAVCZ
                    begin
                        if PrepmtReverseApplication then
                            PostCustPrepmtReverse(GenJnlLine, DtldCVLedgEntryBuf);
                        if MultiplePostingGrApplied then
            ...
            repeat
                ...
                VATPart :=
                  VATEntry2.GetUnrealizedVATPart(
                    ...
                    TotalUnrealVATAmountFirst,
                    TotalUnrealVATAmountLast, false); // NAVCZ
    ...
    begin
        ...
        // NAVCZ
        VATEntry."VAT Date" := GenJnlLine."VAT Date";
        if VATEntry."VAT Date" = 0D then
            VATEntry."VAT Date" := GenJnlLine."Posting Date";
        VATEntry."Original Document VAT Date" := GenJnlLine."Original Document VAT Date";
        VATEntry."VAT Settlement No." := '';
        // NAVCZ
    ...
    begin
            ...
            if GenJnlLine."Prepmt. Appl. Transaction No." <> 0 then
                if GenJnlLine."Prepmt. Appl. Transaction No." < DtldCustLedgEntry."Transaction No." then
                    DtldCustLedgEntry2.SetRange(
                      "Transaction No.", GenJnlLine."Prepmt. Appl. Transaction No.", DtldCustLedgEntry."Transaction No.")
                else
                    DtldCustLedgEntry2.SetRange(
                      "Transaction No.", DtldCustLedgEntry."Transaction No.", GenJnlLine."Prepmt. Appl. Transaction No.")
            else
                // NAVCZ
        ...
        repeat
            if DtldCustLedgEntry2."Customer Posting Group" <> DtldCustLedgEntry."Customer Posting Group" then
                MultiplePostingGrApplied := true;
        ...
        repeat
            ...
            // NAVCZ
            if CustPostingGr.Code <> DtldCustLedgEntry2."Customer Posting Group" then
                CustPostingGr.Get(DtldCustLedgEntry2."Customer Posting Group");
            // NAVCZ
            ...
            PostDtldCustLedgEntryUnapply(
              GenJnlLine, DtldCVLedgEntryBuf, CustPostingGr, DtldCustLedgEntry2."Transaction No.", MultiplePostingGrApplied); // NAVCZ
            ...
            if DtldCustLedgEntry2."Entry Type" = DtldCustLedgEntry2."Entry Type"::Application then begin
                ...
                then
                    TaxCorrDocMgt.Init(CustLedgEntry2."Document No.");
            end;
            if DtldCustLedgEntry2."Entry Type" = DtldCustLedgEntry2."Entry Type"::"Payment Discount (VAT Excl.)" then
                if not UnappliedPmtDiscVAT then
                    UnappliedPmtDiscVAT := TaxCorrDocMgt.CreateDocForUnapply(NewDtldCustLedgEntry, DtldCustLedgEntry2."Document No.");
        // NAVCZ
        until DtldCustLedgEntry2.Next() = 0;

        // NAVCZ
        OnBeforeCreateGLEntriesForTotalAmountsUnapply(DtldCustLedgEntry, CustPostingGr, GenJnlLine, TempInvPostBuf);
        CreateGLEntriesForTotalAmountsUnapply(GenJnlLine, TempInvPostBuf, GetReceivablesAccNo(DtldCustLedgEntry."Cust. Ledger Entry No."));
    ...
    begin
        ...
        // NAVCZ
        DtldVendLedgEntry2.SetRange("Entry Type", DtldVendLedgEntry."Entry Type"::Application);
        DtldVendLedgEntry2.FindSet;
        repeat
            if DtldVendLedgEntry2."Vendor Posting Group" <> DtldVendLedgEntry."Vendor Posting Group" then
                MultiplePostingGrApplied := true;
        ...
        repeat
            ...
            // NAVCZ
            if VendPostingGr.Code <> DtldVendLedgEntry2."Vendor Posting Group" then
                VendPostingGr.Get(DtldVendLedgEntry2."Vendor Posting Group");
            // NAVCZ
            ...
            PostDtldVendLedgEntryUnapply(
              GenJnlLine, DtldCVLedgEntryBuf, VendPostingGr, DtldVendLedgEntry2."Transaction No.", MultiplePostingGrApplied); // NAVCZ
        ...
        CreateGLEntriesForTotalAmountsUnapply(GenJnlLine, TempInvPostBuf, GetPayablesAccNo(DtldVendLedgEntry."Vendor Ledger Entry No."));
    ...
    begin
        ...
        CreateGLEntry(
          GenJnlLine, GetPostingAccountNo(VATPostingSetup, VATEntry, true, GenJnlLine."Postponed VAT"), VATEntry.Amount, AmountAddCurr, false);
        GLEntryNoFromVAT :=
          CreateGLEntryFromVATEntry(
            GenJnlLine, GetPostingAccountNo(VATPostingSetup, VATEntry, false, GenJnlLine."Postponed VAT"), -VATEntry.Amount, -AmountAddCurr,
            VATEntry);
    ...
    begin
        // NAVCZ
        if PostFromPurchDoc or PostFromPurchAdvLetter then
            exit;
        // NAVCZ
        ...
        case GenJnlLine."Document Type" of
            GenJnlLine."Document Type"::Invoice:
                begin
                    ...
                    PurchInvHeader.SetFilter("No.", '<>%1', GenJnlLine."Document No.");
            ...
            GenJnlLine."Document Type"::"Credit Memo":
                begin
                    ...
                    PurchCrMemoHdr.SetFilter("No.", '<>%1', GenJnlLine."Document No.");
    ...
    begin
        ...
        exit(CustPostingGr.GetReceivablesAccNo(
    ...
    begin
        ...
        exit(VendPostingGr.GetPayablesAccNo(
    ...
    [Scope('OnPrem')]
    procedure ApplyCustPrepmtRefunds(var GenJnlLineApply: Record "Gen. Journal Line"; var CustLedgEntryForApply: Record "Cust. Ledger Entry")
    var
        SourceCodeSetup: Record "Source Code Setup";
        CustLedgEntry: Record "Cust. Ledger Entry";
    begin
        // NAVCZ
        if CustLedgEntryForApply.IsEmpty then
            exit;

        with CustLedgEntryForApply do begin
            FindSet;
            repeat
            ...
            GenJnlLineApply."Document Date" := GenJnlLineApply."Posting Date";
            GenJnlLineApply."VAT Date" := GenJnlLineApply."Posting Date";
            GenJnlLineApply."Account Type" := GenJnlLineApply."Account Type"::Customer;
            GenJnlLineApply."Account No." := "Customer No.";
            CalcFields("Debit Amount", "Credit Amount", "Debit Amount (LCY)", "Credit Amount (LCY)");
            GenJnlLineApply.Correction :=
              ("Debit Amount" < 0) or ("Credit Amount" < 0) or
              ("Debit Amount (LCY)" < 0) or ("Credit Amount (LCY)" < 0);
            GenJnlLineApply."Document Type" := "Document Type";
            GenJnlLineApply.Description := Description;
            GenJnlLineApply."Shortcut Dimension 1 Code" := "Global Dimension 1 Code";
            GenJnlLineApply."Shortcut Dimension 2 Code" := "Global Dimension 2 Code";
            GenJnlLineApply."Posting Group" := "Customer Posting Group";
            GenJnlLineApply."Source Type" := GenJnlLineApply."Source Type"::Customer;
            GenJnlLineApply."Source No." := "Customer No.";
            SourceCodeSetup.Get();
            GenJnlLineApply."Source Code" := SourceCodeSetup."Unapplied Sales Entry Appln.";
            GenJnlLineApply."System-Created Entry" := true;

            PrepmtReverseApplication := true;
            CustPostApplyCustLedgEntry(GenJnlLineApply, CustLedgEntry);
            PrepmtReverseApplication := false;
        end;
    end;

    [Scope('OnPrem')]
    procedure ApplyVendPrepmtRefunds(var GenJnlLineApply: Record "Gen. Journal Line"; var VendLedgEntryForApply: Record "Vendor Ledger Entry")
    var
        SourceCodeSetup: Record "Source Code Setup";
        VendLedgEntry: Record "Vendor Ledger Entry";
    begin
        // NAVCZ
        if VendLedgEntryForApply.IsEmpty then
            exit;

        with VendLedgEntryForApply do begin
            FindSet;
            repeat
            ...
            GenJnlLineApply."Document Date" := GenJnlLineApply."Posting Date";
            GenJnlLineApply."VAT Date" := GenJnlLineApply."Posting Date";
            GenJnlLineApply."Account Type" := GenJnlLineApply."Account Type"::Vendor;
            GenJnlLineApply."Account No." := "Vendor No.";
            CalcFields("Debit Amount", "Credit Amount", "Debit Amount (LCY)", "Credit Amount (LCY)");
            GenJnlLineApply.Correction :=
              ("Debit Amount" < 0) or ("Credit Amount" < 0) or
              ("Debit Amount (LCY)" < 0) or ("Credit Amount (LCY)" < 0);
            ...
            GenJnlLineApply."Shortcut Dimension 1 Code" := "Global Dimension 1 Code";
            GenJnlLineApply."Shortcut Dimension 2 Code" := "Global Dimension 2 Code";
            GenJnlLineApply."Posting Group" := "Vendor Posting Group";
            GenJnlLineApply."Source Type" := GenJnlLineApply."Source Type"::Vendor;
            GenJnlLineApply."Source No." := "Vendor No.";
            GenJnlLineApply."Source Code" := SourceCodeSetup."Purchase Entry Application";
            SourceCodeSetup.Get();
            GenJnlLineApply."Source Code" := SourceCodeSetup."Unapplied Purch. Entry Appln.";
            GenJnlLineApply."System-Created Entry" := true;

            PrepmtReverseApplication := true;
            VendPostApplyVendLedgEntry(GenJnlLineApply, VendLedgEntry);
            PrepmtReverseApplication := false;
        end;
    end;

    [Scope('OnPrem')]
    procedure PostCustPrepmtReverse(GenJnlLine: Record "Gen. Journal Line"; DtldCVLedgEntryBuf: Record "Detailed CV Ledg. Entry Buffer")
    var
        CustLedgEntry: Record "Cust. Ledger Entry";
    begin
        // NAVCZ
        ...
        GenJnlLine."Document Type" := CustLedgEntry."Document Type";
        CreateGLEntry(
          GenJnlLine, GetReceivablesAccNo(DtldCVLedgEntryBuf."CV Ledger Entry No."), DtldCVLedgEntryBuf."Amount (LCY)", 0, false);
    end;

    [Scope('OnPrem')]
    procedure PostVendPrepmtReverse(GenJnlLine: Record "Gen. Journal Line"; DtldCVLedgEntryBuf: Record "Detailed CV Ledg. Entry Buffer")
    var
        VendLedgEntry: Record "Vendor Ledger Entry";
    begin
        // NAVCZ
        ...
        CreateGLEntry(GenJnlLine, GetPayablesAccNo(DtldCVLedgEntryBuf."CV Ledger Entry No."), DtldCVLedgEntryBuf."Amount (LCY)", 0, false);
    end;

    [Scope('OnPrem')]
    procedure PrepareAdvCustUnapply(CustLedgerEntryNo: Integer; var CustLedgEntryForAppln: Record "Cust. Ledger Entry"; var UnapplyAdvVATEntries: Boolean; var ReverseAdvVATTransNo: Integer; var ApplnCustLedgEntryNo: Integer; var CurrencyCode: Code[10])
    var
        CustLedgEntry: Record "Cust. Ledger Entry";
    begin
        // NAVCZ
        ...
        if CustLedgEntry."Document Type" = CustLedgEntry."Document Type"::Invoice then begin
            ReverseAdvVATTransNo := CustLedgEntry."Transaction No.";
            CurrencyCode := CustLedgEntry."Currency Code";
        end;
    end;

    [Scope('OnPrem')]
    procedure PrepareAdvVendUnapply(VendLedgerEntryNo: Integer; var VendLedgEntryForAppln: Record "Vendor Ledger Entry"; var UnapplyAdvVATEntries: Boolean; var ReverseAdvVATTransNo: Integer; var ApplnVendLedgEntryNo: Integer; var CurrencyCode: Code[10])
    var
        VendLedgEntry: Record "Vendor Ledger Entry";
    begin
        // NAVCZ
        VendLedgEntry.Get(VendLedgerEntryNo);
        if VendLedgEntry."Prepayment Type" = VendLedgEntry."Prepayment Type"::Advance then
            exit;
        if VendLedgEntry."Document Type" = VendLedgEntry."Document Type"::Invoice then begin
            ReverseAdvVATTransNo := VendLedgEntry."Transaction No.";
    ...
    begin
        ...
        CustUnrealizedVAT(GenJnlLine, CustLedgEntry2, GenJnlLine.Amount);
    ...
    begin
        ...
        UnrealVATEntry.SetRange("Postponed VAT", true);
                ...
                if VATEntry.FindSet then
                    repeat
                        ...
                        with NewVATEntry do begin
                            ...
                            if GenJnlLine."VAT Date" <> 0D then
                                "VAT Date" := GenJnlLine."VAT Date";
                            if GenJnlLine."Original Document VAT Date" <> 0D then
                                "Original Document VAT Date" := GenJnlLine."Original Document VAT Date";
                        ...
                        CreateGLEntry(GenJnlLine, VATPostingSetup."Sales VAT Account",
                          -VATEntry.Amount, 0, false);
    ...
    begin
        // NAVCZ
        with GenJnlLine do begin
            InitNonDeductibleVAT(GenJnlLine);
            if VATPostingSetup."Allow Non Deductible VAT" then
                SetNonDeductibleVAT(GenJnlLine);

            if ("VAT % (Non Deductible)" = 0) and ("Primary VAT Entry No." = 0) then
                exit;
            TestField("Gen. Posting Type", "Gen. Posting Type"::Purchase);
            GLEntry.Amount := GLEntry.Amount + "VAT Amount LCY (Non Deduct.)";
            GLEntry."VAT Amount" := GLEntry."VAT Amount" - "VAT Amount LCY (Non Deduct.)";
            "VAT Base Amount (LCY)" := GLEntry.Amount;

            if "Source Currency Code" = GLSetup."Additional Reporting Currency" then
                NonDedVATAmtACY := "VAT Amount (Non Deductible)"
            else
                NonDedVATAmtACY := CalcLCYToAddCurr("VAT Amount LCY (Non Deduct.)");
            GLEntry."Additional-Currency Amount" := GLEntry."Additional-Currency Amount" + NonDedVATAmtACY;
            AddCurrGLEntryVATAmt := AddCurrGLEntryVATAmt - NonDedVATAmtACY;
            "Source Curr. VAT Base Amount" := GLEntry."Additional-Currency Amount";
        end;
    end;

    [Scope('OnPrem')]
    procedure CalculatedVATAmountLCY(GenJnlLine: Record "Gen. Journal Line"): Decimal
    var
        Currency: Record Currency;
    ...
    [Scope('OnPrem')]
    procedure CheckAdvanceLink(EntryNo: Integer)
    var
    ...
    begin
        ...
        if not AdvanceLink.IsEmpty then
    ...
    begin
        // NAVCZ
        PostFromCashReg := PostFromCashRegNew;
    end;

    procedure xGetSalesLetterHeader(var TempSalesAdvLetterHeader2: Record "Sales Advance Letter Header" temporary)
    begin
        // NAVCZ
        TempSalesAdvLetterHeader.Reset();
        if TempSalesAdvLetterHeader.Find('-') then
            repeat
                TempSalesAdvLetterHeader2 := TempSalesAdvLetterHeader;
                TempSalesAdvLetterHeader2.Insert();
            until TempSalesAdvLetterHeader.Next = 0;
    end;

    procedure xGetPurchLetterHeader(var TempPurchAdvLetterHeader2: Record "Purch. Advance Letter Header" temporary)
    begin
        // NAVCZ
        TempPurchAdvLetterHeader.Reset();
        if TempPurchAdvLetterHeader.Find('-') then
            repeat
                TempPurchAdvLetterHeader2 := TempPurchAdvLetterHeader;
                TempPurchAdvLetterHeader2.Insert();
            until TempPurchAdvLetterHeader.Next = 0;
    end;

    procedure SetPostAdvInvAfterBatch(PostAdvInvAfterBatchNew: Boolean)
    begin
        // NAVCZ
        PostAdvInvAfterBatch := PostAdvInvAfterBatchNew;
    end;

    [Scope('OnPrem')]
    procedure xSetCallFromAdjust(FromAdjustmentNew: Boolean)
    begin
        // NAVCZ
        FromAdjustment := FromAdjustmentNew;
    end;

    [Scope('OnPrem')]
    procedure SetFAHistory(GenJnlLine: Record "Gen. Journal Line")
    var
    ...
    begin
        ...
        if FASetup."Fixed Asset History" and
           (GenJnlLine."FA Posting Type" = GenJnlLine."FA Posting Type"::Disposal) and
           (FASetup."Default Depr. Book" = GenJnlLine."Depreciation Book Code")
        then
            if GenJnlLine."FA Error Entry No." = 0 then begin
                FAJnlPostLine.InsertFAHistoryEntry(0, GenJnlLine."Account No.");
                FAJnlPostLine.InsertFAHistoryEntry(1, GenJnlLine."Account No.");
            end else begin
                FAJnlPostLine.UpdateFAHistoryEntry(0, GenJnlLine."Account No.");
                FAJnlPostLine.UpdateFAHistoryEntry(1, GenJnlLine."Account No.");
            end;
    end;
    ...
    begin
        // NAVCZ
        with GenJnlLine do begin
            if (TempGenJournalLine."VAT % (Non Deductible)" <> "VAT % (Non Deductible)") or
               (TempGenJournalLine."VAT Base (Non Deductible)" <> "VAT Base (Non Deductible)") or
               (TempGenJournalLine."VAT Amount (Non Deductible)" <> "VAT Amount (Non Deductible)") or
               (TempGenJournalLine."VAT Base LCY (Non Deduct.)" <> "VAT Base LCY (Non Deduct.)") or
               (TempGenJournalLine."VAT Amount LCY (Non Deduct.)" <> "VAT Amount LCY (Non Deduct.)")
            then
            ...
            "VAT Base LCY (Non Deduct.)" := 0;
            "VAT Amount LCY (Non Deduct.)" := 0;
        end;
    end;
    ...
    begin
        // NAVCZ
        with GenJnlLine do begin
            "VAT % (Non Deductible)" := TempGenJournalLine."VAT % (Non Deductible)";
            "VAT Base (Non Deductible)" := TempGenJournalLine."VAT Base (Non Deductible)";
            "VAT Amount (Non Deductible)" := TempGenJournalLine."VAT Amount (Non Deductible)";
            "VAT Base LCY (Non Deduct.)" := TempGenJournalLine."VAT Base LCY (Non Deduct.)";
            "VAT Amount LCY (Non Deduct.)" := TempGenJournalLine."VAT Amount LCY (Non Deduct.)";
        end;
    end;
    ...
    begin
        // NAVCZ
        PostFromPurchDoc := NewPostFromPurchDoc;
    end;

    [Scope('OnPrem')]
    procedure SetPostFromPurchAdvLetter(NewPostFromPurchAdvLetter: Boolean)
    begin
        // NAVCZ
    ...
    begin
        ...
        with GenJnlLine do
            case true of
                ("Account Type" = "Account Type"::"Bank Account") and (Amount <> 0):
                    CheckBankAccount("Account No.");
                ("Bal. Account Type" = "Bal. Account Type"::"Bank Account") and (Amount <> 0):
                    CheckBankAccount("Bal. Account No.");
            end;
    end;
    ...
    begin
        ...
        if BankAccount."Account Type" = BankAccount."Account Type"::"Cash Desk" then
            if not PostFromCashReg then
                BankAccount.FieldError("Account Type");
    end;

    local procedure GetPostingAccountNo(VATPostingSetup: Record "VAT Posting Setup"; VATEntry: Record "VAT Entry"; UnrealizedVAT: Boolean; PostponedVAT: Boolean): Code[20]
    var
        TaxJurisdiction: Record "Tax Jurisdiction";
    begin
        ...
        case VATEntry.Type of
            VATEntry.Type::Sale:
                begin
                    if UnrealizedVAT and PostponedVAT then begin
                        VATPostingSetup.TestField("Sales VAT Postponed Account");
                        exit(VATPostingSetup."Sales VAT Postponed Account");
                    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeInitGLEntry(var GenJournalLine: Record "Gen. Journal Line"; GLAccNo: Code[20]; SystemCreatedEntry: Boolean; Amount: Decimal; AmountAddCurr: Decimal)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeInitVAT(var GenJournalLine: Record "Gen. Journal Line"; var GLEntry: Record "G/L Entry"; var VATPostingSetup: Record "VAT Posting Setup"; var IsHandled: Boolean)
    begin
    end;
