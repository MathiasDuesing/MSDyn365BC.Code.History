codeunit 12 "Gen. Jnl.-Post Line"
{
    ...
    begin
        ...
        if not IsHandled then begin
            with GenJnlLine do begin
                ...
                if "Derogatory Line" then begin
                    MakeDerogFAJnlLine(FAJnlLine, GenJnlLine);
                    if "FA Error Entry No." <> 0 then
                        FAJnlLine."FA Error Entry No." := FAJnlPostLine.GetNextMatchingFALedgEntry(FAJnlLine, "FA Error Entry No.", FAJnlLine."Depreciation Book Code");
    ...
    begin
        with FAJnlLine do begin
            ...
            if not FAJnlSetup.Get("Depreciation Book Code", UserId) then
                FAJnlSetup.Get("Depreciation Book Code", '');
            "Journal Template Name" := FAJnlSetup."FA Jnl. Template Name";
            "Journal Batch Name" := FAJnlSetup."FA Jnl. Batch Name";
            "FA Posting Type" := "FA Journal Line FA Posting Type".FromInteger(GenJnlLine."FA Posting Type".AsInteger() - 1);
            "FA No." := GenJnlLine."Account No.";
            if GenJnlLine."FA Posting Date" <> 0D then
                "FA Posting Date" := GenJnlLine."FA Posting Date"
            else
                "FA Posting Date" := GenJnlLine."Posting Date";
            "Posting Date" := GenJnlLine."Posting Date";
            ...
            Validate(Amount, GenJnlLine."VAT Base Amount");
