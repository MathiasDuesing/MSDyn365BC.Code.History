codeunit 13 "Gen. Jnl.-Post Batch"
{
            ...
            repeat
                ...
                if not IsHandled then
                    if ForceCheckBalance or ("Posting Date" <> LastDate) or GenJnlTemplate."Force Doc. Balance" and
                       ((("Document Type" <> LastDocType) and (not GenJnlTemplate."Not Check Doc. Type")) or ("Document No." <> LastDocNo))
                    then begin
    ...
    begin
        with GenJournalLine do
            if ("Posting Date" <> LastDate) or ("Document Type" <> LastDocType) or ("Document No." <> LastDocNo) then begin
                ...
                // NAVCZ
                CheckCorrection := (Correction <> DocCorrection) and not GenJnlTemplate."Not Check Correction";
                OnBeforeCheckCorrection(GenJnlTemplate, GenJnlBatch, GenJournalLine, CheckCorrection);
                if CheckCorrection then
                    ...
                    if GLSetup."Check Posting Debit/Credit" then begin
                        ...
                        case GLAccount."Debit/Credit" of
                            GLAccount."Debit/Credit"::Debit:
                                Validate("Debit Amount", -GLAcc."Net Change");
                            GLAccount."Debit/Credit"::Credit:
                                Validate("Credit Amount", GLAcc."Net Change");
        ...
        with GenJnlLine do begin
            ...
            then begin
                ...
                if "Advance Letter Link Code" <> '' then
                    ...
                    SalesAdvanceLetterLine.SetRange("Applies-to ID", "Document No.");
                end;
            ...
            then begin
                ...
                if "Advance Letter Link Code" <> '' then
                    ...
                    TestField("Document No.");
                    PurchAdvanceLetterLine.SetRange("Applies-to ID", "Document No.");
                end;
    ...
    begin
        with GenJournalLine do begin
            ...
            if ("Recurring Method".AsInteger() >= "Recurring Method"::"RF Reversing Fixed".AsInteger()) and ("Posting Date" <> 0D) and ("Recurring Method".AsInteger() <> "Recurring Method"::"BD Balance by Dimension".AsInteger()) then begin
                ...
                then begin
                    GLAccount.Get(TempGenJnlLine4."Account No.");
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeCheckCorrection(var GenJnlTemplate: Record "Gen. Journal Template"; var GenJnlBatch: Record "Gen. Journal Batch"; var GenJnlLine: Record "Gen. Journal Line"; var CheckCorrection: Boolean)
    begin
    end;
