codeunit 13 "Gen. Jnl.-Post Batch"
{
    ...
    begin
        ...
        with GenJournalLine do
            if ("Posting Date" <> LastDate) or ("Document Type" <> LastDocType) or ("Document No." <> LastDocNo) or
               (CurrentBalance = 0)
            then begin
    ...
    begin
        with GenJournalLine do
            if PaymentApplication and PaymentBalanceVAT then begin
                PaymentApplication := false;
                PaymentBalanceVAT := false;
                Error(PmtApplnErr, "Journal Template Name", "Journal Batch Name", "Line No.");
            end;
    end;
    ...
    begin
        with GenJnlLine do
            exit(
              (("Document Type" in ["Document Type"::Payment, "Document Type"::Refund]) and
               (("Applies-to Doc. No." <> '') or ("Applies-to ID" <> ''))) and
              ((("Account Type" in ["Account Type"::Customer, "Account Type"::Vendor]) and ("Account No." <> '')) or
               (("Bal. Account Type" in ["Account Type"::Customer, "Account Type"::Vendor]) and ("Bal. Account No." <> ''))));
    end;
    ...
    begin
        with GenJnlLine do
            exit(
              ("Document Type" in ["Document Type"::Payment, "Document Type"::Refund]) and
              (("Account Type" = "Account Type"::"G/L Account") and ("Account No." <> '') and
               ("Gen. Posting Type" <> "Gen. Posting Type"::" ")) or
              (("Bal. Account Type" = "Account Type"::"G/L Account") and ("Bal. Account No." <> '') and
               ("Bal. Gen. Posting Type" <> "Gen. Posting Type"::" ")));
    end;
