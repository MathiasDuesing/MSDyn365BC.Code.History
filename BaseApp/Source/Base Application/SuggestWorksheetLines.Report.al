...
report 840 "Suggest Worksheet Lines"
{
    ...
    dataset
    {
        dataitem("Cash Flow Forecast"; "Cash Flow Forecast")
        {
            ...
            {
                DataItemTableView = SORTING("Letter No.", "Line No.") WHERE(Status = CONST("Pending Advance Payment"));

                trigger OnAfterGetRecord()
                begin
                    Window.Update(2, SalesAdvancesMsg);
                    Window.Update(3, "Letter No.");

                    SalesAdvanceLetterHeader.Get("Letter No.");
                    if SalesAdvanceLetterHeader."Bill-to Customer No." <> '' then
                        Customer.Get(SalesAdvanceLetterHeader."Bill-to Customer No.")
                    else
                        Customer.Init();

                    InsertCFLineForSalesAdvanceLetterLine;
                end;

                trigger OnPreDataItem()
                begin
                    if not ConsiderSource["Cash Flow Source Type"::"Sales Advance Letters"] then
                        CurrReport.Break();
            ...
            dataitem("Purch. Advance Letter Line"; "Purch. Advance Letter Line")
            {
                ...
                trigger OnAfterGetRecord()
                begin
                    Window.Update(2, PurchaseAdvancesMsg);
                    Window.Update(3, "Letter No.");

                    PurchAdvanceLetterHeader.Get("Letter No.");
                    if PurchAdvanceLetterHeader."Pay-to Vendor No." <> '' then
                        Vendor.Get(PurchAdvanceLetterHeader."Pay-to Vendor No.")
                    else
                        Vendor.Init();

                    InsertCFLineForPurchAdvanceLetterLine;
                end;

                trigger OnPreDataItem()
                begin
                    if not ConsiderSource["Cash Flow Source Type"::"Purchase Advance Letters"] then
                        CurrReport.Break();
    ...
    requestpage
    {
            ...
            area(content)
            {
                group(Options)
                {
                    ...
                    group("Source Types to Include:")
                    {
                        ...
                        field("ConsiderSource[SourceType::""Sales Advance Letters""]"; ConsiderSource["Cash Flow Source Type"::"Sales Advance Letters"])
                        {
                            ApplicationArea = Basic, Suite;
                        ...
                        field("ConsiderSource[SourceType::""Purchase Advance Letters""]"; ConsiderSource["Cash Flow Source Type"::"Purchase Advance Letters"])
                        {
                            ApplicationArea = Basic, Suite;
        ...
        begin
            ...
            if ConsiderSource["Cash Flow Source Type"::"Sales Advance Letters".AsInteger()] then
                ConsiderSource["Cash Flow Source Type"::"Sales Advance Letters".AsInteger()] := "Sales Advance Letter Line".ReadPermission;
            if ConsiderSource["Cash Flow Source Type"::"Purchase Advance Letters".AsInteger()] then
                ConsiderSource["Cash Flow Source Type"::"Purchase Advance Letters".AsInteger()] := "Purch. Advance Letter Line".ReadPermission;
        end;
    }
    ...
    begin
        ...
        then begin
            ...
            if "Cash Flow Forecast"."Consider CF Payment Terms" and (Customer."Cash Flow Payment Terms Code" <> '') then
                PaymentTermsCode := Customer."Cash Flow Payment Terms Code"
            else
                PaymentTermsCode := SalesAdvanceLetterHeader."Payment Terms Code";

            InsertCFLineForAdvanceLetterLine(
              SalesAdvanceLetterHeader."Document Date",
              CFWorksheetLine2."Source Type"::"Sales Advance Letters",
              "Sales Advance Letter Line"."Letter No.",
              SalesAdvanceLetterHeader."Shortcut Dimension 1 Code",
              SalesAdvanceLetterHeader."Shortcut Dimension 2 Code",
              SalesAdvanceLetterHeader."Dimension Set ID",
              CFSetup."S. Adv. Letter CF Account No.",
              CopyStr(
                StrSubstNo(
                  Text025,
              ...
              SalesAdvanceLetterHeader."Advance Due Date",
              "Sales Advance Letter Line"."Letter No.",
              CalculateLineAmountForSalesAdvanceLetterLine(SalesAdvanceLetterHeader, "Sales Advance Letter Line"),
              PaymentTermsCode);
        end;
    end;
    ...
    begin
        ...
        then begin
            ...
            if "Cash Flow Forecast"."Consider CF Payment Terms" and (Vendor."Cash Flow Payment Terms Code" <> '') then
                PaymentTermsCode := Vendor."Cash Flow Payment Terms Code"
            else
                PaymentTermsCode := PurchAdvanceLetterHeader."Payment Terms Code";

            InsertCFLineForAdvanceLetterLine(
              PurchAdvanceLetterHeader."Document Date",
              CFWorksheetLine2."Source Type"::"Purchase Advance Letters",
              "Purch. Advance Letter Line"."Letter No.",
              PurchAdvanceLetterHeader."Shortcut Dimension 1 Code",
              PurchAdvanceLetterHeader."Shortcut Dimension 2 Code",
              PurchAdvanceLetterHeader."Dimension Set ID",
              CFSetup."P. Adv. Letter CF Account No.",
              CopyStr(
                StrSubstNo(
                  Text025,
                ...
                1, MaxStrLen(CFWorksheetLine2.Description)),
              "Purch. Advance Letter Line"."Advance Due Date",
              "Purch. Advance Letter Line"."Letter No.",
              CalculateLineAmountForPurchAdvanceLetterLine(PurchAdvanceLetterHeader, "Purch. Advance Letter Line"),
              PaymentTermsCode);
        end;
    end;
    ...
    begin
        with CFWorksheetLine2 do begin
            ...
            "Cash Flow Account No." := CashFlowAccountNo;
            Description := Description2;
            SetCashFlowDate(CFWorksheetLine2, CashFlowDate);
    ...
    var
        ...
        AmountToLinkLCY: Decimal;
        OrderAmount: Decimal;
        OrderAmountLCY: Decimal;
    begin
        // NAVCZ
        ...
        AdvanceLetterLineRelation.SetRange("Document Type", AdvanceLetterLineRelation."Document Type"::Order);
        AdvanceLetterLineRelation.CalcSums(Amount);
        OrderAmount := AdvanceLetterLineRelation.Amount;

        if SalesAdvanceLetterHeader2."Currency Code" <> '' then begin
            Currency.Get(SalesAdvanceLetterHeader2."Currency Code");
            AmountToLinkLCY :=
              Round(
                CurrExchRate.ExchangeAmtFCYToLCY(
                  SalesAdvanceLetterHeader2."Posting Date", SalesAdvanceLetterHeader2."Currency Code",
                  SalesAdvanceLetterLine."Amount To Link", SalesAdvanceLetterHeader2."Currency Factor"),
                Currency."Amount Rounding Precision");
            OrderAmountLCY :=
              Round(
                CurrExchRate.ExchangeAmtFCYToLCY(
                  SalesAdvanceLetterHeader2."Posting Date", SalesAdvanceLetterHeader2."Currency Code",
                  OrderAmount, SalesAdvanceLetterHeader2."Currency Factor"),
                Currency."Amount Rounding Precision");
        end else begin
            AmountToLinkLCY := SalesAdvanceLetterLine."Amount To Link";
            OrderAmountLCY := OrderAmount;
        end;
    ...
    begin
        ...
        if PurchAdvanceLetterHeader2."Currency Code" <> '' then begin
            Currency.Get(PurchAdvanceLetterHeader2."Currency Code");
            AmountToLinkLCY :=
              Round(
                CurrExchRate.ExchangeAmtFCYToLCY(
                  PurchAdvanceLetterHeader2."Posting Date", PurchAdvanceLetterHeader2."Currency Code",
                  PurchAdvanceLetterLine."Amount To Link", PurchAdvanceLetterHeader2."Currency Factor"),
                Currency."Amount Rounding Precision");
            OrderAmountLCY :=
              Round(
                CurrExchRate.ExchangeAmtFCYToLCY(
                  PurchAdvanceLetterHeader2."Posting Date", PurchAdvanceLetterHeader2."Currency Code",
                  OrderAmount, PurchAdvanceLetterHeader2."Currency Factor"),
                Currency."Amount Rounding Precision");
        end else begin
            AmountToLinkLCY := PurchAdvanceLetterLine."Amount To Link";
            OrderAmountLCY := OrderAmount;
        end;
