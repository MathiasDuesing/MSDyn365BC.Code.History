codeunit 7307 "Whse.-Activity-Register"
{
    ...
    begin
        with WhseActivLine do begin
            ...
            if not TempBinContentBuffer.Get("Location Code", "Bin Code", "Item No.", "Variant Code", UOMCode, "Lot No.", "Serial No.", "CD No.")
            then begin
            ...
            then begin
                ...
                if "Activity Type" <> "Activity Type"::"Invt. Movement" then // Invt. Movement from Dedicated Bin is allowed
                    QtyOnDedicatedBinsBase :=
                        WhseAvailMgt.CalcQtyOnDedicatedBins("Location Code", "Item No.", "Variant Code", "Lot No.", "Serial No.", "CD No.");
                ...
                if SubTotalBase < 0 then begin
                    CreatePick.FilterWhsePickLinesWithUndefinedBin(
                      ...
                      WhseItemTrackingSetup."CD No. Required", "CD No.");
    ...
    local procedure CheckItemTrackingInfoBlocked(ItemNo: Code[20]; VariantCode: Code[10]; SerialNo: Code[50]; LotNo: Code[50]; CDNo: Code[30])
    var
    ...
    begin
        ...
        if CDNo <> '' then
            if CDNoInfo.Get(CDNoInfo.Type::Item, ItemNo, VariantCode, CDNo) then
                ...
                repeat
                    ...
                    then
                        CheckItemTrackingInfoBlocked(
                          WhseActivLine."Item No.", WhseActivLine."Variant Code", WhseActivLine."Serial No.", WhseActivLine."Lot No.", WhseActivLine."CD No.");
                until WhseActivLine.Next = 0;
        ...
        if WhseItemTrackingSetup.TrackingRequired() then
            if Item."Reserved Qty. on Inventory" > 0 then begin
                ...
                LateBindingMgt.ReleaseForReservation(
                  ...
                  WhseActivLine."Serial No.", WhseActivLine."Lot No.", WhseActivLine."CD No.", QtyToRelease);
                Item.CalcFields("Reserved Qty. on Inventory");
            end;
    ...
    begin
        ...
        if WhseActivLine."CD No." <> '' then
            Error(Text005, WhseActivLine.FieldCaption("CD No."), WhseActivLine."CD No.");
    end;
