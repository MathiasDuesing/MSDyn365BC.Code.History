                ...
                case "Entry Type" of
                    ...
                    "Entry Type"::Output:
                        begin
                            ...
                            if ShouldCopyFromSingleProdOrderLine then begin
...
#pragma warning restore
                ...
                if "Applies-from Entry" <> 0 then begin
                    ...
                    if "Entry Type" = "Entry Type"::Consumption then begin
                        ...
                        if ManufSetup."Exact Cost Rev.Manda. (Cons.)" then begin
                            ItemLedgEntry.TestField("Order No.", "Order No.");
                            ItemLedgEntry.TestField("Order Line No.", "Order Line No.");
                            ItemLedgEntry.TestField("Prod. Order Comp. Line No.", "Prod. Order Comp. Line No.");
                            ItemLedgEntry.TestField("Entry Type", "Entry Type");
                        end;
                    end;
    ...
    begin
        ...
        if ("Entry Type" = "Entry Type"::Consumption) and
           ("Value Entry Type" <> "Value Entry Type"::Revaluation) and
           (CurrentFieldNo = FieldNo("Applies-from Entry"))
        then begin
            ManufSetup.Get();
            if ManufSetup."Exact Cost Rev.Manda. (Cons.)" then begin
                ItemLedgEntry.SetCurrentKey(
                ...
                ItemLedgEntry.SetRange("Order No.", "Order No.");
                ItemLedgEntry.SetRange("Order Line No.", "Order Line No.");
                ItemLedgEntry.SetRange("Prod. Order Comp. Line No.", "Prod. Order Comp. Line No.");
                ItemLedgEntry.SetRange("Entry Type", "Entry Type");
            end;
        end;
        // NAVCZ
    ...
    begin
        ...
        if "Intrastat Transaction" and Item.IsInventoriableType() then begin
            ...
            if StatReportingSetup."Transaction Spec. Mandatory" and ("Transaction Specification" = '') then
                Error(Text26500, FieldCaption("Transaction Specification"), "Item No.");
            ...
            if StatReportingSetup."Net Weight Mandatory" and ("Net Weight" = 0) then
                Error(Text26500, FieldCaption("Net Weight"), "Item No.");
            if StatReportingSetup."Country/Region of Origin Mand." and
               ("Country/Region of Origin Code" = '') and ("Entry Type" <> "Entry Type"::Sale)
            then
                Error(Text26500, FieldCaption("Country/Region of Origin Code"), "Item No.");
        end;
    end;
