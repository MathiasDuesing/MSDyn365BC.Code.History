page 6510 "Item Tracking Lines"
{
    ...
    layout
    {
        area(content)
        {
            ...
            {
                ShowCaption = false;
                field(AvailabilitySerialNo; TrackingAvailable(Rec, ItemTrackingType::"Serial No."))
                {
                    ApplicationArea = ItemTracking;
                    ...
                    trigger OnDrillDown()
                    begin
                        LookupAvailable(ItemTrackingType::"Serial No.");
                    end;
                }
                field("Serial No."; "Serial No.")
                {
                    ...
                    begin
                        ...
                        "Bin Code" := ForBinCode;
                        if ("Source Type" = DATABASE::"Transfer Line") and (FormRunMode = FormRunMode::Reclass) then
                        ...
                        ItemTrackingDataCollection.AssistEditTrackingNo(Rec,
                          ...
                          InsertIsBlocked, CurrentSignFactor, ItemTrackingType::"Serial No.", MaxQuantity);
                        "Bin Code" := '';
                        CurrPage.Update;
                    end;
                    ...
                    var
                        ...
                        LotNo: Code[50];
                    begin
                        Clear(SerialNoInfoList);
                        SerialNoInfo.SetRange("Item No.", "Item No.");
                        SerialNoInfo.SetRange("Variant Code", "Variant Code");
                        SerialNoInfo."Serial No." := "Serial No.";
                        SerialNoInfoList.SetTableView(SerialNoInfo);
                        SerialNoInfoList.SetRecord(SerialNoInfo);
                        SerialNoInfoList.LookupMode(true);
                        if SerialNoInfoList.RunModal = ACTION::LookupOK then begin
                            SerialNoInfoList.GetRecord(SerialNoInfo);
                ...
                field(AvailabilityLotNo; TrackingAvailable(Rec, ItemTrackingType::"Lot No."))
                {
                    ApplicationArea = ItemTracking;
                    ...
                    trigger OnDrillDown()
                    begin
                        LookupAvailable(ItemTrackingType::"Lot No.");
                    end;
                }
                field("Lot No."; "Lot No.")
                {
                    ...
                    begin
                        ...
                        ItemTrackingDataCollection.AssistEditTrackingNo(Rec,
                          (CurrentSignFactor * SourceQuantityArray[1] < 0) and not
                          InsertIsBlocked, CurrentSignFactor, ItemTrackingType::"Lot No.", MaxQuantity);
                        "Bin Code" := '';
                        CurrPage.Update;
                    end;
                ...
                field("TrackingAvailable(Rec,2)"; TrackingAvailable(Rec, "Item Tracking Type"::"CD No."))
                {
                    ApplicationArea = ItemTracking;
                    Caption = 'Availability, CD No.';
                    Editable = false;
                    ToolTip = 'CD No. Availability';

                    trigger OnDrillDown()
                    begin
                        LookupAvailable("Item Tracking Type"::"CD No.");
                    end;
                }
                field("CD No."; "CD No.")
                {
                    ApplicationArea = ItemTracking;
                    Editable = CDNoEditable;
                    ...
                    begin
                        ...
                        ItemTrackingDataCollection.AssistEditTrackingNo(Rec,
                          (CurrentSignFactor * SourceQuantityArray[1] < 0) and not
                          InsertIsBlocked, CurrentSignFactor, "Item Tracking Type"::"CD No.", MaxQuantity);
                        "Bin Code" := '';
                        CurrPage.Update;
                    end;
                    ...
                    begin
                        ...
                        CDNoInfoList.SetTableView(CDNoInfo);
                        CDNoInfoList.SetRecord(CDNoInfo);
                        CDNoInfoList.LookupMode(true);
                        if CDNoInfoList.RunModal = ACTION::LookupOK then begin
                            CDNoInfoList.GetRecord(CDNoInfo);
                            "CD No." := CDNoInfo."CD No.";
                        end;
                    end;
                }
                field("New CD No."; "New CD No.")
                {
                    ApplicationArea = ItemTracking;
                    Editable = NewCDNoEditable;
                    Visible = NewCDNoVisible;
                }
    ...
    var
        ...
        Text12490: Label 'Tracking Specification with Serial No. %1 and Lot No. %2 and CD No. %3 already exists.', Comment = '%1 = Serial No., %2 = Lot No. and %3 = CD No.';
        ...
        case Controls of
            ...
            Controls::Reclass:
                begin
                    ...
                    NewCDNoVisible := SetAccess;
                    NewCDNoEditable := SetAccess;
                    ...
                    repeat
                        if xTempItemTrackingLine.Get(TempItemTrackLineModify."Entry No.") then begin
                            ...
                            if ((ModifyLoop = 1) and Decrease) or ((ModifyLoop = 2) and not Decrease) then begin
                                i := i + 1;
                                if (not xTempItemTrackingLine.HasSameTracking(TempItemTrackLineModify)) or
    ...
    begin
        ...
        if (TrackingSpecification."Lot No." <> '') or (TrackingSpecification."CD No." <> '') then begin
            if (TrackingSpecification."Source Type" = DATABASE::"Transfer Line") and
    ...
    begin
        ...
        if Exists and CurrentPageIsOpen then
            if "Serial No." = '' then
                Message(Text12490, "Serial No.", "Lot No.", "CD No.")
            else
