codeunit 5601 "FA Insert G/L Account"
{
    ...
    begin
        ...
        FAGLPostBuf."Disposal Entry No." := "Disposal Entry No.";
        ...
        if "Disposal Entry No." in [-1, 1] then
            case "FA Posting Category" of
              ...
              "FA Posting Category"::"Bal. Disposal":
                    Flag := true;
                else begin
                    ...
                    if NewAmount <> 0 then begin
                        ...
                        if Flag then begin
                            ...
                            if FAPostingGr."Sales Bal. Acc." <> '' then begin
                                FAGLPostBuf."Account No." := FAPostingGr."Sales Bal. Acc.";
                                FAGLPostBuf.Amount := -NewAmount;
                                FAGLPostBuf.Correction := Correction;
                                FAGLPostBuf."Global Dimension 1 Code" := "Global Dimension 1 Code";
                                FAGLPostBuf."Global Dimension 2 Code" := "Global Dimension 2 Code";
    ...
    var
        ReturnValue: Boolean;
    begin
        TempFAGLPostBuf.SetCurrentKey("Disposal Entry No.", "Entry No.");
    ...
    [Scope('OnPrem')]
    procedure InsertDispAcc(var FALedgEntry: Record "FA Ledger Entry"; Convert: Boolean)
    var
        FA: Record "Fixed Asset";
    ...
    begin
        with FALedgEntry do begin
            ...
            FAPostingGr.TestField("Sales Bal. Acc.");
            FAGLPostBuf."Account No." := FAPostingGr."Sales Bal. Acc.";
            FAGLPostBuf.Amount := Amount;
            if Convert then
                FAGLPostBuf.Amount := -FAGLPostBuf.Amount;
            FAGLPostBuf.Correction := Correction;
            FAGLPostBuf."Global Dimension 1 Code" := "Global Dimension 1 Code";
            FAGLPostBuf."Global Dimension 2 Code" := "Global Dimension 2 Code";
            FAGLPostBuf."Dimension Set ID" := "Dimension Set ID";
            FAGLPostBuf."FA Entry No." := "Entry No.";
            if not Convert then
                FAGLPostBuf."Disposal Entry No." := -1  //for sorting
            else
                FAGLPostBuf."Disposal Entry No." := "Disposal Entry No.";
            if "Entry No." > 0 then
                FAGLPostBuf."FA Entry Type" := FAGLPostBuf."FA Entry Type"::"Fixed Asset";
