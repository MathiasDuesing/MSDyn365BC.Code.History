...
page 50 "Purchase Order"
{
    ...
    layout
    {
        area(content)
        {
            ...
            {
                ObsoleteState = Pending;
                ObsoleteReason = 'Will be replaced by standard group Prepayment.';
                ObsoleteTag = '20.0';
#else
            group(Prepayment)
            {
...
#pragma warning restore
        ...
        area(processing)
        {
            ...
            group(Action1220036)
            {
                ...
                action("Link Advance Letter")
                {
                    ...
                    begin
                        // NAVCZ
                        AdvLetterLinkingCard.SetPurchDoc("Document Type", "No.");
                        AdvLetterLinkingCard.LookupMode := true;
                        if AdvLetterLinkingCard.RunModal() = ACTION::LookupOK then begin
                ...
                action("Cancel All Adv. Payment Relations")
                {
                    ...
                    begin
                        // NAVCZ
                        if Confirm(CancelAllRelationsQst, false) then
                            CancelAllRelations;
                        // NAVCZ
                    end;
                }
                action("Adjust VAT by Adv. Payment Deduction")
                {
                    ...
                    begin
                        PurchPostAdvances.CorrectVATbyDeductedVAT(Rec); // NAVCZ
                    end;
                }
            }
#endif
    ...
    var
        AdvLetter: Record "Purch. Advance Letter Header";
        PurchAPTempl: Record "Purchase Adv. Payment Template";
        PurchPostAdvances: Codeunit "Purchase-Post Advances";
        AdvLetterNo: Code[20];
    begin
        // NAVCZ
        if not PurchAPTempl.IsEmpty() then
            if not (PAGE.RunModal(PAGE::"Purchase Adv. Paym. Selection", PurchAPTempl) = ACTION::LookupOK) then
                Error('');
