table 23 Vendor
{
    ...
    fields
    {
        ...
        field(12401; "OKPO Code"; Code[10])
        {
            Caption = 'OKPO Code';
        }
        field(12402; "OKONX Code"; Code[10])
        {
            Caption = 'OKONX Code';
        }
        field(12403; "Customer No."; Code[20])
        {
            Caption = 'Customer No.';
        ...
        field(12405; "Employee No."; Code[20])
        {
            ...
            begin
                TestField("Vendor Type", "Vendor Type"::"Resp. Employee");
                ...
                if "Employee No." <> xRec."Employee No." then begin
                    ...
                    if not VendLedgerEntry.IsEmpty then
                        Error(Text012, FieldCaption("Employee No."));
                end;
            end;
        }
        field(12406; "Tax Authority Code"; Code[8])
        {
        ...
        field(12413; "VAT Agent Type"; Option)
        {
            Caption = 'VAT Agent Type';
            OptionCaption = ' ,Non-resident,State Authority';
            OptionMembers = " ","Non-resident","State Authority";
        }
        ...
        field(12481; "Control Tax Organ"; Text[30])
        {
            Caption = 'Control Tax Organ';
        }
        field(12490; "Agreement Posting"; Option)
        {
            ...
            begin
                ...
                if "Agreement Posting" = "Agreement Posting"::"No Agreement" then begin
                    ...
                    VendLedgEntry.SetRange(Open, true);
                    if VendLedgEntry.FindFirst then
                        Error(Text010, FieldCaption("Agreement Posting"), TableCaption);
                end;
            end;
        }
        field(12491; "Agreement Filter"; Code[20])
        {
            ...
            FieldClass = FlowFilter;
            TableRelation = "Vendor Agreement"."No." WHERE("Vendor No." = FIELD("No."));
        }
    ...
    begin
        ...
        "Agreement Nos." := PurchSetup."Vendor Agreement Nos.";
    ...
    var
        ...
        Text012: Label 'The field %1 cannot be changed for a vendor with ledger entries.';
    ...
    [Scope('OnPrem')]
    procedure CopyFromCustomer(var Vend: Record Vendor)
    var
        Cust: Record Customer;
        Text12400: Label 'Copy General information\from Customer %3 (%4)\to Vendor %2 (%1)?';
    begin
        Vend.TestField("Customer No.");
        Vend.CalcFields("Customer Name");

        if Cust.Get("Customer No.") then begin
            if Confirm(Text12400, true, Name, "No.", "Customer No.", "Customer Name") then begin
                Vend.Get("No.");
                Vend.Name := Cust.Name;
                Vend."Search Name" := Cust."Search Name";
                Vend."Name 2" := Cust."Name 2";
                Vend.Address := Cust.Address;
                Vend."Address 2" := Cust."Address 2";
                Vend.City := Cust.City;
                Vend.Contact := Cust.Contact;
                Vend."Phone No." := Cust."Phone No.";
                Vend."Telex No." := Cust."Telex No.";
                Vend."Fax No." := Cust."Fax No.";
                Vend."E-Mail" := Cust."E-Mail";
                Vend."Home Page" := Cust."Home Page";
                Vend."Post Code" := Cust."Post Code";
    ...
    begin
        with GenJnlLine do begin
            ...
            PurchHeader.Get(PurchHeader."Document Type"::Order, "Prepayment Document No.");
            PurchHeader.TestField("Buy-from Vendor No.", "Account No.");
            PurchHeader.TestField("Prices Including VAT", true);
            PurchHeader.TestField(Status, PurchHeader.Status::Released);
            PurchHeader.TestField("Agreement No.", "Agreement No.");
            if not Post then
