                        ...
                        if DefaultDim.Find('-') then begin
                            repeat
                                if TempSelectedDim.Get(
                                     UserId, 3, REPORT::"Calculate Inventory", '', DefaultDim."Dimension Code")
                                then begin
                                    TempDimBufIn.Init();
                                    TempDimBufIn."Table ID" := DATABASE::"Item Ledger Entry";
                                    TempDimBufIn."Entry No." := 0;
                                    TempDimBufIn."Dimension Code" := DefaultDim."Dimension Code";
                                    TempDimBufIn."Dimension Value Code" := DefaultDim."Dimension Value Code";
                                    TempDimBufIn.Insert();
                                end;
        ...
        with ItemJnlLine do begin
            ...
            if (Quantity2 <> 0) or ZeroQty then begin
                ...
                if Location."Bin Mandatory" then
                    "Dimension Set ID" := 0;
                if not CardDim then begin // NAVCZ
