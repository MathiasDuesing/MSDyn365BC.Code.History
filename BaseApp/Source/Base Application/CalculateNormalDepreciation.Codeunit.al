codeunit 5611 "Calculate Normal Depreciation"
{
            ...
            then begin
                if (FADeprBook."Depreciation Starting Date" <= DeprPeriod) and not SkipOnZero then begin
                    ...
                    case DepreciationCalc.ExistingDeprPeriods(FANo, DeprBookCode, Period) of
                        0:
                            if FADeprBook."Depreciation Starting Date" < DeprPeriod then //it's not first depr
                                Message(Text12400 + '\' + Text12402, Period, FANo, DeprBookCode)
                            else
                                OK := true;
                        1, 3:
                            Message(Text12401 + '\' + Text12402, Period, FANo, DeprBookCode);
                        2:
                            OK := true;
                    end;
    ...
    [Scope('OnPrem')]
    procedure CalcDBSLRUAmount(): Decimal
    var
        SLAmount: Decimal;
        DBAmount: Decimal;
    begin
    ...
    [Scope('OnPrem')]
    procedure CalcDBSLRUTaxGroupAmount(): Decimal
    var
        SLAmount: Decimal;
        DBAmount: Decimal;
        DBSLTaxAmount: Decimal;
    begin
