codeunit 5611 "Calculate Normal Depreciation"
{
    ...
    begin
        ...
        Custom1Amount2 := 0;
        Custom2Amount2 := 0;
        UseCustom1 := UseCustom11;
        UseCustom2 := UseCustom21;
        ForcedPercent1 := ForcedPercent11 / 100;
        ForcedPercent2 := ForcedPercent22 / 100;
        ...
        if not SkipRecord then begin
            ...
            then begin
                ...
                BookValueLeft := BookValue + DeprAmount;
                if (Custom1Amount <> 0) and (BookValueLeft <= -Custom1Amount) then begin
                    Custom1Amount2 := -BookValueLeft;
                    Custom2Amount2 := 0;
                end else begin
                    Custom1Amount2 := Sign * Custom1Amount;
                    BookValueLeft := BookValueLeft + Custom1Amount2;
                    if (Custom2Amount <> 0) and (BookValueLeft <= -Custom2Amount) then
                        Custom2Amount2 := -BookValueLeft
                    else
                        Custom2Amount2 := Sign * Custom2Amount;
                end;
    ...
    begin
        ...
        if UseCustom1 then begin
            if ForcedPercent1 = 0 then
                Custom1Amount := DepreciationCalc.CalcRounding(
                    DeprBook.Code,
                    -TableDeprCalc.GetTablePercent(DeprBook.Code, DeprTableCode,
                      FirstUserDefinedDeprDate, FirstDeprDate, EndingDate, 1) *
                    DeprBasis * Factor);
            if ForcedPercent1 <> 0 then
                Custom1Amount :=
                  DepreciationCalc.CalcRounding(DeprBook.Code, -ForcedPercent1 * DeprBasis * Factor);
        end;
        if UseCustom2 then begin
            if ForcedPercent2 = 0 then
                Custom2Amount :=
                  DepreciationCalc.CalcRounding(DeprBook.Code,
                    -TableDeprCalc.GetTablePercent(DeprBook.Code, DeprTableCode,
                      FirstUserDefinedDeprDate, FirstDeprDate, EndingDate, 2) *
                    DeprBasis * Factor);
            if ForcedPercent2 <> 0 then
                Custom2Amount :=
                  DepreciationCalc.CalcRounding(DeprBook.Code, -ForcedPercent2 * DeprBasis * Factor);
        end;
        exit(
          -TableDeprCalc.GetTablePercent(DeprBook.Code, DeprTableCode,
            FirstUserDefinedDeprDate, FirstDeprDate, EndingDate, 0) *
          DeprBasis * Factor);
    end;
