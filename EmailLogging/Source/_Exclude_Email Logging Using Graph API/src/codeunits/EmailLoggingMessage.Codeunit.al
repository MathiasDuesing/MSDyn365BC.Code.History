codeunit 1687 "Email Logging Message"
{
    ...
    internal procedure GetToAndCcRecipients(var Recipients: List of [Text]): Boolean
    begin
        Recipients.AddRange(ToRecipients);
        Recipients.AddRange(CcRecipients);
        exit(Recipients.Count() > 0);
    end;

    internal procedure GetToRecipients(var Recipients: List of [Text]): Boolean
    begin
        Recipients := ToRecipients;
        exit(Recipients.Count() > 0);
    end;

    internal procedure GetCcRecipients(var Recipients: List of [Text]): Boolean
    begin
    ...
    begin
        ...
        if not IsDraft then begin
            ...
            GetEmailListProperty(JsonObject, 'toRecipients', DictinctRecipients, ToRecipients);
            GetEmailListProperty(JsonObject, 'ccRecipients', DictinctRecipients, CcRecipients);
        end;
        Initialized := true;
    end;
    ...
    local procedure GetEmailListProperty(var JsonObject: JsonObject; PropertyName: Text; var DistinctEmails: Dictionary of [Text, Boolean]; var EmailList: List of [Text]): Boolean
    var
