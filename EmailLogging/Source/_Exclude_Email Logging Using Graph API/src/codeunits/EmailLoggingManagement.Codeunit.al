codeunit 1681 "Email Logging Management"
{
    ...
    begin
        if FeatureKey.Get(GetEmailLoggingUsingGraphApiFeatureKey()) then
    ...
    begin
        ...
        if GuiAllowed() then begin
            ...
            AssistedSetup.Run(Page::"Email Logging Setup Wizard");
        end;
    end;
    ...
    begin
        ...
        if AssistedSetup.Exists(Page::"Setup Email Logging") then begin
            AssistedSetup.Remove(Page::"Setup Email Logging");
            NeedCommit := true;
        end;

        if AssistedSetup.Exists(Page::"Email Logging Setup Wizard") then begin
            if NeedCommit then
                Commit();
            exit(false);
        end;
        ...
        AssistedSetup.Add(GetAppId(), Page::"Setup Email Logging", EmailLoggingSetupShortTitleTxt, AssistedSetupGroup::ApprovalWorkflows, '', VideoCategory::ApprovalWorkflows, EmailLoggingSetupHelpTxt, EmailLoggingSetupDescriptionTxt);
